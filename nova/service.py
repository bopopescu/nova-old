...
class ScheduledLaucher(object):
    """ The laucher will fork two process to do RPC tasks and periodic tasks.
    Force main woker don't do periodic tasks by set it's periodic_interval to
    None. And force periodic worker just do periodic tasks by invoked method
    start_periodic_task.
    """
    def __init__(self):
        self.running = True
    ...
    def _kill_children(self, *args):
        LOG.info(_('SIGTERM, SIGINT received, kill children'))
        self.runnning = False
        signal.signal(signal.SIGTERM, signal.SIG_DFL)
        signal.signal(signal.SIGINT, signal.SIG_DFL)
    ...
    def launch_server(self, server):
        self.server = server
        signal.signal(signal.SIGTERM, self._kill_children)
        signal.signal(signal.SIGINT, self._kill_children)
        self.start_main_task()
        time.sleep(1)
        self.start_peri_task()

    def start_main_task(self):
        pid = os.fork()
        if pid == 0:
            LOG.info(_('Running main task.'))
            signal.signal(signal.SIGHUP, signal.SIG_DFL)
            signal.signal(signal.SIGTERM, signal.SIG_DFL)
            signal.signal(signal.SIGINT, signal.SIG_IGN)
            eventlet.hubs.use_hub()

            #Note(gtt): do not run periodic task in main task
            self.server.periodic_interval = None
            self.server.start()
            os.write(self.write, self.cmd)
            self.server.wait()

            LOG.info(_('Main task exit!'))
            sys.exit(0)
        else:
            LOG.info(_('Main task pid is :%d'), pid)
            self.main_pid = pid

    def start_peri_task(self):
        pid = os.fork()
        if pid == 0:
            LOG.info(_('Running periodic task.'))
            signal.signal(signal.SIGHUP, signal.SIG_DFL)
            signal.signal(signal.SIGTERM, signal.SIG_DFL)
            signal.signal(signal.SIGINT, signal.SIG_IGN)
            eventlet.hubs.use_hub()

            # Use pipe to interative with main task process, when main task
            # finish doing pre-work, start to run periodic task.
            LOG.info(_('Waiting main task init host finished.'))
            os.read(self.read, 8)
            LOG.info(_('Main task init host finished. Start periodic task'))

            self.server.start_periodic_task()
            self.server.wait()

            LOG.info(_('Periodic task exit!'))
            sys.exit(0)
        else:
            LOG.info(_('Periodic task pid is :%d'), pid)
            self.peri_pid = pid

    def wait(self):
        while self.running:
            try:
                # use original wait method
                _os = eventlet.patcher.original('os')
                pid, status = _os.wait()

                # recover children if is killed by somebody
                if os.WIFSIGNALED(status):
                    sig = os.WTERMSIG(status)
                    ...
                    if self.running:
                        if pid == self.main_pid:
                            self.start_main_task()
                        elif pid == self.peri_pid:
                            self.start_peri_task()
                        else:
                            pass

                # Exit all process group, if something wrong.
                ...
                LOG.info(_('Keyboard interrupt, Exiting.'))
                break
...
class Service(object):
    ...
    def report_state(self):
        ...
        try:
            ...
            LOG.debug(_('Report state. %s') % state_catalog)
