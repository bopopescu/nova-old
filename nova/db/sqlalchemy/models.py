...
class ComputeNode(BASE, NovaBase):
    ...
    private_network_mbps_used = Column(Integer, default=0)
    public_network_mbps_used = Column(Integer, default=0)
    total_private_network_mbps = Column(Integer, default=0)
    total_public_network_mbps = Column(Integer, default=0)
...
class InstanceSystemMetadataExtension(BASE, NovaBase):
    """
    Represents a system-owned metadata extension key/value pair
    for an instance, value is Text type.
    """
    __tablename__ = 'instance_system_metadata_extension'
    id = Column(Integer, primary_key=True)
    key = Column(String(255))
    value = Column(Text)
    instance_uuid = Column(String(36),
    ...
    primary_join = ('and_(InstanceSystemMetadataExtension.instance_uuid == '
                    'Instance.uuid, '
                    'InstanceSystemMetadataExtension.deleted == False)')
    instance = relationship(Instance, backref="system_metadata_extension",
                            foreign_keys=instance_uuid,
                            primaryjoin=primary_join)
