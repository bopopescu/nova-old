...
@require_admin_context
def floating_ip_get_pools_usage(context):
    query = model_query(context, models.FloatingIp.pool,
                            func.count(models.FloatingIp.id)).\
                        filter(models.FloatingIp.project_id != None).\
                        group_by(models.FloatingIp.pool)
    result = {}
    for pool in query.all():
        pool_name = pool[0]
        pool_usage = pool[1]
        result.update({pool_name: pool_usage})
    return result


@require_admin_context
def floating_ip_get_pools_capacity(context):
    query = model_query(context, models.FloatingIp.pool,
                            func.count(models.FloatingIp.id)).\
                        group_by(models.FloatingIp.pool)
    result = {}
    for pool in query.all():
        pool_name = pool[0]
        pool_capacity = pool[1]
        result.update({pool_name: pool_capacity})
    return result
    ...
    with session.begin():
        if assign_address:
            floating_ip_ref = model_query(context, models.FloatingIp,
                                  ...
                                  filter_by(pool=pool).\
                                  filter_by(address=assign_address).\
...
@require_context
def floating_ip_count_by_project_type(context, project_id, is_private,
                                      private_pools=[], session=None):
    authorize_project_context(context, project_id)
    query = model_query(context, models.FloatingIp, read_deleted="no",
                       session=session).\
                   filter_by(project_id=project_id).\
                   filter_by(auto_assigned=False)
    if is_private:
        res = query.filter(models.FloatingIp.pool.in_(private_pools))
    else:
        res = query.filter(~models.FloatingIp.pool.in_(private_pools))
    return res.count()
...
@require_admin_context
def floating_ip_get_allocated(context):
...
@require_admin_context
def fixed_ip_associate_pool_in_subnet(context, network_id, subnet_cidr,
                                      start_ip=None, instance_uuid=None,
                                      host=None):
    session = get_session()
    with session.begin():
        network_or_none = or_(models.FixedIp.network_id == network_id,
                              models.FixedIp.network_id == None)
        fixed_ips_ref = model_query(context, models.FixedIp, session=session,
                                   read_deleted="no")

        # NOTE(stanzgy): fixed ips in database are arranged in ascendant order,
        #                so use ascendant id to identify the very fixed ips.
        subnet = netaddr.IPNetwork(subnet_cidr)
        subnet_ip = str(subnet.ip)
        subnet_broadcast = str(subnet.broadcast)

        if start_ip:
            start_ip = netaddr.IPAddress(start_ip)
            fixed_ip_by_location_start_ip = fixed_ips_ref.\
                                    filter_by(address=str(start_ip)).\
                                    first()
        else:
            fixed_ip_by_location_start_ip = fixed_ips_ref.\
                                    filter_by(address=str(subnet[0])).\
                                    first()

        if fixed_ip_by_location_start_ip is None:
            raise exception.FixedIpNotFoundForAddress(address=str(start_ip))
        else:
            fixed_ip_by_location_start_id = fixed_ip_by_location_start_ip['id']

        fixed_ip_end_id = fixed_ip_by_location_start_id + subnet.size
        fixed_ip_ref = fixed_ips_ref.filter(network_or_none).\
            filter_by(reserved=False).\
            filter_by(leased=False).\
            filter_by(instance_uuid=None).\
            filter_by(host=None).\
            filter(models.FixedIp.id >= fixed_ip_by_location_start_id).\
            filter(models.FixedIp.id < fixed_ip_end_id).\
            filter(models.FixedIp.address != subnet_ip).\
            filter(models.FixedIp.address != subnet_broadcast).\
            order_by(asc(models.FixedIp.id)).\
            with_lockmode('update').\
            first()

        if netaddr.IPAddress(fixed_ip_ref['address']) not in subnet:
            fixed_ip_ref = None
...
@require_admin_context
def fixed_ip_get_all_allocated(context, session=None):
    result = model_query(context, models.FixedIp, session=session).\
...
def fixed_ip_get_by_host(context, host):
    ...
    if not result:
        raise Exception(_("Can not get fixed ip for '%(host)s'"), locals())
...
@require_context
def instance_get_all_by_image_ref(context, image_ref, session=None):
    result = _build_instance_get(context, session=session).\
...
@require_admin_context
def instance_get_all_by_host_with_fixed_ip(context, host):
    fip_and = and_(models.FixedIp.instance_uuid == models.Instance.uuid,
                   models.FixedIp.deleted == False)

    session = get_session()
    result = session.query(models.Instance.id,
                           models.Instance.uuid,
                           models.Instance.user_id,
                           models.Instance.project_id,
                           models.Instance.host,
                           models.Instance.hostname,
                           models.Instance.server_name,
                           models.Instance.deleted,
                           models.Instance.image_ref,
                           models.Instance.power_state,
                           models.Instance.vm_state,
                           models.Instance.task_state,
                           models.Instance.availability_zone,
                           models.FixedIp.id,
                           models.FixedIp.address,
                           models.FixedIp.network_id,
                           models.FixedIp.deleted,
                           models.FixedIp.allocated,
                           models.FixedIp.leased,
                           models.FixedIp.reserved,
                           models.FixedIp.virtual_interface_id,
                           models.FixedIp.host
                           ).\
                           join((models.FixedIp, fip_and)).\
                           filter(models.FixedIp.instance_uuid != None).\
                           filter(models.FixedIp.deleted == False).\
                           filter(models.Instance.deleted == False).\
    ...
    for i in result:
        ...
        cleaned['uuid'] = i[1]
        cleaned['user_id'] = i[2]
        cleaned['project_id'] = i[3]
        cleaned['host'] = i[4]
        cleaned['hostname'] = i[5]
        cleaned['server_name'] = i[6]
        cleaned['deleted'] = i[7]
        cleaned['image_ref'] = i[8]
        cleaned['power_state'] = i[9]
        cleaned['vm_state'] = i[10]
        cleaned['task_state'] = i[11]
        cleaned['availability_zone'] = i[12]
        cleaned['fixed_ip_id'] = i[13]
        cleaned['fixed_ip_address'] = i[14]
        cleaned['fixed_ip_network_id'] = i[15]
        cleaned['fixed_ip_deleted'] = i[16]
        cleaned['fixed_ip_allocated'] = i[17]
        cleaned['fixed_ip_leased'] = i[18]
        cleaned['fixed_ip_reserved'] = i[19]
        cleaned['fixed_ip_virtual_interface_id'] = i[20]
        cleaned['fixed_ip_host'] = i[21]
...
@require_context
def instance_get_all_by_project_with_session(context, project_id, session):
    authorize_project_context(context, project_id)
    return model_query(context, models.Instance, project_only=False,
                        session=session).\
                        options(joinedload('info_cache')).\
                        options(joinedload('security_groups')).\
                        options(joinedload('metadata')).\
...
@require_admin_context
def instance_get_all_by_project_include_fixed_ip(context, project_id):
    fip_and = and_(models.FixedIp.instance_uuid == models.Instance.uuid,
                        models.FixedIp.deleted == False)
...
@require_context
def key_pair_update_fingerprint(context, user_id, name, fingerprint,
                                                        session=None):
    """
    Update keypairs fingerprint.
    Add this function to identify create or import action on keypairs by
    using fingerprint.
    When create keypair, fingerprint is add an extra string '.create' in
    extended_keypairs api, but import still is normal fingerprint.
    """
    if session is None:
        session = get_session()
    ...
    # update the value of fingerprint
    item = {"fingerprint": fingerprint}

    try:
        keypair_ref = key_pair_get(context, user_id, name, session)
    except exception.KeypairNotFound:
        raise

    keypair_ref.update(item)
    keypair_ref.save(session=session)
...
@require_admin_context
def quota_get_all(context):
    rows = model_query(context, models.Quota, read_deleted="no").\
                   all()

    results = {}
    for row in rows:
        result = results.get(row.project_id, {})
        result.update({'project_id': row.project_id})
        result[row.resource] = row.hard_limit
        results.update({row.project_id: result})
...
@require_admin_context
def quota_usage_get_all(context):
...
#######################
# System-owned metadata extension

def _instance_system_metadata_extension_get_query(context, instance_uuid,
                                                  session=None):
    return model_query(context, models.InstanceSystemMetadataExtension,
                       session=session).\
                    filter_by(instance_uuid=instance_uuid)


@require_context
def instance_system_metadata_extension_get(context, instance_uuid,
                                           session=None):
    rows = _instance_system_metadata_extension_get_query(context,
                                                         instance_uuid,
                                                         session=session).all()
...
@require_context
def instance_system_metadata_extension_delete(context, instance_uuid, key):
    _instance_system_metadata_extension_get_query(context, instance_uuid).\
        filter_by(key=key).\
        update({'deleted': True,
                'deleted_at': timeutils.utcnow(),
                'updated_at': literal_column('updated_at')})
...
def _instance_system_metadata_extension_get_item(context, instance_uuid, key,
                                       session=None):
    result = _instance_system_metadata_extension_get_query(
                            context, instance_uuid, session=session).\
                    filter_by(key=key).\
                    first()
...
@require_context
def instance_system_metadata_extension_update(context, instance_uuid,
    ...
    # Set existing metadata to deleted if delete argument is True
    if delete:
        original_metadata = instance_system_metadata_extension_get(
                context, instance_uuid, session=session)
        for meta_key, meta_value in original_metadata.iteritems():
            if meta_key not in metadata:
                meta_ref = _instance_system_metadata_extension_get_item(
                        context, instance_uuid, meta_key, session)
                meta_ref.update({'deleted': True})
                meta_ref.save(session=session)
    ...
    for meta_key, meta_value in metadata.iteritems():
        ...
        try:
            ...
            meta_ref = models.InstanceSystemMetadataExtension()
            item.update({"key": meta_key, "instance_uuid": instance_uuid})
...
@require_context
def _bandwidth_count_by_project(context, project_id, session=None):
    authorize_project_context(context, project_id)
    insts = instance_get_all_by_project(context, project_id)
    pub = 0
    pvt = 0
    for i in insts:
        metadata = instance_system_metadata_get(context,
                                                i['uuid'],
                                                session=None)
...
@require_context
def public_bandwidth_count_by_project(context, project_id, session=None):
    (pub, pvt) = _bandwidth_count_by_project(context, project_id, session=None)
    return pub


@require_context
def private_bandwidth_count_by_project(context, project_id, session=None):
    (pub, pvt) = _bandwidth_count_by_project(context, project_id, session=None)
    return pvt
