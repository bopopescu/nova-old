...
class AdminActionsController(wsgi.Controller):
    ...
    def _migrate(self, req, id, body):
        ...
        if body['migrate'] is None or 'keep_ephemeral' not in body['migrate']:
            keep_ephemeral = True
        else:
            keep_ephemeral = \
                    utils.bool_from_string(body['migrate']['keep_ephemeral'])
        force_host = None
        if body['migrate'] is not None and 'force_host' in body['migrate']:
            force_host = body['migrate']['force_host']
        try:
            instance = self.compute_api.get(context, id)
            self.compute_api.resize(req.environ['nova.context'],
                                    instance, None, keep_ephemeral, force_host)
        except exception.InstanceInvalidState as state_error:
            common.raise_http_conflict_for_instance_invalid_state(state_error,
                    'migrate')
        except Exception, e:
    ...
    @wsgi.action('os-relocate')
    def _relocate(self, req, id, body):
        """Permit admins to relocate a server."""
        context = req.environ["nova.context"]
        authorize(context, 'relocate')

        try:
            entity = body['os-relocate']
        except (KeyError, TypeError):
            raise exc.HTTPBadRequest(_("Malformed require body"))

        image_id = None

        try:
            image_id = entity['imageRef']
        except (KeyError, TypeError):
            image_id = None

        try:
            instance = self.compute_api.get(context, id)
            self.compute_api.schedule_relocate(context, instance, image_id)
        except exception.InstanceInvalidState as state_error:
            common.raise_http_conflict_for_instance_invalid_state(state_error,
                    'relocate')
        except exception.InstanceNotFound:
            ...
            msg = _("Cannot find image for relocate")
            raise exc.HTTPBadRequest(explanation=msg)
        except Exception:
            readable = traceback.format_exc()
            LOG.exception(_("Compute.api::relocate %s"), readable)
            raise exc.HTTPUnprocessableEntity()
        return webob.Response(status_int=202)
