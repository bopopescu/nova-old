...
class ActionDeserializer(CommonDeserializer):
    ...
    def _action_resize(self, node):
        ...
        if node.hasAttribute("keep_ephemeral"):
            resize['keep_ephemeral'] =\
                    utils.bool_from_string(node.getAttribute("keep_ephemeral"))
        else:
            resize['keep_ephemeral'] = True
...
class Controller(wsgi.Controller):
    ...
    def create(self, req, body):
        ...
        try:
            ...
            raise exc.HTTPBadRequest(explanation=msg)
        except exception.EcuKeyError as error:
            msg = _("Key 'ecus_per_vcpu:' may not be set.")
    ...
    def _resize(self, req, instance_id, flavor_id, keep_ephemeral, **kwargs):
    ...
    def _action_resize(self, req, id, body):
        ...
        if 'keep_ephemeral' in body['resize']:
            keep_ephemeral = \
                    utils.bool_from_string(body['resize']['keep_ephemeral'])
        else:
            keep_ephemeral = True
        return self._resize(req, id, flavor_ref, keep_ephemeral, **kwargs)
