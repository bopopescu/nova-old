...
metadata_opts = [
    cfg.StrOpt('config_drive_skip_versions',
               ...
               help=('List of metadata versions to skip placing into the '
                     'config drive')),
    cfg.BoolOpt('record_all_vms_heartbeat',
                default=True,
                help='Determine record all VMs heartbeat state or not.'),
    ]
...
class InstanceMetadata():
    ...
    def get_ec2_metadata(self, version):
        ...
        floating_ip = floating_ips or ''
        floating_ip_pub = []
        floating_ip_pvt = []
        if FLAGS.use_private_floating_ip:
            pvt_float_ip_range = netaddr.IPNetwork(
                                 FLAGS.private_floating_ip_range)
            for k in floating_ips:
                floating = netaddr.IPAddress(k)
                if floating in pvt_float_ip_range:
                    floating_ip_pvt.append(k)
                else:
                    floating_ip_pub.append(k)
        else:
            for i in floating_ips:
                floating_ip_pub.append(i)
        ...
        if self._check_version('2007-01-19', version):
            ...
            meta_data['public-ipv4-pvt'] = floating_ip_pvt
            meta_data['public-ipv4-pub'] = floating_ip_pub
    ...
    def get_openstack_item(self, path_tokens):
        ...
        metadata['user_id'] = self.instance['user_id']
...
def get_memcache_client():
    ...
    if MEMCACHE_CLIENT is not None:
        ...
        if FLAGS.memcached_servers:
            MEMCACHE_CLIENT = memcache.Client(FLAGS.memcached_servers, debug=0)
        return MEMCACHE_CLIENT
