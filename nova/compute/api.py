...
class API(base.Base):
    ...
    def _check_num_instances_quota(self, context, instance_type, rate_pub,
                                   min_count, max_count):
        ...
        req_local_gb = max_count * (instance_type['root_gb'] +
                                    instance_type['ephemeral_gb'])

        try:
            ecus_per_vcpu = int(instance_type['extra_specs']['ecus_per_vcpu:'])
        except KeyError:
            raise exception.EcuKeyError()
        req_ecus = req_cores * ecus_per_vcpu

        if FLAGS.use_network_qos:
            req_bw = max_count * rate_pub
        ...
        try:
            if FLAGS.use_network_qos:
                reservations = QUOTAS.reserve(context, instances=max_count,
                                              ...
                                              ecus=req_ecus,
                                              public_network_bandwidth=req_bw,
            ...
            one_instance_need_ecus = instance_type['vcpus'] * ecus_per_vcpu
            if one_instance_need_ecus:
                allowed = min(allowed,
                              headroom['ecus'] // one_instance_need_ecus)

            if FLAGS.use_network_qos:
                allowed = min(allowed,
                              headroom['public_network_bandwidth'] // rate_pub)

            root_gb = instance_type['root_gb'] or 0
            ephemeral_gb = instance_type['ephemeral_gb'] or 0
            local_gb = root_gb + ephemeral_gb
            if local_gb:
                allowed = min(allowed,
                              headroom['local_gb'] // local_gb)
            ...
            requested = dict(instances=min_count, cores=req_cores,
                             ram=req_ram, ecus=req_ecus, local_gb=req_local_gb)
            if FLAGS.use_network_qos:
                requested['public_network_bandwidth'] = req_bw
    ...
    def _create_instance(self, context, instance_type,
        ...
        # NOTE(stanzgy): prepare network public bandwidth infos
        if FLAGS.use_network_qos:
            qos_config = linux_net.tc_manager.get_network_qos_config()
            rate_pub_meta = metadata.get('network_qos_public_rate', None)

            if rate_pub_meta:
                rate_pub = int(rate_pub_meta)
            else:
                k = instance_type['vcpus']
                if qos_config['policy']['key'] == 'ecu':
                    try:
                        ecus_per_vcpu = int(
                                instance_type['extra_specs']['ecus_per_vcpu:'])
                    except KeyError:
                        raise exception.EcuKeyError()

                    k = k * ecus_per_vcpu

                pub_conf = (qos_config['shaping']['public'].get(str(k), None)
                            or qos_config['shaping']['public']['default'])
                rate_pub = pub_conf['rate']

            try:
                rate_pub = int(rate_pub)
            except ValueError:
        ...
        num_instances, quota_reservations = self._check_num_instances_quota(
                context, instance_type, rate_pub, min_count, max_count)
        ...
        # NOTE(hzyangtk): To store inject files into db for rebuild.
        #                 store into table instance_system_metadata_extension
        if FLAGS.allow_store_personality and injected_files != []:
            for instance_uuid in instance_uuids:
                try:
                    meta_item = {}
                    meta_item['personality'] = jsonutils.dumps(injected_files)
                    self.db.instance_system_metadata_extension_update(
                            context, instance_uuid, meta_item, delete=False)
                except Exception:
                    LOG.exception(_("Store personality into DB failed with "
    ...
    def _delete(self, context, instance):
        ...
        try:
            ...
            ecus = instance_type["vcpus"] * \
                   int(instance_type["extra_specs"]["ecus_per_vcpu:"])
    ...
    def _create_reservations(self, context, old_instance, new_instance,
                             project_id, ecus, instance_type, host):
        instance_vcpus = old_instance['vcpus']
        instance_memory_mb = old_instance['memory_mb']
        instance_local_gb = (old_instance['root_gb'] +
                             old_instance['ephemeral_gb'])
        ...
        if old_instance['task_state'] in (task_states.RESIZE_MIGRATED,
            ...
            if (migration_ref and
                ...
                try:
                    ...
                    ecus = old_inst_type['vcpus'] * \
                            int(old_inst_type['extra_specs']['ecus_per_vcpu:'])
                    LOG.debug(_("going to delete a resizing instance"))

        if  FLAGS.use_network_qos and not host:
            qos_config = linux_net.tc_manager.get_network_qos_config()
            ...
            pub_conf = (qos_config['shaping']['public'].get(str(k), None)
                            or qos_config['shaping']['public']['default'])
            pub_net_bw = pub_conf['rate']

            try:
                pub_net_bw = int(pub_net_bw)
            except ValueError:
                msg = _("'rate' must be int.")
                raise exception.InvalidTcParam(err=msg)
            ...
            reservations = QUOTAS.reserve(context,
                                      ...
                                      public_network_bandwidth=-pub_net_bw)
    ...
    @check_instance_state(vm_state=[vm_states.ACTIVE, vm_states.STOPPED],
                          task_state=[None])
    def schedule_relocate(self, context, instance, image=None):
        """ Send request to scheduler to schedule a best host to locate it"""

        # check if image is exists.
        if image is not None:
            self._get_image(context, image)
        ...
        self.scheduler_rpcapi.relocate(context, request_spec,
                                       filter_properties, instance, image)

    @wrap_check_policy
    @check_instance_lock
    @check_instance_state(vm_state=[vm_states.ACTIVE, vm_states.STOPPED],
                          task_state=[None])
    def relocate(self, context, instance, image, host, **kwargs):
        """ Ask new host to start relocate the instance.  """
        LOG.debug(_("Going to try to relocate instance"), instance=instance)
        self.compute_rpcapi.relocate_instance(context, instance, image, host)
    ...
    def confirm_resize(self, context, instance):
        ...
        LOG.debug(_("going to update migration(%d) status to confirming") %
                  migration_ref['id'], instance=instance)
        self.db.migration_update(context, migration_ref['id'],
                {'status': 'confirming'})
        ...
        LOG.debug(_("going to update migration(%d) status to confirmed") %
                  migration_ref['id'], instance=instance)
        self.db.migration_update(context, migration_ref['id'],
                {'status': 'confirmed'})
    ...
    def _resize_quota_delta(context, new_instance_type,
        ...
        def _get_ecus_per_vcpu(instance_type):
            try:
                ...
                db_inst_type = API().get_instance_type(context,
                                          instance_type["id"])
    ...
    def resize(self, context, instance, flavor_id=None,
               keep_ephemeral=True, force_host=None, **kwargs):
        ...
        if (not flavor_id and not FLAGS.allow_migrate_to_same_host):
            filter_properties['ignore_hosts'].append(instance['host'])
        if force_host:
    ...
    def _get_memcache_client(self):
        ...
        if FLAGS.memcached_servers:
            if MEMCACHE_CLIENT_API is None:
    ...
    def check_allow_attach(self, context, instance):
        """Check instance and it's host is allowed to attach volume or not."""
        instance_type = self.get_instance_type(context,
                                instance["instance_type_id"])
        inst_allow_attach = instance_type["extra_specs"].get("nbs")
        if not utils.bool_from_str(inst_allow_attach):
            return False
        ...
        elevated = context.elevated()
        metadata = self.db.aggregate_metadata_get_by_host(elevated, host)
        aggregate_vals = metadata.get("nbs")
        if not aggregate_vals:
            return False

        host_allow_attach = False
        for aggregate_val in aggregate_vals:
            if utils.bool_from_str(aggregate_val):
                host_allow_attach = True
                break

        return host_allow_attach
    ...
    def check_nbs_size(self, context, volume_id, expected_size):
        """Check nbs volume can be extended or not."""
        try:
            volume = self.nbs_api.get(context, volume_id)["volumes"][0]
        except (IndexError, KeyError, TypeError):
            LOG.error(_("Get wrong info from nbs server, volume %s")
                        % volume_id, context=context)
            raise exception.VolumeNotFound(volume_id=volume_id)
    ...
    def extend_nbs_volume(self, context, volume_id, size, instance=None):
        ...
        if instance is None:
            ...
            LOG.info(_("Call nbs to extend volume %s") % volume_id)
            self._call_nbs_to_extend(context, volume_id, size)
        else:
            LOG.info(_("Going to extend an attached volume %s") % volume_id)
    ...
    def update_nbs_qos(self, context, instance, volume_id, qos_info):
        ...
        """
        LOG.info(_("Going to update qos info of volume %s") % volume_id)
        self.compute_rpcapi.update_nbs_qos(context, instance=instance,
                                    volume_id=volume_id, qos_info=qos_info)
    ...
    def attach_nbs_volume(self, context, instance, volume_id):
        ...
        if len(db_vol) > 0:
            self.db.block_device_mapping_destroy_by_instance_and_device(
                        context, instance['uuid'], jsonutils.dumps(device))
    ...
    def describe_availability_zones(self, context, **kwargs):
        ...
        services = self.db.service_get_all(ctxt, None)
        available_zones = {}

        # default status is "not available", if any service of AZ is enable,
        # change status to "available"
        az_status = collections.defaultdict(lambda: "not available")

        for service in services:
            # I don't care other service.
            ...
            if not service.disabled:
                az_status[zone] = 'available'

            if zone in available_zones and show_hosts:
                hosts = available_zones[zone].get("hosts", set())
                hosts.add(host)
            elif show_hosts:
                hosts = set()
                hosts.add(host)
                available_zones.update({zone: {
                                              'zoneName': zone,
                                              'zoneState': az_status[zone],
                                              'hosts': hosts
                                              }})
            else:
                available_zones.update({zone: {
        ...
        for zone in available_zones:
            result.append(available_zones[zone])
        return {'availability_zones': result}
...
class KeypairAPI(base.Base):
    ...
    def get_all_key_pairs(self, context, search_opts=None):
        """List key pairs."""
        key_pairs = []
        if context.is_admin:
            if (search_opts and 'all_tenants' in search_opts):
                key_pairs = self.db.key_pair_get_all(context)
            else:
                key_pairs = self.db.key_pair_get_all_by_user(context,
                                                    context.user_id)
        rval = []
...
class SecurityGroupAPI(base.Base):
    ...
    @wrap_check_security_groups_policy
    def ensure_to_instance(self, context, instance, sec_groups):
        """Ensure these security groups associated with the instance"""
        LOG.debug(_('Going to ensure security groups: %(sec_groups)s'),
                  locals(), instance=instance)
        instance_uuid = instance['uuid']
        ...
        db_sec_groups = []
        # When query from database, if target security group not exists, will
        # raise NotFound exception.
        for group in sec_groups:
            security_group = self.db.security_group_get_by_name(context,
                                            context.project_id, group)
            db_sec_groups.append(security_group)

        # associate new security group to instance
        for new_sec_group in db_sec_groups:
            associated = self.is_associated_with_server(new_sec_group,
                                                        instance_uuid)
            if not associated:
                LOG.debug(_("Add security group %(security_group)s"), locals(),
                          instance=instance)
                self.db.instance_add_security_group(context.elevated(),
                                        instance_uuid, new_sec_group['id'])

        # Disassociate security group which not in request from instance.
        for old_sec in instance['security_groups']:
            group_name = old_sec['name']
            group_id = old_sec['id']
            if group_name not in sec_groups:
                LOG.debug(_("Remove security group %(group_name)s"),
                          locals(), instance=instance)
                self.db.instance_remove_security_group(context.elevated(),
        ...
        for sec_group in sec_groups:
            self.security_group_rpcapi.refresh_security_group_rules(context,
                    sec_group, host=instance['host'])

        # FIXME(gtt): Currently trigger_handler do nothing. But maybe do
        # some interesting in the futhuer. So add trigger_handler at that time.
        # self.trigger_handler('instance_set_security_group',
        #        context, instance, security_group_name)
