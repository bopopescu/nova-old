...
class ComputeAPI(nova.openstack.common.rpc.proxy.RpcProxy):
    ...
    def update_nbs_qos(self, ctxt, instance, volume_id, qos_info):
        instance_p = jsonutils.to_primitive(instance)
        self.cast(ctxt, self.make_msg('update_nbs_qos',
                instance=instance_p, volume_id=volume_id, qos_info=qos_info),
                topic=_compute_topic(self.topic, ctxt, None, instance))
    ...
    def finish_resize(self, ctxt, instance, migration_id, image, disk_info,
            host, reservations=None, keep_ephemeral=True):
        instance_p = jsonutils.to_primitive(instance)
    ...
    def resize_instance(self, ctxt, instance, migration_id, image,
        ...
        self.cast(ctxt, self.make_msg('resize_instance',
                instance=instance_p, migration_id=migration_id,
                image=image, reservations=reservations,
                keep_ephemeral=keep_ephemeral), topic)
    ...
    def relocate_instance(self, ctxt, instance, image, host,
                          reservations=None):
        topic = _compute_topic(self.topic, ctxt, host, None)
        instance_p = jsonutils.to_primitive(instance)
        self.cast(ctxt, self.make_msg('relocate_instance',
                                      instance=instance_p, image=image,
                                      reservations=reservations), topic)

    def pre_relocate_instance(self, ctxt, instance, reservations=None):
        topic = _compute_topic(self.topic, ctxt, None, instance)
        instance_p = jsonutils.to_primitive(instance)
        return self.call(ctxt, self.make_msg('pre_relocate_instance',
                    instance=instance_p, reservations=reservations), topic)
