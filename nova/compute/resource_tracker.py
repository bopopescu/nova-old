...
class Claim(object):
    ...
    def __str__(self):
        msg = "[Claim %s: %d MB memory, %d GB disk, %d VCPUS" % \
                (self.claim_id, self.memory_mb, self.disk_gb, self.vcpus)
        if FLAGS.enable_cpuqos:
            msg += ", %d ECUS" % self.instance['ecus_used']
        if FLAGS.use_network_qos:
            msg += ", network bandwith(%d private, %d public)" % \
                    (self.instance['private_network_bandwith'],
                     self.instance['public_network_bandwith'])
        return msg + "]"
...
class ResourceTracker(object):
    ...
    def _sync_instance_network_bandwith(self, instance, inst_type):
        """update instance network bandwith info"""
        qos_info = get_instance_network_qos(inst_type, instance['uuid'])

        private_network_bandwith = qos_info['private_qos']
        public_network_bandwith = qos_info['public_qos']

        instance['private_network_bandwith'] = private_network_bandwith
        instance['public_network_bandwith'] = public_network_bandwith
    ...
    def _update_usage_from_instance(self, resources, instance):
        ...
        cpu_info_decoded = False
        if FLAGS.enable_cpuqos:
            try:
                resources['cpu_info'] = jsonutils.loads(resources['cpu_info'])
                cpu_info_decoded = True
            except (ValueError, TypeError, KeyError):
                LOG.warn(_("Cpu info json decode error, %s") % resources)
                pass

        if FLAGS.enable_cpuqos or FLAGS.use_network_qos:
            inst_type_id = instance['instance_type_id']
            inst_type = instance_types.get_instance_type(inst_type_id)
            if FLAGS.enable_cpuqos:
                ecus_used = (inst_type['vcpus'] *
                        int(inst_type['extra_specs']['ecus_per_vcpu:']))
                instance['ecus_used'] = ecus_used
            if FLAGS.use_network_qos:
                self._sync_instance_network_bandwith(instance, inst_type)
        ...
        if is_new_instance or is_deleted_instance:
            ...
            if FLAGS.enable_cpuqos and cpu_info_decoded:
                resources['cpu_info']['ecus_used'] += (sign *
                                    instance['ecus_used'])
            if FLAGS.use_network_qos:
                resources['private_network_mbps_used'] += (sign *
                                    instance['private_network_bandwith'])
                resources['public_network_mbps_used'] += (sign *
                                    instance['public_network_bandwith'])
        ...
        if FLAGS.enable_cpuqos and cpu_info_decoded:
            resources['cpu_info'] = jsonutils.dumps(resources['cpu_info'])
    ...
    def _update_usage_from_instances(self, resources, instances):
        ...
        if FLAGS.use_network_qos:
            resources['private_network_mbps_used'] = \
                    FLAGS.reserved_host_network_private_bandwidth
            resources['public_network_mbps_used'] = \
                    FLAGS.reserved_host_network_public_bandwidth
            resources['total_private_network_mbps'] = \
                    FLAGS.network_qos_host_private_bandwidth * \
                    FLAGS.network_qos_private_allocation_ratio
            resources['total_public_network_mbps'] = \
                    FLAGS.network_qos_host_public_bandwidth * \
                    FLAGS.network_qos_public_allocation_ratio

        if FLAGS.enable_cpuqos:
            try:
                cpu_info = jsonutils.loads(resources['cpu_info'])
                ecu_ratio = cpu_info['ecu_ratio']
                cpu_info['ecus'] = ecu_ratio * resources['vcpus']
                cpu_info['ecus_used'] = 0
                resources['cpu_info'] = jsonutils.dumps(cpu_info)
            except (ValueError, TypeError, KeyError):
                LOG.warn(_("Cpu info json decode error, %s") % resources)
                pass
