...
def _get_instances_storage_info():
    """
    get instances storage disk info including disk path and disk number

    :returns: list of storage disk just like: [{'disk': '/dev/sda',
                                                'number': '254:0'}]
    """
    instances_path = FLAGS.instances_path
    result = []

    def _get_instances_storage_disk(instances_path):
        """
        get instance storage disk path

        :param instances_path: FLAGS.instances_path
        :returns: instance storage disk path just like: '/dev/sda' or
                  '/dev/dm-0' for logical volume device or
                  None if something error
        """
        try:
            out, _err = utils.execute('df', '-P', '-B', '1', instances_path,
                                      run_as_root=True)
            out = out.splitlines()[1]
            disk = out.split()[0]
            if os.path.islink(disk):
                disk, _err = utils.execute('readlink', '-nm', disk,
                                       run_as_root=True)
        except Exception, e:
            return None
    ...
    def _read_disknumber_from_sys_dev(path):
        """
        read disk number from sys filesystem

        :param path: path just like '/sys/block/sda/dev'
        """
        disknumber = None
        ...
        try:
            with open(path) as fp:
                text = fp.readline()
                disknumber = text.split('\n')[0]
        except Exception, e:
            pass

        return disknumber

    disk = _get_instances_storage_disk(instances_path)

    if disk is None:
        return []

    elif disk.startswith("/dev/dm-"):
        dname = os.path.basename(disk)
        path = os.path.join("/sys/block/", dname, "subordinates")
        for d in os.listdir(path):
            dig_re = re.compile(r'(\d)')
            disk = dig_re.split(d)[0]
            sys_dev = os.path.join("/sys/block/", disk, "dev")
            disk_number = _read_disknumber_from_sys_dev(sys_dev)
            if disk_number:
                disk_info = {'disk': os.path.join("/dev", disk),
                             'number': disk_number}
                result.append(disk_info)

    else:
        dig_re = re.compile(r'(\d)')
        disk = dig_re.split(disk)[0]
        dname = os.path.basename(disk)
        sys_dev = os.path.join("/sys/block/", dname, "dev")
        disk_number = _read_disknumber_from_sys_dev(sys_dev)
        if disk_number:
            disk_info = {'disk': os.path.join("/dev", dname),
                         'number': disk_number}
            result.append(disk_info)
...
compute_opts = [
    ...
    cfg.StrOpt('rsync_config_file',
               default='/etc/rsyncd.conf',
               help="Rsync config file path, using to start rsync daemon"),
    cfg.ListOpt('kernel_modules_for_compute',
               default=[],
               help="Kernel modules which need to be loaded when "
                    "nova-compute service starts"),
    ]
...
ioqos_opts = [
    cfg.ListOpt('instance_storage_devices',
               default=_get_instances_storage_info(),
               help='disk number of disks that instances path mounted on'),
    cfg.BoolOpt('use_instance_storage_qos',
                default=False,
                help='use instance storage qos if True'),
    cfg.IntOpt('host_blkio_read_bps',
               default=0,
               help='host blkio read bps'),
    cfg.IntOpt('host_blkio_write_bps',
               default=0,
               help='host blkio write bps'),
    cfg.IntOpt('host_blkio_read_iops',
               default=0,
               help='host blkio read iops'),
    cfg.IntOpt('host_blkio_write_iops',
               default=0,
               help='host blkio write iops'),
    ]

FLAGS.register_opts(ioqos_opts)
...
class ComputeManager(manager.SchedulerDependentManager):
    ...
    def _ensure_rsync_daemon(self):
        def _check_and_remove_pidfile():
            lines = []
            pidfile = ''
            with open(FLAGS.rsync_config_file) as f:
                lines = f.readlines()
            for line in lines:
                if 'pidfile' in line.replace(' ', ''):
                    pidfile = line.split()[-1].split('=')[-1]
                    break
            utils.delete_if_exists(pidfile)

        LOG.info(_("Checking rsync daemon status"))
        try:
            out, err = utils.execute('pgrep', 'rsync')
        except exception.ProcessExecutionError as ex:
            if ex.exit_code == 1:
                LOG.warn(_("Rsync daemon stopped, restart now"))
                _check_and_remove_pidfile()
                config_file = '--config=' + FLAGS.rsync_config_file
                utils.execute('rsync', '--daemon', config_file)
            else:
                raise ex

    def _set_host_blkio(self):
        """
        set host blkio for instance storage ioqos
        """
        if FLAGS.use_instance_storage_qos:
            weight_value = 1000
            read_iops_value = FLAGS.host_blkio_read_iops
            write_iops_value = FLAGS.host_blkio_write_iops
            read_bps_value = FLAGS.host_blkio_read_bps
            write_bps_value = FLAGS.host_blkio_write_bps
        else:
            weight_value = 0
            read_iops_value = 0
            write_iops_value = 0
        ...
        if FLAGS.instance_storage_devices:
            try:
                blkio = os.path.join(FLAGS.blkio_cgroup_path, 'blkio.')
                weight_device = '%s%s' % (blkio, 'weight_device')
                read_iops = '%s%s' % (blkio, 'throttle.read_iops_device')
                write_iops = '%s%s' % (blkio, 'throttle.write_iops_device')
                read_bps = '%s%s' % (blkio, 'throttle.read_bps_device')
                write_bps = '%s%s' % (blkio, 'throttle.write_bps_device')
                for dev in FLAGS.instance_storage_devices:
                    dev = dev['number']
                    weight_device_info = "%s %s\n" % (dev, weight_value)
                    read_iops_info = "%s %s\n" % (dev, read_iops_value)
                    write_iops_info = "%s %s\n" % (dev, write_iops_value)
                    read_bps_info = "%s %s\n" % (dev, read_bps_value)
                    write_bps_info = "%s %s\n" % (dev, write_bps_value)
                    utils.set_cgroup_parameter(weight_device,
                                               weight_device_info)
                    utils.set_cgroup_parameter(read_iops, read_iops_info)
                    utils.set_cgroup_parameter(write_iops, write_iops_info)
                    utils.set_cgroup_parameter(read_bps, read_bps_info)
                    utils.set_cgroup_parameter(write_bps, write_bps_info)
            except Exception, ex:
                LOG.warning(_('set blkio for host failed, exception msg: %s')
                            % str(ex))
        else:
    ...
    def _check_and_wait_nbs_vol(self, context, instance_uuid, wait_ticks):
        ...
        for bdm in bdms:
            try:
                host_dev = jsonutils.loads(bdm['connection_info'])['host_dev']
            except (TypeError, ValueError, KeyError):
                LOG.warn(_("host dev info of bdm %s is missing") %
                            bdm['volume_id'])
                continue
        ...
        while len(disappeared_host_devs) > 0 and wait_ticks > 0:
            LOG.info(_("Wait for nbs volume %(host_dev)s 3 seconds, "
                       "ticks: %(wait_ticks)d") % {"host_dev": host_dev,
        ...
        if len(disappeared_host_devs) > 0:
            LOG.warning(_("Nbs volume disappeared %(disappeared)s which is "
                          "used by %(instance)s")
                          % {"disappeared": disappeared_host_devs,
                             "instance": instance_uuid})

        return wait_ticks

    def modprobe_modules_for_compute(self):
        """
        Modprobe modules which are used by nova-compute service
        """
        for mod in FLAGS.kernel_modules_for_compute:
            utils.modprobe_module(mod.split())

    def init_host(self):
        """Initialization for a standalone compute service."""
        # modprobe kernel modules for nova-compute
        self.modprobe_modules_for_compute()
        ...
        try:
            for count, instance in enumerate(instances):
                ...
                if FLAGS.use_network_qos:
                    tc = linux_net.tc_manager
                    tc._register(instance['id'])
        ...
        if FLAGS.ebs_backend == 'nbs' and is_nbs:
            for bdm in bdms:
                try:
                    ...
                    LOG.warn(_("bdm info of instance %s is missing")
                                % instance_uuid)
                    continue
    ...
    def _shutdown_instance(self, context, instance):
        ...
        self._deallocate_network(context, instance)
        LOG.debug(_("deallocate network end."), context=context,
                  instance=instance)

        # NOTE(vish) get bdms before destroying the instance
        # FIXME(wangpan): we currently don't deal with nbs volumes when
        #                 deleting an instance with nbs volumes attached.
        #                 User will detach them, but it's better we do that.
        bdms = self._get_instance_volume_bdms(context, instance['uuid'], True)
        block_device_info = self._get_instance_volume_block_device_info(
            context, instance['uuid'])
        LOG.debug(_("begin to call libvirt driver to destroy instance."),
                  context=context, instance=instance)
        ...
        if FLAGS.ebs_backend == 'nbs':
            ...
            for bdm in bdms:
                self.nbs_api.detach(context, bdm['volume_id'], host_ip)
                self.db.block_device_mapping_destroy_by_instance_and_volume(
                                context, instance['uuid'], bdm['volume_id'])
            self.nbs_api.wait_for_detach(context, bdms)
        else:
            for bdm in bdms:
                try:
                    # NOTE(vish): actual driver detach done in driver.destroy,
                    #       so just tell nova-volume that we are done with it.
                    volume = self.volume_api.get(context, bdm['volume_id'])
    ...
    def _cleanup_volumes(self, context, instance_uuid):
        # FIXME(wangpan): we currently don't deal with nbs volumes deleting,
        #                 User will call nbs api to delete them.
        if FLAGS.ebs_backend == 'nbs':
            return
    ...
    def snapshot_instance(self, context, image_id, instance,
        ...
        # Note(hzrandd): when snapshot error,make sure
        # task_state update to None.
        try:
    ...
    def finish_revert_resize(self, context, migration_id, instance,
        ...
        with self._error_out_instance_on_exception(context, instance['uuid'],
            ...
            if FLAGS.use_network_qos:
                # get network qos info in system metadata before delete
                ...
                # NOTE(stanzgy): tear down instance network qos on dest host
                LOG.info(_("tear down network qos for instance %(instance_id)s"
                           " on source host %(instance_host)s") % locals())
                self.network_api.delete_instance_network_qos(context,
                                                instance['id'],
            ...
            # NOTE(stanzgy): setup instance network qos on source host
            if FLAGS.use_network_qos:
                instance_id = instance['id']
                instance_host = instance['host']

                # NOTE(stanzgy): setup instance network qos on dest host
                LOG.info(_("setup network qos for instance %(instance_id)s "
                           "on dest host %(instance_host)s") % locals())
                ...
                for i in qos_info:
                    if i['type'] == 'public':
                        ...
                        # NOTE(stanzgy): private network qos should be defined
                        #                by config file
                        spec_pvt = None

                self.network_api.create_instance_network_qos(context,
                                                             instance['id'],
                                                             spec_pub,
                                                             spec_pvt)
    ...
    def prep_resize(self, context, image, instance, instance_type,
                    reservations=None, keep_ephemeral=True):
        ...
        with self._error_out_instance_on_exception(context, instance['uuid'],
            ...
            # Ensure rsync daemon is running on dest host
            if not same_host and FLAGS.use_rsync:
                self._ensure_rsync_daemon()
            ...
            self.compute_rpcapi.resize_instance(context, instance,
                    migration_ref['id'], image, reservations, keep_ephemeral)
    ...
    @reverts_task_state
    @wrap_instance_fault
    def resize_instance(self, context, instance, migration_id,
                        image, reservations=None, keep_ephemeral=True):
        ...
        with self._error_out_instance_on_exception(context, instance['uuid'],
            ...
            disk_info = self.driver.migrate_disk_and_power_off(
                    ...
                    block_device_info, keep_ephemeral)
    ...
    def _finish_resize(self, context, instance, migration_ref, disk_info,
        ...
        if FLAGS.use_network_qos:
            ...
            # NOTE(stanzgy): tear down instance network qos on source host
            LOG.info(_("tear down network qos for instance %(instance_id)s "
                       "on source host %(instance_host)s") % locals())
            self.network_api.delete_instance_network_qos(context,
                                               instance['id'],
        ...
        if old_instance_type_id != new_instance_type_id:
            ...
            #if the disk size of new instance type were larger than the old,
            #just keep the disk size of the old instance type here
            #root_gb = max(instance['root_gb'], instance_type['root_gb'])
            ephemeral_gb = 0
            if keep_ephemeral:
                ephemeral_gb = max(instance['ephemeral_gb'],
        ...
        # NOTE(stanzgy): ensure instance tenant ipset on new host
        if FLAGS.intranet_firewall_mode == 'tenant':
            topic = rpc.queue_get_for(context,
                                      FLAGS.network_topic,
                                      instance['host'])
            fixed_ip_ref = self.db.fixed_ip_get_by_instance(context,
                                                            instance['uuid'])
            args = {}
            args['address'] = fixed_ip_ref[0]['address']

            LOG.debug(_("ensure ipset for instance %(instance_uuid)s on host "
                        "%(host)s") % {'instance_uuid': instance['uuid'],
                                       'host': instance['host']})
            rpc.call(context, topic,
                     {'method': 'add_fixed_ip_to_ipset',
                      'args': args})
    ...
    def finish_resize(self, context, migration_id, disk_info, image,
                      instance, reservations=None, keep_ephemeral=True):
        ...
        try:
            self._finish_resize(context, instance, migration_ref,
                                disk_info, image, keep_ephemeral)
            self._quota_commit(context, reservations)
    ...
    @reverts_task_state
    @wrap_instance_fault
    def relocate_instance(self, context, instance, image=None,
                          reservations=None):
        """ Relocate instance in this host. """
        with self._error_out_instance_on_exception(context, instance['uuid'],
                                                   reservations):

            # Note(gtt): mark instance to relocate_prep, even if the real
            # rpc call failed.
            self._instance_update(context, instance['uuid'],
                                  task_state=task_states.RELOCATE_PREP,
                                  expected_task_state=None)
            # rpc call, if timeout continue
            try:
                self.compute_rpcapi.pre_relocate_instance(context, instance)
            except rpc_common.Timeout:
                LOG.warn("rpc called pre_relocate_instance timeout, continue.",
                         context=context, instance=instance)

            self._finish_relocate_instance(context, instance, image)

    @exception.wrap_exception(notifier=notifier, publisher_id=publisher_id())
    @reverts_task_state
    @wrap_instance_fault
    def pre_relocate_instance(self, context, instance, reservations=None):
        """Shutdown the instance in this host.
            The method is called by new host.
        """
        with self._error_out_instance_on_exception(context, instance['uuid'],
                                                   reservations):
            ...
            self._notify_about_instance_usage(
                context, instance, "pre_relocate.start",
                network_info=network_info)
            ...
            self.network_api.setup_networks_on_host(
                context, instance, self.host, teardown=True)
    ...
    @reverts_task_state
    @wrap_instance_fault
    def _finish_relocate_instance(self, context, instance, image=None,
                                  reservations=None):
        with self._error_out_instance_on_exception(context, instance['uuid'],
                                                   reservations):
            ...
            if FLAGS.use_network_qos:
                ...
                # NOTE(stanzgy): tear down instance network qos on source host
                LOG.info(_("tear down network qos for instance "
                           "%(instance_id)s on source host %(instance_host)s")
                                                        % locals())
                self.network_api.delete_instance_network_qos(context,
                                                             instance['id'],
                                                             instance_host,
            ...
            self._notify_about_instance_usage(
                context, instance, "relocate.start", network_info=network_info)

            self._instance_update(context, instance['uuid'],
                                task_state=task_states.RELOCATING,
                                expected_task_state=task_states.RELOCATE_PREP,
                                host=self.host)
            ...
            #Note(gtt): If specified new image, use it.
            #           Otherwise use old image.
            if image:
                image_meta = _get_image_meta(context, image)
                instance = self._instance_update(context, instance['uuid'],
                                      image_ref=image)
            else:
                image_meta = self._check_image_size(context, instance)
            ...
            self._instance_update(context, instance['uuid'],
                                task_state=None,
                                expected_task_state=task_states.RELOCATING)

            self._notify_about_instance_usage(
                context, instance, "relocate.end", network_info=network_info)
        ...
        def do_reserve(used_dev):
            ...
            for bdm in bdms:
                try:
                    ...
                    device = None
                if not device:
                    LOG.warn(_("bdm %s device name not found")
                                % bdm['volume_id'],
                                context=context, instance=instance)
                    continue
                slot = device.get('slot')
                if slot:
                    used_dev['slot'].append(slot)
                dev = device.get('mountpoint', '').rpartition("/")[2]
                if dev:
                    used_dev['dev'].append(dev)
        ...
        try:
            ...
            if device is None:
                ...
                try:
                    ...
                    LOG.error(_("device of volume %s not found") % volume_id,
                                context=context, instance=instance)
                    raise e

            # get the original volume size, used for nbs rollback
            try:
                vol = self.nbs_api.get(context, volume_id)["volumes"][0]
                old_size = int(vol["size"])
            except (IndexError, KeyError, TypeError, ValueError):
                LOG.error(_("Get original size of volume %s error") %
                            volume_id, context=context, instance=instance)
            ...
            try:
                self.driver.extend_nbs_volume(instance['name'], device, size)
            except Exception as e:
                LOG.warn(_("Extend nbs volume %s failed") % volume_id,
                            context=context, instance=instance)
                raise e
            else:
                # update QoS params for nbs volume after extending
                (host_dev, qos_info) = self.nbs_api.get_host_dev_and_qos_info(
                                                context, volume_id, host_ip)

                self.update_nbs_qos(context, instance, volume_id, qos_info)
    ...
    @reverts_task_state
    @wrap_instance_fault
    def update_nbs_qos(self, context, instance, volume_id, qos_info):
        """
        Update qos info of nbs volume which has been attached on an instance.
        """
        LOG.audit(_("Updating qos info of nbs volume %(volume_id)s to "
                    "%(qos_info)s") % {"volume_id": volume_id,
                                       "qos_info": qos_info},
                    context=context, instance=instance)
        # Get attached device from nova db
        bdm = self._get_instance_volume_bdm(context, instance['uuid'],
        ...
        try:
            ...
            LOG.error(_("Volume info of %s is missing") % volume_id,
                        context=context, instance=instance)
            raise e

        try:
            self.driver.update_nbs_qos(instance['name'], instance['uuid'],
                                       host_dev, qos_info)
        except Exception as e:
            LOG.warn(_("Update qos info of nbs volume %s failed") % volume_id,
                        context=context, instance=instance)
            raise e
        else:
            try:
                connection_info = jsonutils.loads(bdm['connection_info'])
                connection_info['qos_info'].update(qos_info)
                values = {'instance_uuid': instance['uuid'],
                ...
                LOG.warn(_("connection info is missing of volume %s")
                            % volume_id, context=context, instance=instance)
            self.nbs_api.notify_nbs_qos_updated(context, volume_id, qos_info)
    ...
    def _attach_nbs_volume(self, context, volume_id, device, instance):
        ...
        try:
            ...
            with excutils.save_and_reraise_exception():
                ...
                self.nbs_api.notify_nbs_libvirt_result(context, volume_id,
                            'attach', False, device=device['real_path'],
                            host_ip=host_ip, instance_uuid=instance['uuid'])
        ...
        def get_new_device(used_dev):
            ...
            """
            # FIXME(wangpan): we may need to replace this method with
            #                 get_device_for_nbs_volume, but currently the DB
            #                 operations of these two method is different.
            bdms = self.db.block_device_mapping_get_all_by_instance(context,
                                                            instance['uuid'])
            if used_dev is None:
                used_dev = {'slot': [], 'dev': []}
        ...
        try:
            ...
            while retry:
                retry, used = self.driver.attach_nbs_volume(instance['name'],
                       instance['uuid'], device, host_dev, qos_info, volume_id)
                if retry:
                    if used.get('dev'):
                    ...
                    LOG.info(_("Sleep 3s and retry to attach nbs volume "
                               " %(volume)s at %(device)s")
                               % {"volume": volume_id, "device": device},
                               context=context, instance=instance)
                    time.sleep(3)
        except Exception:  # pylint: disable=W0702
            with excutils.save_and_reraise_exception():
    ...
    def _detach_nbs_volume(self, context, instance, bdm):
        ...
        try:
            ...
            LOG.error(_("device of volume %s not found") % bdm['volume_id'],
                        context=context, instance=instance)
            raise exception.InvalidVolume(reason="device not found")
        ...
        try:
            self.driver.detach_nbs_volume(instance['name'], instance['uuid'],
                                          mp, host_dev)
            ...
            time.sleep(FLAGS.attach_detach_interval)
    ...
    def detach_nbs_volume(self, context, volume_id, instance):
        ...
        if bdm is None:
            ...
            if bdm["updated_at"] and not timeutils.is_older_than(
                            bdm["updated_at"], FLAGS.attach_detach_interval):
                LOG.debug(_("sleep %(wait)s to avoid attaching detaching too "
                            "frequently for volume %(vol)s") %
                            {"wait": FLAGS.attach_detach_interval,
                             "vol": volume_id},
                            context=context, instance=instance)
                time.sleep(FLAGS.attach_detach_interval)
    ...
    @manager.periodic_task
    def _cleanup_instance_iptables(self, context):
        # NOTE(stanzgy): clean up iptables. search for those instances whose
        #                iptables rules should be removed whereas not in fact.
        instances_ipt = self.driver.firewall_driver.instances.keys()
        instances_db = [i['id'] for i in self.db.instance_get_all_by_host(
                                                                    context,
                                                                    self.host)]

        bifh = list(set(instances_ipt).difference(set(instances_db)))
        if bifh:
            LOG.warn(_("found instances whose iptables rules should be removed"
                       " whereas not indeed: %(bifh)s"), locals())
            for i in bifh:
                instance = {}
                instance['id'] = i
                self.driver.firewall_driver.remove_filters_for_instance(
                                                                    instance)
    ...
    @exception.wrap_exception(notifier=notifier, publisher_id=publisher_id())
    def add_aggregate_host(self, context, aggregate_id, host, subordinate_info=None):
