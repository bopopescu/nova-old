...
network_opts = [
    ...
    cfg.ListOpt('private_floating_pools',
                default=[],
                help='private floating ip pools for quotas check'),
    ...
    cfg.BoolOpt('use_dhcp_by_location',
               default=False,
               help='Use DHCP to give out IP address according to'
                    'physical location'),
    cfg.StrOpt('fixed_range_by_location',
               default='10.0.1.0/24',
               help='Local virtual machine Fixed IP pool, used when'
                    'use_dhcp_by_location is True'),
    cfg.StrOpt('fixed_range_by_location_start',
               default='10.0.1.10',
               help='Local virtual machine dhcp start Fixed IP'),
    cfg.ListOpt('kernel_modules_for_network',
               default=[],
               help='Kernel modules which need to be loaded when '
                    'nova-network service starts'),
    cfg.BoolOpt('use_vhost_net',
                default=False,
                help='If True, modprobe vhost_net module when '
                     'nova-network service starts'),
   ]
...
class RPCAllocateFixedIP(object):
    ...
    def deallocate_fixed_ip(self, context, address, host=None, teardown=True):
        ...
        # Note(gtt): Origin nova network has rpcapi module, but currently we
        # don't have. So uncomment this if we got the rpcapi.
...
def get_vcpu_num(instance):
    inst_type_id = instance['instance_type_id']
    inst_type = instance_types.get_instance_type(inst_type_id)
    return inst_type["vcpus"]


def get_ecu_num(instance):
    inst_type_id = instance['instance_type_id']
    inst_type = instance_types.get_instance_type(inst_type_id)
    for (k, v) in inst_type["extra_specs"].iteritems():
        if k.encode() == "ecus_per_vcpu:":
            return inst_type["vcpus"] * int(v)
    return None
...
class FloatingIP(object):
    ...
    def allocate_for_instance(self, context, **kwargs):
        ...
        """
        LOG.debug(_("enter into rpc_call method allocate_for_instance"))
        ...
        if FLAGS.auto_assign_floating_ip:
            ...
            LOG.debug(_("floating IP auto allocation for instance, "
                        "floating ip: |%(floating_address)s|, "
                        "fixed ip: |%(fixed_address)s|,"),
        ...
        LOG.debug(_("leave rpc_call method allocate_for_instance"))
        return nw_info
    ...
    def deallocate_for_instance(self, context, **kwargs):
        ...
        rpc.called by network_api
        """
        LOG.debug(_("enter into rpc_call method deallocate_for_instance"))
        instance_id = kwargs.get('instance_id')
        ...
        LOG.debug(_("leave rpc_call method deallocate_for_instance"))
    ...
    def allocate_floating_ip(self, context, project_id, auto_assigned=False,
                             pool=None, assign_address=None):
        """Gets a floating ip from the pool."""
        # NOTE(tr3buchet): all network hosts in zone now use the same pool
        pool = pool or FLAGS.default_floating_pool
        assign_address = assign_address or None
        use_quota = not auto_assigned

        # Check the quota; can't put this in the API because we get
        # called into from other places
        try:
            if use_quota and FLAGS.use_private_floating_ip and\
                            pool in FLAGS.private_floating_pools:
                reservations = QUOTAS.reserve(context, private_floating_ips=1)
            elif use_quota:
    ...
    def deallocate_floating_ip(self, context, address,
        ...
        try:
            if use_quota and FLAGS.use_private_floating_ip and\
                        floating_ip['pool'] in FLAGS.private_floating_pools:
                reservations = QUOTAS.reserve(context, private_floating_ips=-1)
            elif use_quota:
                reservations = QUOTAS.reserve(context, floating_ips=-1)
            else:
    ...
    def associate_floating_ip(self, context, floating_address, fixed_address,
                              affect_auto_assigned=False, spec=None):
        # NOTE(stanzgy): add wrapper for associate_floating_ip() here and
        #                rename original associate_floating_ip() to
        #                do_associate_floating_ip(), thus we can specify
        #                the rpc host in this wrapper. FMI: CLOUD-5744
        fixed_ip = self.db.fixed_ip_get_by_address(context, fixed_address)

        if not fixed_ip.get('instance'):
            raise exception.FixedIpNotAssociatedWithInstance(
        ...
        if host == self.host:
            self.do_associate_floating_ip(context,
                                          floating_address,
                                          fixed_address,
            ...
            rpc.call(context,
                     rpc.queue_get_for(context, FLAGS.network_topic, host),
                     {'method': 'do_associate_floating_ip',
                      'args': {'floating_address': floating_address,
                               'fixed_address': fixed_address,
                               'affect_auto_assigned': affect_auto_assigned,
                               'spec': spec}})

    def do_associate_floating_ip(self, context, floating_address,
        ...
        if floating_ip['fixed_ip_id']:
            ...
            except exception.FloatingIpNotAssociated as e:
                LOG.warn(_("Cannot auto disassociate floating ip "
                           "%(floating_address)s before associating.")
        ...
        if host == self.host:
            ...
            self._associate_floating_ip(context, floating_address,
                            fixed_address, interface, fixed_ip, instance)
        else:
    ...
    def _associate_floating_ip(self, context, floating_address, fixed_address,
                               interface, fix_ip, instance, spec=None):
        ...
        try:
            ...
            # setup floating ip network qos
            if FLAGS.use_network_qos:
                LOG.debug(_("adding network qos for floating ip "
                            "%(floating_address)s, fixed ip "
                            "%(fixed_address)s.") % locals())
                fixed_ip = fix_ip
                instance_ref = instance

                if FLAGS.use_private_floating_ip:
                    floating_ip = netaddr.IPAddress(floating_address)
                    r = netaddr.IPNetwork(FLAGS.private_floating_ip_range)
                    if floating_ip in r:
                        qtype = 'private'
                    else:
                        qtype = 'public'
                ...
                self.create_address_network_qos(context,
                                                instance_ref,
                                                qtype,
                                                'floating_ip',
                                                floating_address)
    ...
    def disassociate_floating_ip(self, context, address,
                                 affect_auto_assigned=False):
        # NOTE(stanzgy): add wrapper for disassociate_floating_ip() here and
        #                rename original disassociate_floating_ip() to
        #                do_dissassociate_floating_ip(), thus we can specify
        #                the rpc host in this wrapper. FMI: CLOUD-5744
        floating_ip = self.db.floating_ip_get_by_address(context, address)

        if (not floating_ip.get('fixed_ip') or
            not floating_ip['fixed_ip'].get('instance')):
            floating_address = floating_ip['address']
            raise exception.FloatingIpNotAssociated(address=floating_address)

        instance = floating_ip['fixed_ip']['instance']
        host = instance['host']

        if host == self.host:
            self.do_disassociate_floating_ip(context, address,
                                             affect_auto_assigned)
        else:
            rpc.call(context,
                     rpc.queue_get_for(context, FLAGS.network_topic, host),
                     {'method': 'do_disassociate_floating_ip',
                      'args': {'address': address,
                               'affect_auto_assigned': affect_auto_assigned}})

    def do_disassociate_floating_ip(self, context, address,
                                    affect_auto_assigned=False):
    ...
    def _disassociate_floating_ip(self, context, address, interface,
        ...
        # tear down network qos
        if FLAGS.use_network_qos:
            LOG.debug(_("deleting network qos for floating ip %s.") %
                                                            address)
            ...
            # pylint: disable=E1101
            self.delete_address_network_qos(context,
                                            instance_ref,
                                            qtype,
                                            address)
    ...
    @wrap_check_policy
    def get_floating_ips(self, context, search_opts=None):
        """Returns the floating IPs"""
        if context.is_admin and (search_opts and 'all_tenants' in search_opts
                            and search_opts['all_tenants']):
            ips = self.db.floating_ip_get_allocated(context)
        else:
...
class NetworkManager(manager.SchedulerDependentManager):
    ...
    def init_host(self):
        ...
        if FLAGS.use_network_qos:
            self.driver.tc_manager._init_network_qos_device()

            instances = self.db.instance_get_all_by_host(ctxt, self.host)

            for instance in instances:
                metadata = self.db.instance_system_metadata_get(
                                                ctxt,
                                                instance['uuid'])

                qos_conf = metadata.get('network-qos', None)
                spec_pub = None
                spec_pvt = None

                if qos_conf:
                    qos_info = json.loads(qos_conf)
                    for c in qos_info:
                        if c['type'] == 'public':
                            spec_pub = c
                        elif c['type'] == 'private':
                ...
                self.create_instance_network_qos(ctxt,
                                                 instance['id'],
                                                 spec_pub,
                                                 spec_pvt)
    ...
    def allocate_for_instance(self, context, **kwargs):
        ...
        self._allocate_fixed_ips(admin_context, instance_id,
                                 ...
                                 requested_networks=requested_networks)
        nw_info = self.get_instance_nw_info(context, instance_id,
                                            instance_uuid, rxtx_factor, host)
        LOG.debug(_("leave rpc_call method allocate_for_instance"))
        return nw_info
    ...
    def deallocate_for_instance(self, context, **kwargs):
        ...
        fixed_ips_ser = []
        for f in fixed_ips:
            fixed_ips_ser.append(utils.model_2_dict(f))

        LOG.debug(_("network deallocation for instance %(instance_id)s with "
                    "fixed ips: %(fixed_ips_ser)s") % locals(),
    ...
    @manager.periodic_task
    def _ensure_ipset(self, context):
        """
        Ensure the accuracy and reliability of the lists and
        members of nova related ipsets
        """
        if FLAGS.intranet_firewall_mode == 'tenant':
            instances = self.db.instance_get_all_by_host(context, self.host)
            projects = set()
            for i in instances:
                projects.add(i['project_id'])

            for pid in projects:
                self._do_ensure_tenant_ipset(context, pid)

    def _do_ensure_tenant_ipset(self, context, project_id):
        instances = self.db.instance_get_all_by_project_include_fixed_ip(
                                    context, project_id)
        for i in instances:
            vm_state = i['vm_state']
            address = i['fixed_ip_address']
            tenant = i['project_id']
            allocated = i['fixed_ip_allocated']

            if vm_state not in ["error", "deleted"] and allocated:
                if not self.driver.is_ipset_existed(tenant):
                    LOG.warn(_('Detect ipset for %(tenant)s not existed, '
                               'created automatically'), locals())
                    self.driver.create_tenant_ipset(tenant)

                if not self.driver.is_address_in_ipset(address, tenant):
                    LOG.warn(_('Detect address %(address)s not in '
                               '%(tenant)s, added automatically'),
                                locals())
                    self.driver.add_address_to_ipset(address, tenant)

            elif vm_state in ("error", "deleted") or not allocated:
                if self.driver.is_address_in_ipset(address, tenant):
                    LOG.warn(_('Detect address %(address)s in %(tenant)s, '
                               'deleted automatically'), locals())
                    self.driver.delete_address_from_ipset(address, tenant)

    def _rpc_add_fixed_ip_to_ipset(self, context, address):
        self.add_fixed_ip_to_ipset(context, address)

    def _rpc_del_fixed_ip_from_ipset(self, context, address):
        self.del_fixed_ip_from_ipset(context, address)

    def add_fixed_ip_to_ipset(self, context, address):
        ...
        host = self.host
        LOG.debug(_('Added ip %(address)s to ipset %(tenant)s'
                    'on host %(host)s'), locals())

        if not self.driver.is_ipset_existed(tenant):
            LOG.debug(_('Create ipset %s'), tenant)
            # NOTE(stanzgy): create_tenant_ipset(tenant) will add all address
            #                of instances that belong to the tenant into the
            #                tenant ipset including this function's invoker's
            #                address as well
            self.driver.create_tenant_ipset(tenant)

        self.driver.add_address_to_ipset(address, tenant)

    def del_fixed_ip_from_ipset(self, context, address):
        fixed_ip_ref = self.db.fixed_ip_get_by_address(context, address)
        instance = self.db.instance_get_by_uuid(context,
                                                fixed_ip_ref['instance_uuid'])
        tenant = instance['project_id']
        if self.driver.is_ipset_existed(tenant):
            host = self.host
            LOG.debug(_('Deleted ip %(address)s from ipset %(tenant)s'
                        'on host %(host)s'), locals())
            self.driver.delete_address_from_ipset(address, tenant)
        else:
            LOG.error("Unable to delete %(address)s from %(tenant)s because"
                      "ipset doesn't exist.", locals())

    def _check_spec(self, spec):
        rate = spec.get('rate', None)
        ceil = spec.get('ceil', None)
        burst = spec.get('burst', None)

        if not rate:
            msg = _('"rate" must be set')
    ...
    def get_instance_qos_id(self, context, instance_id):
        ...
        if FLAGS.use_network_qos:
            return rpc.call(context, topic,
                            {
                             'method': '_get_instance_qos_id',
                             'args': args
                            })

    def _get_instance_qos_id(self, context, instance_id):
        tc = self.driver.tc_manager
        return tc._get_instance_qos_id(instance_id)

    def get_instance_fw_mark(self, context, instance_id):
        instance = self.db.instance_get(context, instance_id)
        ...
        if FLAGS.use_network_qos:
            return rpc.call(context, topic,
                            {
                             'method': '_get_instance_fw_mark',
                             'args': args
                            })

    def _get_instance_fw_mark(self, context, instance_id):
        tc = self.driver.tc_manager
        return tc._instance_fw_mark(instance_id)

    def create_instance_network_qos(self, context, instance_id,
                                     spec_public=None, spec_private=None):
        instance = self.db.instance_get(context, instance_id)

        if FLAGS.use_network_qos:
            if instance['host'] == self.host:
                self._create_instance_network_qos(context,
                                                  instance,
                                                  spec_public,
                                                  spec_private)
            else:
                ...
                args['spec_public'] = spec_public
                args['spec_private'] = spec_private

                rpc.call(context, topic,
                         {
                          'method': '_create_instance_network_qos',
                          'args': args
                         })

    def _create_instance_network_qos(self, context, instance,
                                     spec_public=None, spec_private=None):
        instance_id = instance['id']

        LOG.debug(_("creating network qos for instance %(instance_id)s "
                    "with public spec: %(spec_public)s, "
                    "private spec %(spec_private)s") % locals())

        tc = self.driver.tc_manager

        qos_conf = tc.config
        policy_pub_conf = qos_conf['policy']['public']
        policy_pvt_conf = qos_conf['policy']['private']
        shaping_key = qos_conf['policy']['key']
        shaping_pub_conf = qos_conf['shaping']['public']
        shaping_pvt_conf = qos_conf['shaping']['private']

        if shaping_key == 'ecu':
            key = str(get_ecu_num(instance))
        else:
            key = str(get_vcpu_num(instance))

        shaping_pub_def = shaping_pub_conf.get(key,
                                               shaping_pub_conf['default'])
        shaping_pvt_def = shaping_pvt_conf.get(key,
                                               shaping_pvt_conf['default'])

        # NOTE(stanzgy): get network qos configs from instance metadata if user
        #                has specified
        metadata = self.db.instance_metadata_get(context,
                                                 instance['uuid'])
        m_pub = {}
        m_pvt = {}

        rate_pub_meta = metadata.get('network_qos_public_rate', None)
        rate_pvt_meta = metadata.get('network_qos_private_rate', None)

        if rate_pub_meta:
            try:
                rate_pub_meta = int(rate_pub_meta)
            except ValueError:
                msg = _("'rate' must be int.")
                raise exception.InvalidTcParam(err=msg)

            m_pub['rate'] = rate_pub_meta

            if rate_pub_meta > shaping_pub_def['ceil']:
                m_pub['ceil'] = rate_pub_meta
            else:
                m_pub['ceil'] = shaping_pub_def['ceil']

            m_pub['prio'] = shaping_pub_def['prio']
            self._check_spec(m_pub)

        if rate_pvt_meta:
            try:
                rate_pvt_meta = int(rate_pvt_meta)
            except ValueError:
                msg = _('"rate" must be int.')
                raise exception.InvalidTcParam(err=msg)

            m_pvt['rate'] = rate_pvt_meta

            if rate_pvt_meta > shaping_pvt_def['ceil']:
                m_pvt['ceil'] = rate_pvt_meta
            else:
                m_pvt['ceil'] = shaping_pvt_def['ceil']

            m_pvt['prio'] = shaping_pvt_def['prio']
            self._check_spec(m_pvt)

        spec_inst = {}
        spec_inst['public'] = spec_public or m_pub or shaping_pub_def
        spec_inst['private'] = spec_private or m_pvt or shaping_pvt_def

        # ensure prio
        if not spec_inst['public'].get('prio', None):
            spec_inst['public']['prio'] = shaping_pub_def['prio']

        if not spec_inst['private'].get('prio', None):
            spec_inst['private']['prio'] = shaping_pvt_def['prio']

        try:
            fixed_ips = self.db.fixed_ip_get_by_instance(context,
                                                        instance['uuid'])
        except exception.FixedIpNotFoundForInstance, ex:
            LOG.warn(_("Instance has zero fixed ips, Ignore its network qos."),
                     instance=instance)
            return

        # save prepared network qos info in system metadata
        sys_metadata = self.db.instance_system_metadata_get(
                                        context,
                                        instance['uuid'])
        # remove the old settings
        for key in sys_metadata.keys():
            if key == 'network-qos-pre':
        ...
        for t in ('public', 'private'):
            s = {}
            s['type'] = t
            s.update(spec_inst[t])
            settings.append(s)

        sys_metadata['network-qos-pre'] = json.dumps(settings)
        ...
        for addr in fixed_ips:
            fips = self.db.floating_ip_get_by_fixed_address(context,
                                                            addr['address'])
            for f in fips:
        ...
        l_pub = []
        l_pvt = []
        nodes = self.db.compute_node_get_all(context.elevated())
        for c in nodes:
            l_pub.append(-(c.total_public_network_mbps -
                           c.public_network_mbps_used))
            l_pvt.append(-(c.total_private_network_mbps -
                           c.private_network_mbps_used))

        heapq.heapify(l_pub)
        heapq.heapify(l_pvt)
        l_pub = map(lambda x: -x, l_pub)
        l_pvt = map(lambda x: -x, l_pvt)
        r_pub = spec_inst['public']['rate']
        r_pvt = spec_inst['private']['rate']
        cap_pub = l_pub[0]
        cap_pvt = l_pvt[0]

        if r_pub > cap_pub:
            msg = _("not enough public bandwidth: expected %(r_pub)d, "
                    "max host capacity %(cap_pub)d") % locals()
            raise exception.InvalidTcParam(err=msg)

        if r_pvt > cap_pvt:
            msg = _("not enough private bandwidth: expected %(r_pvt)d, "
                    "max host capacity %(cap_pvt)d") % locals()
            raise exception.InvalidTcParam(err=msg)

        # register instance tc manager
        ret = tc._register(instance['id'])
        instance_id = instance['id']
        LOG.debug(_("register instance %(instance_id)d in tc manager"
                    "with id %(ret)d") % locals())

        # init instance network qos class
        for node in ('ingress', 'egress'):
            for t in ('public', 'private'):
                tc.add_instance_class(instance['id'],
                                      t,
                                      node,
                                      spec_inst[t])

        # setup instance network qos for fixed ips
        for ip in fixed_ips:
            for node in ('ingress', 'egress'):
                if policy_pvt_conf["fixed_ip"][node]:
                    tc.add_instance_filter(instance['id'],
                                           'private',
                                           node,
                                           ip['address'])

        # setup instance network qos for floating ips
        for ip in floating_ips:
            for node in ('ingress', 'egress'):
                if FLAGS.use_private_floating_ip:
                    fip = netaddr.IPAddress(ip['address'])
                    r = netaddr.IPNetwork(FLAGS.private_floating_ip_range)
                    if fip in r:
                        policy_conf = policy_pvt_conf
                        qtype = 'private'
                    else:
                        policy_conf = policy_pub_conf
                        qtype = 'public'
                else:
                    policy_conf = policy_pub_conf
                    qtype = 'public'

                if policy_conf["floating_ip"][node]:
                    tc.add_instance_filter(instance['id'],
                                           qtype,
                                           node,
                                           ip['address'])

        # setup instance network qos of default snat egress
        if policy_pub_conf['default_snat']['egress']:
            tc.add_snat_egress_filter(instance['id'])

        # save network qos info in system metadata
        sys_metadata = self.db.instance_system_metadata_get(
                                        context,
                                        instance['uuid'])
        # remove the old settings
        for key in sys_metadata.keys():
            if key == 'network-qos' or key == 'network-qos-pre':
                del sys_metadata[key]
        ...
        sys_metadata['network-qos'] = json.dumps(settings)
        self.db.instance_system_metadata_update(context,
                                                instance['uuid'],
                                                sys_metadata,
                                                True)

    def create_address_network_qos(self, context, instance_ref, qtype, atype,
                                   address):
        instance = instance_ref

        if FLAGS.use_network_qos:
            if instance['host'] == self.host:
                self._create_address_network_qos(context,
                                                 instance,
                                                 qtype,
                                                 atype,
                                                 address)
                ...
                rpc.call(context, topic,
                         {
                          'method': '_create_address_network_qos',
                          'args': args
                         })

    def _create_address_network_qos(self, context, instance, qtype, atype,
                                    address):
        tc = self.driver.tc_manager

        instance_id = instance['id']
        LOG.debug(_("creating network qos for instance %(instance_id)s "
                    "address %(address)s, type %(qtype)s %(atype)s") %
                                                        locals())
        ...
        if qtype == 'public':
            policy = policy_pub_conf
        else:
            policy = policy_pvt_conf

        for node in ('ingress', 'egress'):
            if policy[atype][node]:
                tc.add_instance_filter(instance['id'],
                                       qtype,
                                       node,
                                       address)

    def delete_instance_network_qos(self, context, instance_id,
                                    host, reserv_quota=False):
        instance = self.db.instance_get(context, instance_id)

        if FLAGS.use_network_qos:
            if host == self.host:
                self._delete_instance_network_qos(context,
                                                  instance,
                                                  reserv_quota)
            else:
                topic = rpc.queue_get_for(context,
                ...
                args = {}
                args['instance'] = instance
                args['reserv_quota'] = reserv_quota

                rpc.call(context, topic,
                         {
                          'method': '_delete_instance_network_qos',
                          'args': args
                         })

    def _delete_instance_network_qos(self, context, instance,
                                     reserv_quota=False):
        LOG.debug(_("deleting network qos for instance %s") %
                  instance['id'])
        ...
        metadata = self.db.instance_system_metadata_get(
                                        context,
                                        instance['uuid'])
        qos_info = metadata.get('network-qos')
        qos_info_pre = metadata.get('network-qos-pre')

        if qos_info or qos_info_pre:
            spec_pub = {}
            spec_pub_pre = {}

            if qos_info:
                qos = json.loads(qos_info)
            ...
            if qos_info_pre:
                qos_pre = json.loads(qos_info_pre)
            ...
            for node in ('ingress', 'egress'):
                for qtype in ('public', 'private'):
                    try:
                        tc.del_instance_class(instance['id'],
                                              qtype,
                                              node)
                    except Exception:
                        pass

            self.db.instance_system_metadata_delete(context,
                                                    instance['uuid'],
                                                    'network-qos')
            self.db.instance_system_metadata_delete(context,
                                                    instance['uuid'],
            ...
            rate_pub = spec_pub.get('rate') or spec_pub_pre.get('rate') or 0
            ...
            if not reserv_quota:
                reserv_pub = QUOTAS.reserve(context,
                                            public_network_bandwidth=-rate_pub,
                                            project_id=project_id)
                QUOTAS.commit(context, reserv_pub, project_id=project_id)

    def delete_address_network_qos(self, context, instance_ref, qtype,
                                                                address):
        instance = instance_ref

        if FLAGS.use_network_qos:
            if instance['host'] == self.host:
                self._delete_address_network_qos(context,
                                                 instance,
                                                 qtype,
                                                 address)
            else:
                ...
                rpc.call(context, topic,
                         {
                          'method': '_delete_address_network_qos',
                          'args': args
                         })

    def _delete_address_network_qos(self, context, instance, qtype, address):
        instance_id = instance['id']

        LOG.debug(_("deleting network qos for instance %(instance_id)s with "
                    "address %(address)s") % locals())

        tc = self.driver.tc_manager

        for node in ('ingress', 'egress'):
            try:
                tc.del_instance_filter(instance['id'],
                                       qtype,
                                       node,
                                       address)
            except Exception, e:
                LOG.warn(_("error while deleting network qos for address "
                           "%(address)s: %(e)s") % locals())

    def modify_network_qos(self, context, instance_id, qtype, spec, flavor_id):
        instance = self.db.instance_get(context, instance_id)

        if FLAGS.use_network_qos:
            if instance['host'] == self.host:
                self._modify_network_qos(context,
                                         instance,
                                         qtype,
                                         spec,
                                         flavor_id)
                ...
                rpc.call(context, topic,
                         {
                          'method': '_modify_network_qos',
                          'args': args
                         })

    def _modify_network_qos(self, context, instance, qtype, spec,
                            flavor_id=None):
        instance_id = instance['id']

        LOG.debug(_("modifying network qos for instance %(instance_id)s, "
                    "type %(qtype)s, spec %(spec)s") % locals())

        tc = self.driver.tc_manager

        if not spec.get('rate'):
            raise exception.InvalidTcParam(err=_("'rate' param is required "
                                                 "when modify network qos"))

        # get network qos info in system metadata
        sys_metadata = self.db.instance_system_metadata_get(
                                        context,
                                        instance['uuid'])
        # get the old settings
        qos_info = json.loads(sys_metadata['network-qos'])
        ...
        for i in qos_info:
            if i['type'] == qtype:
                ...
                if spec.get('ceil') or i.get('ceil'):
                    n['ceil'] = spec.get('ceil') or i.get('ceil')

                if spec.get('burst') or i.get('burst'):
                    n['burst'] = spec.get('burst') or i.get('burst')

                if spec.get('cburst') or i.get('cburst'):
                    n['cburst'] = spec.get('cburst') or i.get('cburst')
        ...
        # update quotas usage
        if qtype == 'public':
            delta = n['rate'] - old_rate
            reserv_pub = QUOTAS.reserve(context,
                                        public_network_bandwidth=delta)
        ...
        try:
            ...
            total_pvt_bw = 0
            expect_bw = 0
            instances = self.db.instance_get_all_by_host(context.elevated(),
                                                         self.host)
            for i in instances:
                metadata = self.db.instance_system_metadata_get(
                                                        context,
                                                        i['uuid'])
                instance_has_netowrk_qos = 'network-qos' in metadata.keys()
                if instance_has_netowrk_qos:
                    instance_qos = json.loads(metadata['network-qos'])
                    for i in instance_qos:
                        if i['type'] == 'public':
                            total_pub_bw += i['rate']
                        else:
                            total_pvt_bw += i['rate']
            ...
            if qtype == 'public':
                expect_bw = total_pub_bw
                max_bw = (FLAGS.network_qos_host_public_bandwidth *
                          FLAGS.network_qos_public_allocation_ratio -
                          FLAGS.reserved_host_network_public_bandwidth)
            else:
                expect_bw = total_pvt_bw
                max_bw = (FLAGS.network_qos_host_private_bandwidth *
                          FLAGS.network_qos_private_allocation_ratio -
                          FLAGS.reserved_host_network_private_bandwidth)

            if expect_bw > max_bw or flavor_id:
                # local bandwidth not enough, resize to other host
                LOG.debug(_("modifying network qos for instance "
                            "%(instance_id)s, expected bandwidth "
                            "%(expect_bw)s greater than max bandwidth "
                            "%(max_bw)d, resize") % locals())

                if flavor_id:
                    try:
                        self.db.instance_type_get_by_flavor_id(context,
                                                               flavor_id)
                ...
                try:
                    ...
                    if rate > cap:
                        msg = _("not enough bandwidth: expected %(rate)d, "
                                "max host capacity %(cap)d") % locals()
                        raise exception.InvalidTcParam(err=msg)

                    compute_api.API().resize(context, instance, flavor_id)
                except Exception, e:
                    # rollback network qos system metadata
                    sys_metadata['network-qos'] = json.dumps(qos_info)
                ...
                # apply tc settings on local host
                LOG.debug(_("modifying network qos for instance "
                            "%(instance_id)s, expected bandwidth "
                            "%(expect_bw)s less than max bandwidth "
                            "%(max_bw)d, local modify") % locals())
                try:
                    for node in ('ingress', 'egress'):
                        tc.mod_instance_class(instance_id, qtype, node, n)
                except Exception, e:
                    # rollback network qos system metadata
                    sys_metadata['network-qos'] = json.dumps(qos_info)
                    self.db.instance_system_metadata_update(
            ...
            if qtype == 'public':
                QUOTAS.commit(context, reserv_pub)
        except Exception, e:
            LOG.error(_("modify network qos error: %s") % e)
            with excutils.save_and_reraise_exception():
                if qtype == 'public':
                    QUOTAS.rollback(context, reserv_pub)

            raise

    def get_network_qos(self, context, instance_id):
        if FLAGS.use_network_qos:
            instance = self.db.instance_get(context, instance_id)

            metadata = self.db.instance_system_metadata_get(context,
                                                            instance['uuid'])
            qos_info = metadata.get('network-qos')
            if qos_info:
                qos = json.loads(qos_info)
    ...
    def allocate_fixed_ip(self, context, instance_id, network, **kwargs):
        ...
        if network['cidr']:
            ...
            if address:
                ...
                if FLAGS.use_dhcp_by_location:
                    address = self.db.fixed_ip_associate_pool_in_subnet(
                                        context,
                                        network['id'],
                                        FLAGS.fixed_range_by_location,
                                        FLAGS.fixed_range_by_location_start,
                                        instance_ref['uuid'])
                else:
            ...
            # NOTE(stanzgy): add fixed ip to tenant ipset if using intranet
            #       firewall if instance of the tenant belongs to more than one
            #       host, updating ipset information on every related host is
            #       required via rpc
            #
            #       - if tenant ipset is not created, create here
            #       - ipset could only create set with setname of 31 characters
            #         at max, but tenant uuid in keystone is 32 characters
            #         long, so truncate the tenant uuid last char
            #
            #       ipset create {tenant-uuid[0:31]} bitmap:ip range
            #           {tenant-network-range}
            #       ipset add {tenant-uuid[0:31]} {inst-fixed-ip}

            if FLAGS.intranet_firewall_mode == 'tenant':
                instance = self.db.instance_get(context, instance_id)
                tenant_instances = self.db.instance_get_all_by_project(context,
                        instance['project_id'])
                tenant_hosts = list(set([i['host'] for i in tenant_instances
                                        if i['host']]))

                instance_uuid = instance['uuid']
                LOG.debug(_('allocate_fixed_ip instance %(instance_uuid)s '
                            'related tenant hosts: %(tenant_hosts)s')
                            % locals())
                ...
                for host in tenant_hosts:
                    if host != self.host:
                        ...
                        rpc.cast(context, topic,
                                 {'method': '_rpc_add_fixed_ip_to_ipset',
                                  'args': args})
                    else:
                        self.add_fixed_ip_to_ipset(context, address)

            if FLAGS.use_network_qos:
                LOG.debug(_("adding network qos for instance %(instance_id)d, "
                            "fixed ip %(address)s.") % locals())
                ...
                if len(fixed_ips) > 1:
                    ...
                    for f in fixed_ips:
                        fixed_ips_ser.append(utils.model_2_dict(f))
                    LOG.warn(_("detect more than one fixed ip on network init:"
                               " %(fixed_ips_ser)s") % locals())

                self.create_instance_network_qos(context,
                                                 instance_ref['id'])
    ...
    def deallocate_fixed_ip(self, context, address, host=None, teardown=True):
        ...
        # NOTE(stanzgy): remove fixed ip from tenant ipset if using intranet
        #       firewall if instance of the tenant belongs to more than one
        #       host, updating ipset information on every related host is
        #       required via rpc
        ...
        if FLAGS.intranet_firewall_mode == 'tenant':
            ...
            instance_uuid = instance['uuid']
            LOG.debug(_('deallocate_fixed_ip instance %(instance_uuid)s '
                        'related tenant hosts: %(tenant_hosts)s') % locals())
            ...
            for host in tenant_hosts:
                if host != self.host:
                    ...
                    rpc.cast(context, topic,
                             {'method': '_rpc_del_fixed_ip_from_ipset',
                              'args': args})
                else:
                    self.del_fixed_ip_from_ipset(context, address)
        ...
        if FLAGS.use_network_qos:
            instance_id = instance['id']
            LOG.debug(_("deleting network qos for instance %(instance_id)d, "
                        "fixed ip %(address)s.") % locals())
            ...
            if len(fixed_ips) > 1:
                ...
                self.delete_instance_network_qos(context,
                                                 instance['id'],
                                                 instance['host'])
    ...
    @wrap_check_policy
    def get_allocated_fixed_ips(self, context):
        """Return allocated fixed ips"""
        fixed_ips = self.db.fixed_ip_get_all_allocated(context)
        return [dict(fixed_ip.iteritems()) for fixed_ip in fixed_ips]
...
class FlatDHCPManager(RPCAllocateFixedIP, FloatingIP, NetworkManager):
    ...
    def modprobe_modules_for_network(self):
        """
        Modprobe modules which are used by nova-network service
        """
        for mod in FLAGS.kernel_modules_for_network:
            if mod == 'vhost_net' and not FLAGS.use_vhost_net:
                continue
            utils.modprobe_module(mod.split())

    def init_host(self):
        """Do any initialization that needs to be run if this is a
        standalone service.
        """
        # modprobe kernel modules for nova-network
        self.modprobe_modules_for_network()

        #NOTE(stanzgy): defer to apply iptable changes until init is done
    ...
    def _setup_network_on_host(self, context, network):
        ...
        if not FLAGS.fake_network:
            dev = self.driver.get_dev(network)
            self.driver.update_dhcp(context, dev, network, kill_dhcp=True)
            if(FLAGS.use_ipv6):
