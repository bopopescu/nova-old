...
class FixedIpNotAssociatedWithInstance(NovaException):
    message = _("Fixed IP address %(address)s is not associated with any "
                "instances.")
...
class NumaNodeFileNotFound(FileNotFound):
    message = _("The NUMA node info file %(file_path)s could not be found")


class EcuKeyError(NovaException):
...
class NbsTimeout(NbsException):
    message = _("Nbs operation timeout")


class NbsAttachForbidden(NbsException):
    message = _("Instance is forbidden to attach nbs volume")
...
class InvalidTcParam(TcInvalid):
    message = _("%(err)s")


class InvalidTcDevice(TcInvalid):
    message = _("%(err)s")


class InvalidTcConfigFile(TcInvalid):
    message = _("%(err)s")


class InvalidTcInstanceId(TcInvalid):
    message = _("%(err)s")


class TcNotFound(NotFound):
    message = _("%(err)s")


class TcDeviceNotFound(TcNotFound):
    message = _("%(err)s")


class TcClassNotFound(TcNotFound):
    message = _("%(err)s")


class TcConfigFileNotFound(TcNotFound):
    message = _("%(err)s")


class PublicBandwidthLimitExceeded(QuotaError):
    message = _("Maximum amount of public network bandwidth exceeded")


class PrivateBandwidthLimitExceeded(QuotaError):
    message = _("Maximum amount of private network bandwidth exceeded")
