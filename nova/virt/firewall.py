...
firewall_opts = [
    ...
    cfg.BoolOpt('allow_other_fixed_ips',
                default=False,
                help='If True, sg rules will be applied to other fixed ips'),
    cfg.BoolOpt('del_sec_rule_conntracks',
                default=False,
                help='Whether to delete conntrack entries when '
                     'security group rules removed.'),
    cfg.BoolOpt('log_inter_tenant_traffic', default=False,
                help='Keep track of inter-tenant traffic'),
    cfg.StrOpt('intra_tenant_traffic_log_rate', default='10/m',
               help='Lmit the rate of logging intra-tenant traffic'),
    cfg.BoolOpt('allow_ipencap', default=True,
                help='Allow all IP encapsulated in IP packets (for LVS-TUN)'),
]
...
class IptablesFirewallDriver(FirewallDriver):
    ...
    def __init__(self, **kwargs):
        ...
        if FLAGS.intranet_firewall_mode == 'tenant':
            ...
            linux_net.init_ipset(admin_context)
    ...
    def do_unfilter_instance(self, instance):
        ret = False
        LOG.debug(_('Going to unfilter instance with id: %s') %
        ...
        if self.instances.pop(instance['id'], None):
            ...
            LOG.debug(_('Filtered instances: %s') % self.instances,
                      instance=instance)
    ...
    def _create_from_filter(self, ips, chain_name):
        return ['-s %s -j $%s' % (ip, chain_name) for ip in ips]

    def _filters_from_instance(self, chain_name, network_info):
        """Creates a rule corresponding to each ip that defines a
             jump to the corresponding instance - chain for all the traffic
             sourced from that ip."""
        # make sure this is legacy nw_info
        network_info = self._handle_network_info_model(network_info)

        ips_v4 = [ip['ip'] for (_n, mapping) in network_info
                  for ip in mapping['ips']]
        ipv4_rules = self._create_from_filter(ips_v4, chain_name)

        ipv6_rules = []
        if FLAGS.use_ipv6:
            ips_v6 = [ip['ip'] for (_n, mapping) in network_info
                      for ip in mapping['ip6s']]
            ipv6_rules = self._create_from_filter(ips_v6, chain_name)
    ...
    def _add_mangle_rules(self, chain_name, ipv4_rules, ipv6_rules):
        for rule in ipv4_rules:
            self.iptables.ipv4['mangle'].add_rule(chain_name, rule)

        if FLAGS.use_ipv6:
            for rule in ipv6_rules:
                self.iptables.ipv6['mangle'].add_rule(chain_name, rule)

    def add_fw_mark(self, instance):
        ipv4_rules = []
        ipv6_rules = []
        ...
        try:
            tc_id = tc._get_instance_qos_id(instance_id)
        except exception.TcInvalid:
            LOG.warn(_("Failed to get instance %(instance_id)s tc id, "
                       "cannot add tc fw mark.") % locals())
            return ipv4_rules, ipv6_rules

        ipv4_rules.append('-j MARK --set-mark %(tc_id)s' % locals())

        if FLAGS.use_ipv6:
            ipv6_rules.append('-j MARK --set-mark %(tc_id)s' % locals())
    ...
    def add_filters_for_instance(self, instance):
        ...
        if FLAGS.use_network_qos:
            self.iptables.ipv4['mangle'].add_chain(chain_name)
            ipv4_rules, ipv6_rules = self._filters_from_instance(chain_name,
                                                                 network_info)
            self._add_mangle_rules('tc', ipv4_rules, ipv6_rules)
            ipv4_rules, ipv6_rules = self.add_fw_mark(instance)
            self._add_mangle_rules(chain_name, ipv4_rules, ipv6_rules)

    def remove_filters_for_instance(self, instance):
        instance_id = instance['id']
        LOG.debug(_("removing iptables filter for instance %(instance_id)s") %
                                                                locals())
        chain_name = self._instance_chain_name(instance)
        ...
        if FLAGS.use_network_qos:
            self.iptables.ipv4['mangle'].remove_chain(chain_name)

            if FLAGS.use_ipv6:
                self.iptables.ipv6['mangle'].remove_chain(chain_name)
    ...
    def _do_basic_rules(self, ipv4_rules, ipv6_rules, network_info):
        ...
        # NOTE(gtt): We see some bugs about this rule.
        # See CLOUD-4980,CLOUD-4804 So remote these.
    ...
    # FIXME: no ipv6 rules?
    def _do_intranet_firewall_rules(self, ipv4_rules, ipv6_rules, setname):
        ipv4_rules.append('-m set --match-set %s src -j ACCEPT' % setname)
        if FLAGS.log_inter_tenant_traffic:
            ipv4_rules.append('-d %s -m conntrack ! --ctstate DNAT -m '
                              'conntrack --ctstate NEW -m limit --limit %s '
                              '-j LOG --log-prefix  "NOVA: "'
                              % (FLAGS.fixed_range,
                                 FLAGS.intra_tenant_traffic_log_rate))
        if not FLAGS.allow_other_fixed_ips:
            ipv4_rules.append('-d %s -m conntrack ! --ctstate DNAT -j DROP'
                                % FLAGS.fixed_range)
    ...
    def _do_ipencap_rules(self, ipv4_rules, ipv6_rules):
        ipv4_rules.append('-p ipencap -j ACCEPT')
        if FLAGS.use_ipv6:
            ipv6_rules.append('-p ipencap -j ACCEPT')
    ...
    def refresh_security_group_members(self, security_group):
        fixed_ips = self.do_refresh_security_group_rules(security_group)
        self.iptables.apply()

        if FLAGS.del_sec_rule_conntracks:
            for i in fixed_ips:
                linux_net.del_conntrack_entries(i)
    ...
    def do_refresh_instance_rules(self, instance):
        # the instance may has been deleted before getting the lock
        if instance['id'] not in self.instances:
            LOG.info(_('Attempt to refresh sg rules of instance which has '
                       'been deleted already'), instance=instance)
            return
        self.remove_filters_for_instance(instance)
        self.add_filters_for_instance(instance)
