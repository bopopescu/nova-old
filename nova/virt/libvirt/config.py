...
class LibvirtConfigGuest(LibvirtConfigObject):
    ...
    def _format_blkiotune(self, root):
        blkiotune = etree.Element("blkiotune")
        if self.blk_devices != [] and self.blk_weight is not None:
            for dev in self.blk_devices:
                device = etree.Element("device")
                device.append(self._text_node("path", dev))
                device.append(self._text_node("weight", self.blk_weight))
                blkiotune.append(device)
        if len(blkiotune) > 0:
            root.append(blkiotune)
