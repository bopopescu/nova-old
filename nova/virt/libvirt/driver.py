...
libvirt_opts = [
    ...
    cfg.StrOpt('vcpu_pin_set',
               ...
               help='Which pcpus can be used by vcpus of instance '
                    'e.g: "4-12,^8,15", or "4-$" means pin to 4-last_pcpu, '
                    'NOTE(wangpan): "1-2,4-$,^8", "4-$,^8", "1-2,4-$" and '
                    '"2,3,4-$" are all invalid'),
    ]

snapshot_opts = [
    cfg.BoolOpt('allow_image_snapshot_optimization',
               default=True,
               help='allow optimizing image snapshot operation or not')
    ]

cpuqos_opts = [
    cfg.BoolOpt('enable_cpuqos',
                default=False,
                help='Determine enable cpu qos or not.'),
    cfg.IntOpt('ecu_ratio',
               default=4,
               help='The num of ecus that a pcpu can support'),
    cfg.IntOpt('cpu_period',
               default=100000,
               help='Cpu period for QoS, you can find more info about it at:'
                    'http://libvirt.org/formatdomain.html#elementsCPUTuning'),
    cfg.IntOpt('cpu_shares',
               default=1024,
               help='Cpu shares for QoS, you can find more info about it at:'
                    'http://libvirt.org/formatdomain.html#elementsCPUTuning'),
    cfg.IntOpt('free_bursting_ecu_limit',
               default=4,
               help='The ecu num limitation of an instance whether can free '
                    'bursting cpu usage or not, total ecu num LESS than '
                    'this flag will enable free bursting'),
    cfg.IntOpt('free_bursting_ecu_num',
               default=1,
               help='The free bursting ecu num, the instance which enable '
                    'free bursting will get more num of ecus as this flag'),
    cfg.ListOpt('fine_tuning_factors',
                default=[1.2, 1.15, 1.1, 1.0],
                help='Fine tuning factors cpu qos quota parameter, '
                     'the first value is the factor of one ecu, '
                     'and the second is the factor of two ecus, and so on.'),
    ]

FLAGS = flags.FLAGS
FLAGS.register_opts(libvirt_opts)
FLAGS.register_opts(snapshot_opts)
FLAGS.register_opts(cpuqos_opts)
...
class LibvirtDriver(driver.ComputeDriver):
    ...
    def init_host(self, host):
        ...
        # Ensuring NUMA node info file exists when nova-compute starts
        numa_node_file = "/sys/devices/system/node/online"
        if not os.path.exists(numa_node_file):
            LOG.error(_("NUMA node info file %(numa_node_file)s could "
                        "not be found") % locals())
            raise exception.NumaNodeFileNotFound(file_path=numa_node_file)
    ...
    def _destroy(self, instance):
        ...
        LOG.debug(_("begin to wait for instance destroy."), instance=instance)
        timer = utils.LoopingCall(_wait_for_destroy, old_domid)
        timer.start(interval=0.5).wait()
        if kwargs['is_running']:
            LOG.info(_("Going to destroy instance again."), instance=instance)
            self._destroy(instance)
        LOG.debug(_("wait for instance destroy end."), instance=instance)
    ...
    def _cleanup(self, instance, network_info, block_device_info):
        ...
        LOG.debug(_("begin to unplug_vifs."), instance=instance)
        self.unplug_vifs(instance, network_info)
        retry = True
        while retry:
            try:
                LOG.debug(_("begin to unfilter_instance."), instance=instance)
    ...
    def _get_dev_num(self, host_dev):
    ...
    def _set_nbs_qos(self, cgroup_dir, qos_info, host_dev):
        dev_num = self._get_dev_num(host_dev)
        if dev_num is None:
            LOG.error(_("Device number of %s not found") % host_dev)
            return

        if ('iotune_read_bytes' in qos_info
                or 'iotune_write_bytes' in qos_info
                or 'iotune_read_iops' in qos_info
                or 'iotune_write_iops' in qos_info):
            read_bps_value = qos_info.get('iotune_read_bytes')
            if read_bps_value is not None:
                cgroup_path = os.path.join(cgroup_dir,
                                           'blkio.throttle.read_bps_device')
                read_bps_info = "%s %s\n" % (dev_num, read_bps_value)
                utils.set_cgroup_parameter(cgroup_path, read_bps_info)

            write_bps_value = qos_info.get('iotune_write_bytes')
            if write_bps_value is not None:
                cgroup_path = os.path.join(cgroup_dir,
                                           'blkio.throttle.write_bps_device')
                write_bps_info = "%s %s\n" % (dev_num, write_bps_value)
                utils.set_cgroup_parameter(cgroup_path, write_bps_info)

            read_iops_value = qos_info.get('iotune_read_iops')
            if read_iops_value is not None:
                cgroup_path = os.path.join(cgroup_dir,
                                           'blkio.throttle.read_iops_device')
                read_iops_info = "%s %s\n" % (dev_num, read_iops_value)
                utils.set_cgroup_parameter(cgroup_path, read_iops_info)

            write_iops_value = qos_info.get('iotune_write_iops')
            if write_iops_value is not None:
                cgroup_path = os.path.join(cgroup_dir,
                                           'blkio.throttle.write_iops_device')
                write_iops_info = "%s %s\n" % (dev_num, write_iops_value)
                utils.set_cgroup_parameter(cgroup_path, write_iops_info)
        elif ('iotune_total_bytes' in qos_info
                or 'iotune_total_iops' in qos_info):
            rw_bps_value = qos_info.get('iotune_total_bytes')
            if rw_bps_value is not None:
                cgroup_read_path = os.path.join(cgroup_dir,
                                           'blkio.throttle.read_bps_device')
                cgroup_write_path = os.path.join(cgroup_dir,
                                           'blkio.throttle.write_bps_device')
                rw_bps_info = "%s %s\n" % (dev_num, rw_bps_value)
                utils.set_cgroup_parameter(cgroup_read_path, rw_bps_info)
                utils.set_cgroup_parameter(cgroup_write_path, rw_bps_info)

            rw_iops_value = qos_info.get('iotune_total_iops')
            if rw_iops_value is not None:
                cgroup_read_path = os.path.join(cgroup_dir,
                                           'blkio.throttle.read_iops_device')
                cgroup_write_path = os.path.join(cgroup_dir,
                                           'blkio.throttle.write_iops_device')
                rw_iops_info = "%s %s\n" % (dev_num, rw_iops_value)
                utils.set_cgroup_parameter(cgroup_read_path, rw_iops_info)
                utils.set_cgroup_parameter(cgroup_write_path, rw_iops_info)
        else:
            LOG.error(_("Cgroup value of %s not set") % host_dev)

    def _set_nbs_qos_on_create(self, dom_xml):
        '''
           We use cgroup to control the qos of nbs volume, but the blkio device
           will be removed after instance stopping, so we need to reset it on
           instance is started.
        '''
        doc = etree.fromstring(dom_xml)
        instance_uuid = doc.findtext('uuid')
        instance_name = doc.findtext('name')
        disk_sources = doc.findall('devices/disk/source')
        disk_host_devs = [s.attrib.get('dev') for s in disk_sources
                            if 'dev' in s.attrib]
        cgroup_dir = utils.get_instance_blkio_cgroup_path(instance_name,
                                                          instance_uuid)
        bdms = db.block_device_mapping_get_all_by_instance(
                            nova_context.get_admin_context(), instance_uuid)
        for bdm in bdms:
            try:
                conn_info = jsonutils.loads(bdm['connection_info'])
                host_dev = conn_info['host_dev']
                qos_info = conn_info['qos_info']
            except (KeyError, TypeError, ValueError):
                LOG.warn(_("QoS info of %s is missing") % instance_uuid)
                continue
            if host_dev not in disk_host_devs:
                LOG.warn(_("%(host_dev)s not in the xml of instance %(uuid)s")
                         % {'host_dev': host_dev, 'uuid': instance_uuid})
            self._set_nbs_qos(cgroup_dir, qos_info, host_dev)
    ...
    def _find_free_dev_and_pcislot(self, instance_name, used_dev):
        ...
        # NOTE(wangpan): the used slot and dev may reduplicate between libvirt
        #                and DB, but it does not matter, we just get the device
        #                not in the list.
        if used_dev is not None:
            if used_dev.get('slot', []) != []:
                used_pci_slot_ids.extend(used_dev['slot'])
            if used_dev.get('dev', []) != []:
                used_disk_devs.extend(used_dev['dev'])
    ...
    def extend_nbs_volume(self, instance_name, device, size):
        ...
        if FLAGS.libvirt_type == 'lxc':
            LOG.error(_("LXC hypervisor is not supported to extend volume."))
    ...
    @exception.wrap_exception()
    def update_nbs_qos(self, instance_name, instance_uuid, host_dev, qos_info):
        """Tell instance the nbs volume qos info has been changed"""
        if FLAGS.libvirt_type == 'lxc':
            LOG.error(_("LXC hypervisor is not supported to update qos info"))
            raise exception.NotSupported()
        ...
        try:
            state = LIBVIRT_POWER_STATE[virt_dom.info()[0]]
            if state == power_state.RUNNING:
                cgroup_dir = utils.get_instance_blkio_cgroup_path(
    ...
    @exception.wrap_exception()
    def attach_nbs_volume(self, instance_name, instance_uuid, device, host_dev,
                          qos_info, volume_id):
        """
        ...
        try:
            ...
            if isinstance(ex, libvirt.libvirtError):
                ...
                if errcode == libvirt.VIR_ERR_OPERATION_FAILED:
                    raise exception.DeviceIsBusy(device=target_dev)
                elif errcode == libvirt.VIR_ERR_INVALID_ARG:
                    # target dev already exists
                    return (True, {'dev': target_dev})
                elif errcode == libvirt.VIR_ERR_XML_ERROR:
                    # pci slot already used
                    return (True, {'slot': device['slot']})
                elif (errcode == libvirt.VIR_ERR_INTERNAL_ERROR and
                        'device_add' in errmsg):
                    # NOTE(wangpan): Fix "internal error unable to execute
                    #                QEMU command 'device_add': Duplicate ID
                    #                'virtio-disk5' for device" and "internal
                    #                error unable to execute QEMU command
                    #                'device_add': Device 'virtio-blk-pci'
                    #                could not be initialized
                    # the device may be deleting in QEMU, try another device.
                    return (True, {'slot': device['slot'], 'dev': target_dev})
                else:
                    LOG.error(_("Error code %(errcode)d, message %(errmsg)s, "
                                "instance name %(inst_name)s, volume_id "
                                "%(volume)s") % {"errcode": errcode,
                                                 "errmsg": errmsg,
    ...
    @exception.wrap_exception()
    def detach_nbs_volume(self, instance_name, instance_uuid,
                          mountpoint, host_dev):
        """Detach a nbs volume from an instance"""
        if FLAGS.libvirt_type == 'lxc':
            LOG.error(_("LXC hypervisor is not supported to detach volume."))
            raise exception.NotSupported()

        # Clear the qos info of detaching volume firstly
        qos_info = {'iotune_read_bytes': 0,
                    'iotune_write_bytes': 0,
                    'iotune_read_iops': 0,
                    'iotune_write_iops': 0}
        self.update_nbs_qos(instance_name, instance_uuid, host_dev, qos_info)
        ...
        try:
            ...
            if error_code == libvirt.VIR_ERR_NO_DOMAIN:
                # NOTE(vish):
                LOG.warn(_("During detach_nbs_volume, instance disappeared."))
            else:
                LOG.exception(ex)
                raise ex
    ...
    def snapshot(self, context, instance, image_href):
        ...
        if FLAGS.allow_image_snapshot_optimization and use_qcow2 \
              and not instance['kernel_id'] and not instance['ramdisk_id']:
            snapshot_optimization = True
        else:
            snapshot_optimization = False
        LOG.info(_("use snapshot optimization in snapshot: %s"),
                snapshot_optimization, instance=instance, context=context)
        ...
        # Make the snapshot, if snapshot optimization isn't used
        if not snapshot_optimization:
        ...
        with utils.tempdir(dir=snapshot_directory) as tmpdir:
            try:
                ...
                if snapshot_optimization:
                    self.update_image_online(context, virt_dom,
                                             state, out_path,
                                             disk_path, image_href,
    ...
    def update_image_online(self, context, dom, dom_state,
                            out_path, src, image_href,
                            image_service, metadata):
        """
        Upload snapshot image to image service while domain is online.
        """
        def commit_temp_snapshot(disk, temp_path):
            """
            Commit snapshot to disk and recover disk's name.

            :param disk: disk path
            :param temp_path: domain disk path after rename
            """
            cmd = ('qemu-img', 'commit', '-f', 'qcow2', '%s' % disk)
            try:
                utils.execute(*cmd, run_as_root=True)
            except Exception:
                LOG.warn(_('Commit new data produced during snapshoting'
                           ' to disk failed, domain lost some data!'))

            utils.execute('rm', '-f', '%s' % disk)
            utils.execute('mv', temp_path, disk)

        def make_temp_snapshot(disk, temp_path):
            """
            Rename disk and make a qcow image from disk using disk's old name.

            :param disk: disk path
            :param temp_path: domain disk path after rename
            """
            if os.path.exists(temp_path):
                backing_file = libvirt_utils.get_disk_backing_file(disk)
                if backing_file == os.path.basename(temp_path):
                    commit_temp_snapshot(disk, temp_path)

            utils.execute('mv', disk, temp_path)
            try:
                libvirt_utils.create_cow_image(temp_path, disk)
            except Exception, ex:
                utils.execute('mv', temp_path, disk)
                raise ex

        temp_path = "%s_temp" % src
        make_temp_snapshot(src, temp_path)

        if dom_state == power_state.RUNNING:
            dom.resume()

        # Merge disk and backing file into snapshot.
        utils.execute('qemu-img', 'convert', '-f', 'qcow2',
                      '-O', 'qcow2', temp_path, out_path)
        ...
        try:
            ...
            commit_temp_snapshot(src, temp_path)
    ...
    def spawn(self, context, instance, image_meta, injected_files,
        ...
        # NOTE(hzyangtk): Resume personality from db for system rebuild.
        #                 catch from table instance_system_metadata_extension
        if FLAGS.allow_store_personality:
            injected_files = self._resume_personality(context, instance,
                                                      injected_files)

        # NOTE(hzyangtk): add extra inject files with auto inject scripts
        #                 and load from inject_files.json
        if FLAGS.allow_inject_files:
            injected_files = self._inject_files_from_backend(instance,
                                                             injected_files)
    ...
    def _resume_personality(self, context, instance, injected_files):
        """Resume personality datas which store in DB into injected_files"""
        try:
            meta_items = db.instance_system_metadata_extension_get(
                                context, instance['uuid'])
            personality = jsonutils.loads(meta_items.get('personality', '[]'))
            if injected_files == [] or injected_files == personality:
                injected_files = personality
            else:
                meta_item = {}
                meta_item['personality'] = str(injected_files)
                db.instance_system_metadata_extension_update(
                        context, instance['uuid'], meta_item, delete=False)
        except Exception:
            LOG.exception(_("Resume personality from DB failed with"
                            " instance: %s") % instance['uuid'])
        return injected_files

    def _inject_files_from_backend(self, instance, injected_files):
        """Load inject files from backend into injected_files"""
        backend_inject_files = []
        try:
            try:
                inject_files_dict = inject_files.get_inject_files()
            except exception.ConfigNotFound:
                LOG.exception(_("Load inject files config failed"))
                inject_files_dict = {}
            except exception.Invalid:
                LOG.exception(_("Inject files content invalid"))
                inject_files_dict = {}
            inject_files_list = inject_files_dict.get('inject_files', [])
            for item in inject_files_list:
                try:
                    path = item['path']
                    contents = item['contents']
                except KeyError as key:
                    LOG.exception(_("Bad inject files "
                                    "format: missing %s") % key)
                    continue
                except TypeError:
                    LOG.exception(_("Bad inject files format"))
                    continue

                contents = utils.decode_base64(contents)
                if contents is None:
                    LOG.exception(_("Inject files content for path %s "
                                    "cannot be decoded") % path)
                    continue
                backend_inject_files.append([path, contents])
        except Exception:
            # NOTE(hzyangtk): when backend inject files exception happens,
            #                 print it, but still let vm create normally.
            LOG.exception(_("Backend inject files function failed with "
                            "instance: %s") % instance['uuid'])
        else:
            injected_files.extend(backend_inject_files)
        return injected_files
    ...
    def get_host_ip_addr():
        global host_intranet_ip
        if FLAGS.use_intranet:
            if host_intranet_ip is None:
                host_intranet_ip = get_host_intranet_ip()
            if host_intranet_ip is None:
                raise exception.ComputeHostNotFound(host=FLAGS.host)
            return host_intranet_ip
        else:
            return FLAGS.my_ip
    ...
    def _create_ephemeral_base_if_not_exists(self, backing_file, size,
                                             fslabel, os_type):
        lock_path = os.path.join(FLAGS.instances_path, 'locks')

        @utils.synchronized(os.path.basename(backing_file),
                            external=True, lock_path=lock_path)
        def create_if_not_exists(backing_file, size, fslabel, os_type):
            if not os.path.exists(backing_file):
                self._create_ephemeral(backing_file, size,
                                       fslabel, os_type)

        create_if_not_exists(backing_file, size, fslabel, os_type)

    def _restore_ephemeral_disk(self, context, instance, target):
        #create base dir if not exsists
        base_dir = os.path.join(FLAGS.instances_path, '_base')
        if not os.path.exists(base_dir):
            utils.ensure_tree(base_dir)

        old_backing_file = libvirt_utils.get_disk_backing_file(target)
        #if the disk does not have backing file, just Raise Exception
        #FIXME:here we raise exception even if the targe really dose not have
        #backing file,but currently won't happen in our system
        if not old_backing_file:
            LOG.warning(_("get backing file failed"),
                       instance=instance, context=context)
            raise exception.DiskNotFound(location=old_backing_file)
        old_backing_file = os.path.join(base_dir, old_backing_file)

        old_size = libvirt_utils.get_disk_size(target)
        old_size = old_size / (1024L * 1024L * 1024L)
        new_size = instance['ephemeral_gb']
        size = max(old_size, new_size)
        #if the current backing file does not exists, create it
        self._create_ephemeral_base_if_not_exists(old_backing_file,
                                                  old_size,
                                                  'ephemeral0',
                                                  instance['os_type'])
        #if resize ephemeral disk, create new backing file if not exist,
        #and extend the tagert to new backing file
        new_backing_file = "ephemeral_%s_%s_%s" % ("0",
                                                   instance['ephemeral_gb'],
                                                   instance['os_type'])
        new_backing_file = os.path.join(base_dir, new_backing_file)
        if size != old_size:
            self._create_ephemeral_base_if_not_exists(new_backing_file,
                                                      size,
                                                      'ephemeral0',
                                                      instance['os_type'])
            libvirt_utils.rebase_cow_image(new_backing_file, target)
    ...
    def _create_image(self, context, instance, libvirt_xml, suffix='',
        ...
        LOG.info(_("use snapshot optimization in _create_image: "
                  "%(snapshot_optimization)s, %(image_type)s, "
                  "%(backing_file)s, %(has_base_id_sha1)s") % locals(),
                  instance=instance, context=context)
        ...
        if ephemeral_gb and not self._volume_in_mapping(
            ...
            if snapshot_optimization and \
                    os.path.exists(basepath('disk.local')):
                self._restore_ephemeral_disk(context, instance,
                                             basepath('disk.local'))
            else:
    ...
    def _gen_cpu_qos_paras(self, guest, inst_type):
        """
        Generate cpu QoS parameters for XML configuration of libvirt.
        """
        fine_tuning_factors = [float(f) for f in FLAGS.fine_tuning_factors]

        ecus_per_vcpu = int(inst_type["extra_specs"]["ecus_per_vcpu:"])
        if ecus_per_vcpu <= 0:
            LOG.warn(_("Instance type %s contains none ecu") % inst_type)
            return

        ecus_total = ecus_per_vcpu * inst_type["vcpus"]
        if ecus_total < FLAGS.free_bursting_ecu_limit:
            ecus_per_vcpu += FLAGS.free_bursting_ecu_num
        # FIXME(wangpan): we have NOT tested for more than 4 ecus per vcpu.
        if ecus_per_vcpu > len(fine_tuning_factors):
            factor = fine_tuning_factors[-1]
        else:
            factor = fine_tuning_factors[ecus_per_vcpu - 1]
        # NOTE(wangpan): libvirt will modify this quota by multiplying
        #                vcpu num, so we needn't do this here.
        guest.cpu_quota = int(float(ecus_per_vcpu) /
                FLAGS.ecu_ratio * FLAGS.cpu_period * factor)
        guest.cpu_period = FLAGS.cpu_period
        if guest.cpu_quota > guest.cpu_period:
            LOG.debug(_("Cpu quota is larger than period, instance type is %s")
                    % inst_type)
            guest.cpu_quota = guest.cpu_period
        guest.cpu_shares = ecus_total * FLAGS.cpu_shares

    def _gen_blkio_info(self, guest, inst_type):
        guest.blk_devices = \
                [dev.get('disk') for dev in FLAGS.instance_storage_devices]
        weight = 5 * (inst_type['root_gb'] + inst_type['ephemeral_gb'])
        if weight < 100:
            weight = 100
        elif weight > 1000:
            weight = 1000
        guest.blk_weight = weight
    ...
    def get_guest_config(self, instance, network_info, image_meta, rescue=None,
        ...
        if FLAGS.enable_cpuqos:
            guest.cpuset = self._vcpu_pin_set
            self._gen_cpu_qos_paras(guest, inst_type)

        if FLAGS.use_instance_storage_qos:
            self._gen_blkio_info(guest, inst_type)
    ...
    def _create_domain(self, xml=None, domain=None, launch_flags=0):
        ...
        self._enable_hairpin(dom_xml)
        self._set_nbs_qos_on_create(dom_xml)
        return domain
    ...
    def _create_domain_and_network(self, xml, instance, network_info,
        ...
        # add by wangpan, remove the configuration in instance XML when we find
        # the host device is not exists, such as user detaches the volume when
        # instance is rebooting.
        def remove_invalid_dev(xml, err_msg):
            domain_root = etree.fromstring(xml)
            devices = domain_root.find("devices")
            disks = devices.findall("disk")
            remove_dev_notfound = False
            for disk in disks:
                # NOTE(hzwangpan): we just remove 'block' disk, ignore 'file'.
                source = disk.find("source").items()[0]
                if source[0] != "dev":
                    continue
                host_dev = source[1]
                if host_dev in err_msg and not os.path.exists(host_dev):
                    LOG.warn(_("remove invalid host dev %s now"),
                             host_dev, instance=instance)
                    devices.remove(disk)
                    remove_dev_notfound = True
                    break
        ...
        while retry:
            try:
                ...
                err_msg = ex.get_error_message()
                LOG.warn(_("libvirt error: error code %(err_code)d, error "
                       "message: %(err_msg)s") % locals(), instance=instance)
                # checking libvirtError is the error we want to catch or not.
                # FIXME(hzwangpan): I just using the error code and specific
                # error message to check the error, this will not appropriate
                # if libvirtError changes later.
                if err_code == libvirt.VIR_ERR_SYSTEM_ERROR and \
                       "No such file or directory" in err_msg:
                    (removed, xml) = remove_invalid_dev(xml, err_msg)
                    if not removed:
                        retry = False
                        raise ex
                    LOG.info(_("retry to create instance when hard reboot"),
                                instance=instance)
                    retry = True
                    pass
                elif err_code == libvirt.VIR_ERR_OPERATION_INVALID and \
                       "domain is already running" in err_msg:
                    LOG.info(_("domain is already running"), instance=instance)
                    domain = None
                    retry = False
                    pass
                else:
    ...
    def _get_cpuset_ids(self):
        ...
        for rule in FLAGS.vcpu_pin_set.split(','):
            rule = rule.strip()
            # Handle multi ','
            if len(rule) < 1:
                continue
    ...
    def _get_vcpu_pin_set(self, total_pcpus):
        rule = FLAGS.vcpu_pin_set.replace(' ', '').replace('\t', '')
        if (rule.endswith('$') and rule.count('-') == 1 and
                rule.replace('-', '').replace('$', '').isdigit()):
            try:
                reserved_start = int(rule.rsplit('-', 1)[0])
            except ValueError:
                raise exception.Invalid(_("Invalid range expression %r")
                                        % rule)
            self._vcpu_total = total_pcpus - reserved_start
            if self._vcpu_total <= 0:
                raise exception.Invalid(_("Invalid vcpu_pin_set config, "
                                          "out of hypervisor cpu range."))
            else:
                self._vcpu_pin_set = rule.replace('$', str(total_pcpus - 1))
        else:
            raise exception.Invalid(_("Invalid range expression %r") % rule)
    ...
    def get_vcpu_total(self):
        ...
        """
        LOG.debug(_("begin to get vcpu total"))
        if self._vcpu_total != 0:
        ...
        if FLAGS.vcpu_pin_set is None or not FLAGS.enable_cpuqos:
            self._vcpu_total = total_pcpus
            return self._vcpu_total

        if '$' in FLAGS.vcpu_pin_set:
            self._get_vcpu_pin_set(total_pcpus)
        else:
            ...
            self._vcpu_pin_set = FLAGS.vcpu_pin_set
        return self._vcpu_total
    ...
    def get_memory_mb_total(self):
        ...
        """
        LOG.debug(_("begin to get memory mb total"))
    ...
    def get_local_gb_total():
        ...
        """
        LOG.debug(_("begin to get local gb total"))

        stats = libvirt_utils.get_fs_info(FLAGS.instances_path)
        LOG.debug(_("get local gb total end"))
    ...
    def get_vcpu_used(self):
        ...
        """
        LOG.debug(_("begin to get vcpu used"))
    ...
    def get_memory_mb_used(self):
        ...
        """
        LOG.debug(_("begin to get memory mb used"))
        ...
        if FLAGS.libvirt_type == 'xen':
            ...
            # Convert it to MB
            LOG.debug(_("get memory mb used end"))
    ...
    def get_local_gb_used(self):
        ...
        """
        LOG.debug(_("begin to get local gb used"))

        stats = libvirt_utils.get_fs_info(FLAGS.instances_path)
        LOG.debug(_("get local gb used end"))
    ...
    def get_hypervisor_type(self):
        ...
        """
        LOG.debug(_("begin to get hypervisor type"))
    ...
    def get_hypervisor_version(self):
        ...
        """
        LOG.debug(_("begin to get hypervisor version"))
    ...
    def get_hypervisor_hostname(self):
        ...
        LOG.debug(_("begin to get hypervisor hostname"))
    ...
    def get_instance_capabilities(self):
        ...
        """
        LOG.debug(_("begin to get instance capabilities"))
        caps = self.get_host_capabilities()
        instance_caps = list()
        ...
        LOG.debug(_("get instance capabilities end"))
    ...
    def get_cpu_info(self):
        ...
        """
        LOG.debug(_("begin to get cpu info"))
        ...
        if FLAGS.enable_cpuqos:
            cpu_info['ecu_ratio'] = FLAGS.ecu_ratio
    ...
    def get_available_resource(self):
        ...
        """
        LOG.debug(_("begin to get available resource"))
        ...
        LOG.debug(_("get available resource end"))
        return dic
    ...
    def get_disk_available_least(self):
        ...
        """
        LOG.debug(_("begin to get disk_available_least"))
        ...
        LOG.debug(_("get disk_available_least end"))
    ...
    def get_host_stats(self, refresh=False):
        ...
        LOG.debug(_("begin to get host stats in LibvirtDriver"))
    ...
    def migrate_disk_and_power_off(self, context, instance, dest,
        ...
        try:
            ...
            for info in disk_info:
                ...
                #when resize,ignore the ephemeral disk if keep_ephemeral=False
                if fname == "disk.local" and not keep_ephemeral:
                    LOG.debug(_("ignore disk.local if keep_ephemeral set to "
                                "False when resize"), instance=instance)
                    continue

                # when resize, we ignore the swap disk
                if fname == "disk.swap":
                    LOG.debug(_("ignore swap disk when resize"),
                              instance=instance)
                    continue
    ...
    def finish_migration(self, context, migration, instance, disk_info,
        ...
        for info in disk_info:
            ...
            if (size and fmt == 'qcow2' and
                    disk.can_resize_fs(info['path'], size, use_cow=True) and
                    not FLAGS.allow_image_snapshot_optimization):
...
class HostState(object):
    ...
    def get_host_stats(self, refresh=False):
        ...
        If 'refresh' is True, run update the stats first."""
        LOG.debug(_("begin to get host stats in HostState"))
        if refresh:
            self.update_status()
        LOG.debug(_("get host stats end in HostState"))
        return self._stats
    ...
    def update_status(self):
        ...
        LOG.debug(_("Updating host stats end"))
        return data
