...
class NWFilterFirewall(base_firewall.FirewallDriver):
    ...
    def setup_basic_filtering(self, instance, network_info):
        ...
        def check_metadata_key(instance, target_key):
            """ Check whether given instance's metadata contains the key """
            try:
                for meta in instance['metadata']:
                    if meta['key'].startswith(str(target_key)):
                        return True
            except KeyError, ex:
                LOG.warn(_('When checking "%(target_key)s"'
                           'Found KeyError %(ex)s. return False'),
                         locals(), instance=instance)
            return False

        if check_metadata_key(instance, 'netease-reserved'):
            # He is LVS instance and LVS-vip instance.
            base_filter = 'nova-lvs'

        if check_metadata_key(instance, 'netease-node'):
            # He is Node instance after LVS.
            base_filter = 'nova-lvs-node'

        LOG.debug(_('Set nwfilter to %(base_filter)s'), locals(),
                  instance=instance)
        for (network, mapping) in network_info:
    ...
    def _ensure_static_filters(self):
        ...
        # LVS need arp-spoofing, ip-spoofing
        self._define_filter(self._filter_container('nova-lvs',
                                                   ['no-mac-spoofing',
                                                    'allow-dhcp-server']))
        # LVS-node need ip-spoofing
        self._define_filter(self._filter_container('nova-lvs-node',
                                                   ['no-mac-spoofing',
                                                    'no-arp-spoofing',
                                                    'allow-dhcp-server']))
        self._define_filter(self._filter_container('nova-lvs',
