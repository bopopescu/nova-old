...
util_opts = [
    ...
    cfg.StrOpt('rsync_auth_user',
               default='nova',
               help='rsync auth user'),
    cfg.StrOpt('rsync_secret_file',
               default='/etc/nova.pas',
               help='rsync secret file'),
    cfg.IntOpt('rsync_port',
               default=10873,
               help='rsync server listen port')
    ]
...
def make_remote_instance_dir(src_dir, dest_host, dest_dir):
    """Make remote instance dir which relative to FLAGS.instances_path
    by using rsync '--dirs' option.

    :param src_dir: Instance dir path on source node, may be renamed
    :param dest_host: IP address of destination node to make dir
    :param dest_dir: Instance dir name to be made
    """
    dest = "%s@%s::nova/%s" % (FLAGS.rsync_auth_user, dest_host, dest_dir)
    try:
        execute('rsync', '--sparse', '--compress', '--dirs', '--dry-run',
                src_dir, dest, '--password-file=%s' % FLAGS.rsync_secret_file,
                '--port=%s' % FLAGS.rsync_port, run_as_root=True)
    except exception.ProcessExecutionError:
        raise
    else:
        execute('rsync', '--sparse', '--compress', '--dirs',
                src_dir, dest, '--password-file=%s' % FLAGS.rsync_secret_file,
                '--port=%s' % FLAGS.rsync_port, run_as_root=True)


def clean_remote_dir(src_dir, dest_host):
    """Clean remote instance dir which relative to FLAGS.instances_path
    by using rsync '--delete' option.
    """
    dest = "%s@%s::nova" % (FLAGS.rsync_auth_user, dest_host)
    include_dir = "--include=%s" % src_dir
    include_files = "--include=%s/*" % src_dir
    exclude_files = "--exclude=*"
    instances_path = FLAGS.instances_path
    if not instances_path.endswith('/'):
        instances_path = "%s/" % instances_path
    try:
        execute('rsync', '--delete', '--force', '--recursive', '--dry-run',
                include_dir, include_files, exclude_files, instances_path,
                dest, '--password-file=%s' % FLAGS.rsync_secret_file,
                '--port=%s' % FLAGS.rsync_port, run_as_root=True)
    except exception.ProcessExecutionError:
        raise
    else:
        execute('rsync', '--delete', '--force', '--recursive',
                include_dir, include_files, exclude_files, instances_path,
                dest, '--password-file=%s' % FLAGS.rsync_secret_file,
                '--port=%s' % FLAGS.rsync_port, run_as_root=True)


def copy_image_to_remote(src, dest, host):
    """Copy a disk image to an existing remote directory

    :param src: Source image
    :param dest: Destination path
    :param host: Remote host
    """
    dest = "%s@%s::nova/%s" % (FLAGS.rsync_auth_user, host, dest)
    try:
        execute('rsync', '--sparse', '--compress', '--dry-run', src, dest,
                '--password-file=%s' % FLAGS.rsync_secret_file,
                '--port=%s' % FLAGS.rsync_port, run_as_root=True)
    except exception.ProcessExecutionError:
        raise
    else:
...
# fetch original image, don't change it's type
def fetch_orig_image(context, target, image_id, user_id, project_id):
    images.fetch(context, image_id, target, user_id, project_id)


def rebase_cow_image(new_backing_file, cow_image):
    qemu_img_cmd = ('qemu-img',
                    'rebase',
                    '-b',
                    new_backing_file,
