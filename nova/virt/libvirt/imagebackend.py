...
__imagebackend_opts = [
    ...
    cfg.BoolOpt('libvirt_ephemeral_use_lvm',
            default=False,
            help='Create logical volume for ephemeral disk'
                 ' if this flag is set to True and'
                 ' libvirt_images_volume_group is not None'),
        ]
...
class Backend(object):
    ...
    def image(self, instance, name, image_type=None):
        ...
        if (FLAGS.libvirt_ephemeral_use_lvm and
            FLAGS.libvirt_images_volume_group is not None and
            name == 'disk.local'):
            image_type = 'lvm'
