...
class HostFiltersTestCase(test.TestCase):
    ...
    def test_network_filter_fails_on_private_bandwith(self):
        self._stub_service_is_up(True)
        filt_cls = self.class_map['NetworkFilter']()
        self.flags(use_network_qos=True)
        self.flags(network_qos_host_private_bandwidth=100)
        self.flags(reserved_host_network_private_bandwidth=50)
        self.flags(network_qos_private_allocation_ratio=1)
        self.flags(network_qos_host_public_bandwidth=60)
        self.flags(reserved_host_network_public_bandwidth=30)
        self.flags(network_qos_public_allocation_ratio=1)
        self.flags(network_qos_config='')
        filter_properties = {'private_network_bandwith': 40,
                             'public_network_bandwith': 5}
        capabilities = {'enabled': True}
        service = {'disabled': False}
        host = fakes.FakeHostState('host1', 'compute',
                            {'private_network_mbps_used': 20,
                             'public_network_mbps_used': 10,
                             'capabilities': capabilities, 'service': service})
        self.assertFalse(filt_cls.host_passes(host, filter_properties))

    def test_network_filter_fails_on_public_bandwith(self):
        self._stub_service_is_up(True)
        filt_cls = self.class_map['NetworkFilter']()
        self.flags(use_network_qos=True)
        self.flags(network_qos_host_private_bandwidth=100)
        ...
        self.flags(network_qos_public_allocation_ratio=1)
        self.flags(network_qos_config='')
        filter_properties = {'private_network_bandwith': 10,
                             'public_network_bandwith': 35}
        capabilities = {'enabled': True}
        service = {'disabled': False}
        host = fakes.FakeHostState('host1', 'compute',
                            {'private_network_mbps_used': 20,
    ...
    def test_network_filter_passes(self):
        self._stub_service_is_up(True)
        filt_cls = self.class_map['NetworkFilter']()
        self.flags(use_network_qos=True)
        self.flags(network_qos_config='')
        ...
        host = fakes.FakeHostState('host1', 'compute',
                {'private_network_mbps_used': 20,
                 'public_network_mbps_used': 10,
                 'total_private_network_mbps': 50,
                 'total_public_network_mbps': 30,
                 'capabilities': capabilities, 'service': service})
        self.assertTrue(filt_cls.host_passes(host, filter_properties))

    def test_network_filter_passes_all(self):
        self._stub_service_is_up(True)
        filt_cls = self.class_map['NetworkFilter']()
        self.flags(use_network_qos=False)
        capabilities = {'enabled': True}
        service = {'disabled': False}
    ...
    def test_ecu_filter_pass(self):
        filt_cls = self.class_map['EcuFilter']()
        host = fakes.FakeHostState('host1', 'compute', {"ecu_ratio": 2,
                                          "ecus": 20, "free_ecus": 4})
        instance_type = {"vcpus": 2, "extra_specs": {"ecus_per_vcpu:": "2"}}
        filter_properties = dict(instance_type=instance_type)
        self.assertTrue(filt_cls.host_passes(host, filter_properties))

    def test_ecu_filter_fail(self):
        filt_cls = self.class_map['EcuFilter']()
        host = fakes.FakeHostState('host1', 'compute', {"ecu_ratio": 2,
                                          "ecus": 20, "free_ecus": 2})
        instance_type = {"vcpus": 2, "extra_specs": {"ecus_per_vcpu:": "2"}}
    ...
    def test_ecu_filter_fails_on_not_support(self):
        filt_cls = self.class_map['EcuFilter']()
        host = fakes.FakeHostState('host1', 'compute', {"ecu_ratio": 2,
                                          "ecus": 20, "free_ecus": 4})
        instance_type = {"vcpus": 1, "extra_specs": {"ecus_per_vcpu:": "3"}}
        filter_properties = dict(instance_type=instance_type)
        self.assertFalse(filt_cls.host_passes(host, filter_properties))
