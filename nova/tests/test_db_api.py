...
class DbApiTestCase(test.TestCase):
    ...
    def test_instance_get_all_by_host_with_fixed_ip(self):
        def create_fixed_ip(context, params):
            default_params = {'address': '192.168.0.1'}
            default_params.update(params)
            return db.fixed_ip_create(context, default_params)

        args1 = {'host': 'test1', 'uuid': '1111', "deleted": False}
        args2 = {'host': 'test1', 'uuid': '2222', "deleted": False}
        args3 = {'host': 'test1', 'uuid': '3333', "deleted": True}
        args4 = {'host': 'test1', 'uuid': '4444', "deleted": False}
        args5 = {'host': 'test2', 'uuid': '5555', "deleted": False}

        instance = []
        instance.append(db.instance_create(self.context, args1))
        instance.append(db.instance_create(self.context, args2))
        instance.append(db.instance_create(self.context, args3))
        instance.append(db.instance_create(self.context, args4))
        instance.append(db.instance_create(self.context, args5))

        fips = []
        fips.append({'instance_uuid': instance[0]['uuid'], "deleted": False})
        fips.append({'instance_uuid': instance[1]['uuid'], "deleted": True})
        fips.append({'instance_uuid': instance[1]['uuid'], "deleted": False})
        fips.append({'instance_uuid': instance[1]['uuid'], "deleted": False})
        fips.append({'instance_uuid': instance[2]['uuid'], "deleted": False})
        fips.append({'instance_uuid': instance[2]['uuid'], "deleted": False})
        fips.append({'instance_uuid': instance[3]['uuid'], "deleted": True})
        fips.append({'instance_uuid': instance[3]['uuid'], "deleted": False})
        fips.append({'instance_uuid': instance[4]['uuid'], "deleted": False})

        ctxt = context.get_admin_context()

        for fip in fips:
            create_fixed_ip(ctxt, fip)

        result = db.instance_get_all_by_host_with_fixed_ip(ctxt, "test1")
        self.assertTrue(4, len(result))

        result = db.instance_get_all_by_host_with_fixed_ip(ctxt, "test2")
...
class TestFloatingIP(test.TestCase):

    def setUp(self):
        super(TestFloatingIP, self).setUp()
        self.context = context.get_admin_context()
        self.instance = self._create_instances_with_args()
        self.network = db.network_create_safe(self.context, {})

    def _create_instances_with_args(self, **kwargs):
        args = {'reservation_id': 'a', 'image_ref': 1, 'host': 'host1',
                'project_id': 'fake_project_id'}
        args.update(kwargs)
        return db.instance_create(self.context, args)

    def _create_floating_ip(self, **params):
        default_params = {}
        default_params.update(params)
        return db.floating_ip_create(self.context, default_params)

    def test_floating_ip_allocate_address(self):
        address = self._create_floating_ip(pool='nova', address='10.0.0.1')
        self.assertEqual(address, '10.0.0.1')

        project_id = self.instance.project_id
        pool = 'nova'
        result = db.floating_ip_allocate_address(self.context, project_id,
                                                 pool, assign_address=None)
        self.assertEqual(result, '10.0.0.1')

    def test_floating_ip_allocate_address_with_assign_address(self):
        address = self._create_floating_ip(pool='nova', address='10.0.0.1')
        self.assertEqual(address, '10.0.0.1')

        project_id = self.instance.project_id
        pool = 'nova'
        assign_address = '10.0.0.1'
        result = db.floating_ip_allocate_address(self.context, project_id,
                                                 pool, assign_address)
        self.assertEqual(result, assign_address)
...
class TestIpAllocation(test.TestCase):
    ...
    def test_fixed_ip_associate_pool_in_subnet_by_subnet_cidr_no_fault(self):
        ctxt = context.get_admin_context()
        values = {'host': 'foo', 'hostname': 'myname'}
        instance = db.instance_create(ctxt, values)
        values = {
        ...
        network = db.network_create_safe(ctxt, values)
        values = [
                  {'address': '192.168.1.1', 'network_id': network['id']},
                  {'address': '192.168.1.100', 'network_id': network['id']},
                  {'address': '10.0.233.10', 'network_id': network['id']},
                  {'address': '10.0.233.90', 'network_id': network['id']},
                  {'address': '10.0.233.100', 'network_id': 123456789},
                  {'address': '10.0.233.110', 'network_id': network['id']},
                  {'address': '10.0.233.200', 'network_id': network['id']},
                  {'address': '10.0.234.1', 'network_id': network['id']},
                  {'address': '10.0.234.100', 'network_id': network['id']}
                 ]
        for v in values:
            self.create_fixed_ip(address=v['address'],
                                 network_id=v['network_id'])

        # in this case get 10.0.233.110
        fixed_address = db.fixed_ip_associate_pool_in_subnet(ctxt,
                                        network_id=network['id'],
                                        subnet_cidr='10.0.233.0/24',
                                        start_ip='10.0.233.100',
                                        instance_uuid=instance['uuid'])

        fixed_ip = db.fixed_ip_get_by_address(ctxt, fixed_address)
        self.assertEqual(fixed_ip['instance_uuid'], instance['uuid'])

        # in this case get 10.0.233.200
        fixed_address = db.fixed_ip_associate_pool_in_subnet(ctxt,
                                        network_id=network['id'],
                                        subnet_cidr='10.0.233.0/24',
                                        start_ip='10.0.233.100',
        ...
        # in this case next fixed ip should be 10.0.234.1 but
        # not in subnet_cidr, raise exception.NoMoreFixedIps()
        self.assertRaises(exception.NoMoreFixedIps,
                          db.fixed_ip_associate_pool_in_subnet,
                          ctxt, network['id'], '10.0.233.0/24',
                          '10.0.233.100', instance['uuid'])

    def test_fixed_ip_associate_pool_in_subnet_fails_if_subnet_wrong(self):
        ctxt = context.get_admin_context()
        values = {'host': 'foo', 'hostname': 'myname'}
        instance = db.instance_create(ctxt, values)
        values = {
        ...
        network = db.network_create_safe(ctxt, values)
        values = [
                  {'address': '192.168.1.1', 'network_id': 123456789},
                  {'address': '192.168.1.100', 'network_id': 123456789},
                  {'address': '10.0.233.10', 'network_id': network['id']},
                  {'address': '10.0.233.90', 'network_id': network['id']},
                  {'address': '10.0.233.100', 'network_id': 123456789},
                  {'address': '10.0.233.110', 'network_id': network['id']},
        ...
        self.assertRaises(exception.NoMoreFixedIps,
                          db.fixed_ip_associate_pool_in_subnet,
                          ctxt, network['id'], '192.168.1.0/24',
                          '192.168.1.100', instance['uuid'])

    def test_fixed_ip_associate_pool_in_subnet_fails_if_start_ip_wrong(self):
        ctxt = context.get_admin_context()
        values = {'host': 'foo', 'hostname': 'myname'}
        instance = db.instance_create(ctxt, values)
        values = {
        ...
        self.assertRaises(exception.FixedIpNotFoundForAddress,
                          db.fixed_ip_associate_pool_in_subnet,
                          ctxt, network['id'], '10.0.163.0/24',
                          '10.0.163.50', instance['uuid'])
