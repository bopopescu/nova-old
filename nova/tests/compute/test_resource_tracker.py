...
class ResourceTestCase(BaseTestCase):
    ...
    def test_sync_instance_network_bandwith(self):
        instance = self._fake_instance()

        def fake_get_instance_network_qos(inst_type, inst_uuid):
            return {'private_qos': 40, 'public_qos': 5}

        self.stubs.Set(resource_tracker, 'get_instance_network_qos',
                       fake_get_instance_network_qos)
        inst_type = {'vcpus': 1,
                     'extra_specs': {'ecus_per_vcpu:': 2}}
        self.tracker._sync_instance_network_bandwith(instance, inst_type)
        self.assertEqual(40, instance['private_network_bandwith'])
        self.assertEqual(5, instance['public_network_bandwith'])

    def test_get_instance_network_qos_failed(self):
        inst_type = {'vcpus': 1,
                     'extra_specs': {'ecus_per_vcpu:': 2}}
        inst_uuid = 'fake_uuid'
        qos_info = {'private_qos': 0, 'public_qos': 0}

        def fake_load(str):
            return None

        def fake_instance_system_metadata_get(context, instance_uuid):
            return {'network-qos': None}

        with utils.tempdir() as d:
            fname = os.path.join(d, 'test.conf')
            self.flags(network_qos_config=fname)
            os.system('touch %s' % fname)
            self.stubs.Set(jsonutils, 'load', fake_load)
            self.stubs.Set(db, 'instance_system_metadata_get',
                                fake_instance_system_metadata_get)
            self.assertEqual(qos_info,
                    resource_tracker.get_instance_network_qos(inst_type,
                                                              inst_uuid))

    def test_get_instance_network_qos_from_default_configure(self):
        inst_type = {'vcpus': 1,
                     'extra_specs': {'ecus_per_vcpu:': 2}}
        inst_uuid = 'fake_uuid'
        qos_info = {'policy': {'key': 'ecu'},
                    'shaping': {'public': {'default': {'rate': 5}},
                                'private': {'default': {'rate': 20},
                                            '2': {'rate': 40}}}}
        default_qos = dict(private_qos=40, public_qos=5)
        ...
        with utils.tempdir() as d:
            ...
            self.stubs.Set(db, 'instance_system_metadata_get',
                                fake_instance_system_metadata_get)
            self.assertEqual(default_qos,
                    resource_tracker.get_instance_network_qos(inst_type,
                                                              inst_uuid))

    def test_get_instance_network_qos_from_system_meta(self):
        inst_uuid = 'fake_uuid'
        inst_type = 'fake_type'
        fake_qos = [{"type": "private", "rate": "35"},
                    {"type": "public", "rate": "75"}]
        net_qos = dict(private_qos=35, public_qos=75)

        def fake_instance_system_metadata_get(context, instance_uuid):
            return {'network-qos': jsonutils.dumps(fake_qos)}
