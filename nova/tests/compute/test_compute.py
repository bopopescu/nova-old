...
class BaseTestCase(test.TestCase):
    ...
    def setUp(self):
        ...
        self.flags(compute_driver='nova.virt.fake.FakeDriver',
                   ...
                   use_instance_storage_qos=False,
                   blkio_cgroup_path='/sys/fs/cgroup/blkio/',
                   instance_storage_devices=[{'disk': '/dev/sda',
                                              'number': '254:0'}])
        ...
        def fake_ensure_rsync_daemon():
            pass

        def fake_set_host_blkio():
            pass
        ...
        self.stubs.Set(self.compute, '_ensure_rsync_daemon',
                fake_ensure_rsync_daemon)
        self.stubs.Set(self.compute, '_set_host_blkio',
                fake_set_host_blkio)
        ...
        for name, value in all_types.iteritems():
            ...
            db.instance_type_extra_specs_update_or_create(self.context,
                                                    inst_type["flavorid"],
                                                    ext_spec)
    ...
    def _create_group(self, group_name='testgroup'):
...
class ComputeTestCase(BaseTestCase):
    ...
    def test_set_host_blkio(self):
        def fake_execute(*cmd, **kwargs):
            cmd1 = ('tee', '/sys/fs/cgroup/blkio/blkio.weight_device')
            cmd2 = ('tee',
            ...
            cmd_ok = (cmd == cmd1 or cmd == cmd2 or cmd == cmd3 or cmd == cmd4
                      or cmd == cmd5)
            self.assertEquals(cmd_ok, True)
            self.assertEquals(kwargs.pop('process_input', None), '254:0 0\n')
            self.assertEquals(kwargs.pop('run_as_root', False), True)

        self.stubs.Set(utils, 'execute', fake_execute)
        self.compute._set_host_blkio()
    ...
    def test_finish_revert_resize(self):
        ...
        def fake_finish_revert_migration_driver(*args, **kwargs):
            ...
            inst = args[0]
            instance_type = \
                    instance_types.get_instance_type(inst['instance_type_id'])
            self.assertEqual(instance_type['flavorid'], '1')
        ...
        def fake_setup_networks_on_host(ctxt, instance, host):
    ...
    def test_attach_nbs_volume(self):
        device = {"slot": 31,
                  "mountpoint": "/dev/vdd",
                  "real_path": "/dev/nbs/xdbf"}
        instance = self._create_fake_instance()

        def fake_get_host_ip_by_ifname(ifname):
            return "127.0.0.1"

        def fake_nbs_attach(self, *args, **kwargs):
            return {"requestId": "fake-nbs-request-id"}

        def fake_wait_for_attached(self, *args, **kwargs):
            return True

        def fake_get_host_dev_and_qos_info(*args, **kwargs):
            return ("/dev/nbs/fake_dev_path",
                    {"maxReadBandWidth": 1234, "maxReadIOPS": 5678,
                     "maxWriteBandWidth": 4321, "maxWriteIOPS": 8765})

        def fake_driver_attach_nbs_volume(self, *args, **kwargs):
            return (False, None)

        def fake_notify_nbs_libvirt_result(*args, **kwargs):
            self.assertEqual(args[4], True)

        def fake_block_device_mapping_update_or_create(*args, **kwargs):
            self.assertEqual(args[1]['device_name'], jsonutils.dumps(device))
            self.assertEqual(args[1]['instance_uuid'], instance['uuid'])

        self.stubs.Set(self.compute, '_get_host_ip_by_ifname',
                        fake_get_host_ip_by_ifname)
        self.stubs.Set(nova.compute.nbs_client.API, 'attach', fake_nbs_attach)
        self.stubs.Set(nova.compute.nbs_client.API, 'wait_for_attached',
                        fake_wait_for_attached)
        self.stubs.Set(nova.compute.nbs_client.API,
                'get_host_dev_and_qos_info', fake_get_host_dev_and_qos_info)
        self.stubs.Set(nova.virt.fake.FakeDriver, 'attach_nbs_volume',
                        fake_driver_attach_nbs_volume)
        self.stubs.Set(nova.compute.nbs_client.API,
                'notify_nbs_libvirt_result', fake_notify_nbs_libvirt_result)
        self.stubs.Set(db, 'block_device_mapping_update_or_create',
                        fake_block_device_mapping_update_or_create)

        self.compute.attach_nbs_volume(self.context, 1, device, instance)
        db.instance_destroy(self.context, instance['uuid'])

    def test_attach_nbs_volume_attach_error(self):
        device = {"slot": 31,
                  "mountpoint": "/dev/vdd",
                  "real_path": "/dev/nbs/xdbf"}
        instance = self._create_fake_instance()
        ...
        def fake_nbs_attach_error(self, *args, **kwargs):
            return None

        def fake_notify_nbs_libvirt_result(*args, **kwargs):
            self.assertEqual(args[4], False)

        def fake_bdm_destroy_by_instance_and_device(*args, **kwargs):
            self.assertEqual(args[1], instance['uuid'])
            self.assertEqual(args[2], jsonutils.dumps(device))

        self.stubs.Set(self.compute, '_get_host_ip_by_ifname',
                        fake_get_host_ip_by_ifname)
        self.stubs.Set(nova.compute.nbs_client.API,
                        'attach', fake_nbs_attach_error)
        self.stubs.Set(nova.compute.nbs_client.API,
                'notify_nbs_libvirt_result', fake_notify_nbs_libvirt_result)
        self.stubs.Set(db,
                       'block_device_mapping_destroy_by_instance_and_device',
                       fake_bdm_destroy_by_instance_and_device)

        self.assertRaises(exception.NbsException,
                          self.compute.attach_nbs_volume,
                          self.context, 1, device, instance)
        db.instance_destroy(self.context, instance['uuid'])

    def test_attach_nbs_volume_device_error(self):
        device1 = {"slot": 31,
                  "mountpoint": "/dev/vdd",
                  "real_path": "/dev/nbs/xdbf"}
        device2 = {"slot": 30,
        ...
        def fake_driver_attach_nbs_volume(self, *args, **kwargs):
            if args[2]['slot'] == device1['slot']:
                return (True, {'dev': device1['mountpoint'],
                               'slot': device1['slot']})
            else:
                return (False, None)

        def fake_driver_get_device_for_nbs_volume(self, *args, **kwargs):
            return device2
        ...
        def fake_block_device_mapping_update_or_create(*args, **kwargs):
            self.assertEqual(args[1]['device_name'], jsonutils.dumps(device2))
            self.assertEqual(args[1]['instance_uuid'], instance['uuid'])
        ...
        self.stubs.Set(nova.virt.fake.FakeDriver, 'attach_nbs_volume',
                        fake_driver_attach_nbs_volume)
        self.stubs.Set(nova.virt.fake.FakeDriver, 'get_device_for_nbs_volume',
                        fake_driver_get_device_for_nbs_volume)
        self.stubs.Set(nova.compute.nbs_client.API,
                'notify_nbs_libvirt_result', fake_notify_nbs_libvirt_result)
        self.stubs.Set(db, 'block_device_mapping_update_or_create',
                        fake_block_device_mapping_update_or_create)

        self.compute.attach_nbs_volume(self.context, 1, device1, instance)
        db.instance_destroy(self.context, instance['uuid'])
    ...
    def test_detach_nbs_volume(self):
        ...
        def fake_get_instance_volume_bdm(*args, **kwargs):
            return {'volume_id': 1, 'device_name': jsonutils.dumps(device),
                    'updated_at': datetime.datetime(2000, 10, 10, 12, 0, 0)}

        def fake_is_older_than(*args, **kwargs):
            return True

        def fake_get_instance_volume_bdm_return_none(*args, **kwargs):
            return None

        def fake_driver_detach_nbs_volume(*args, **kwargs):
            return

        def fake_bdm_destroy_by_instance_and_volume(*args, **kwargs):
            self.assertEqual(args[1], instance['uuid'])
        ...
        def fake_nbs_detach(self, *args, **kwargs):
            return {"requestId": "fake-nbs-request-id"}

        self.flags(attach_detach_interval=0)
        self.stubs.Set(self.compute, '_get_instance_volume_bdm',
                        fake_get_instance_volume_bdm)
        self.stubs.Set(nova.openstack.common.timeutils, 'is_older_than',
                        fake_is_older_than)
        self.stubs.Set(nova.virt.fake.FakeDriver, 'detach_nbs_volume',
                        fake_driver_detach_nbs_volume)
        self.stubs.Set(self.compute, '_get_host_ip_by_ifname',
                        fake_get_host_ip_by_ifname)
        self.stubs.Set(nova.compute.nbs_client.API, 'detach', fake_nbs_detach)
        self.stubs.Set(db,
                        'block_device_mapping_destroy_by_instance_and_volume',
                        fake_bdm_destroy_by_instance_and_volume)

        self.compute.detach_nbs_volume(self.context, 1, instance)

        self.stubs.Set(self.compute, '_get_instance_volume_bdm',
                        fake_get_instance_volume_bdm_return_none)
        self.compute.detach_nbs_volume(self.context, 1, instance)
        db.instance_destroy(self.context, instance['uuid'])

    def test_detach_nbs_volume_driver_error(self):
        instance = self._create_fake_instance()
        device = {"slot": 31,
                  "mountpoint": "/dev/vdd",
                  "real_path": "/dev/nbs/xdbf"}
        called = {"fake_bdm_destroy_by_instance_and_volume": False}
        ...
        def fake_bdm_destroy_by_instance_and_volume(*args, **kwargs):
            # this method should be called even libvirt raises error
            called["fake_bdm_destroy_by_instance_and_volume"] = True
        ...
        self.compute.detach_nbs_volume(self.context, 1, instance)
        self.assertTrue(called["fake_bdm_destroy_by_instance_and_volume"])
        db.instance_destroy(self.context, instance['uuid'])

    def test_extend_nbs_volume(self):
        device = {"slot": 31,
                  "mountpoint": "/dev/vdd",
                  "real_path": "/dev/nbs/xdbf"}
        ...
        def fake_nbs_get(self, *args, **kwargs):
            return {'volumes': [{'status': 'in-use', 'size': 3}]}

        def fake_get_host_ip_by_ifname(ifname):
            return "127.0.0.1"

        def fake_nbs_extend(self, *args, **kwargs):
            return {"requestId": "fake-nbs-request-id"}

        def fake_wait_for_extended(self, *args, **kwargs):
            return True

        def fake_driver_extend_nbs_volume(*args, **kwargs):
            return
        ...
        self.stubs.Set(self.compute, '_get_instance_volume_bdm',
                        fake_get_instance_volume_bdm)
        self.stubs.Set(nova.virt.fake.FakeDriver, 'extend_nbs_volume',
                        fake_driver_extend_nbs_volume)
        self.stubs.Set(self.compute, '_get_host_ip_by_ifname',
                        fake_get_host_ip_by_ifname)
        self.stubs.Set(nova.compute.nbs_client.API, 'get', fake_nbs_get)
        self.stubs.Set(nova.compute.nbs_client.API, 'extend', fake_nbs_extend)
        self.stubs.Set(nova.compute.nbs_client.API,
                        'wait_for_extended', fake_wait_for_extended)
        self.stubs.Set(nova.compute.nbs_client.API,
                'get_host_dev_and_qos_info', fake_get_host_dev_and_qos_info)
        self.stubs.Set(self.compute, 'update_nbs_qos', fake_update_nbs_qos)
        self.stubs.Set(nova.compute.nbs_client.API,
                'notify_nbs_libvirt_result', fake_notify_nbs_libvirt_result)

        instance = self._create_fake_instance()
        self.compute.extend_nbs_volume(self.context, instance, 1, 1, None)
        db.instance_destroy(self.context, instance['uuid'])

    def test_update_nbs_qos(self):
        device = {"slot": 31,
                  "mountpoint": "/dev/vdd",
                  "real_path": "/dev/nbs/xdbf"}
        ...
        def fake_get_instance_volume_bdm(*args, **kwargs):
            return {'volume_id': 1, 'device_name': jsonutils.dumps(device),
                    'connection_info': '{"host_dev": "/dev/mapper/fake-nbs", '
                        '"qos_info": {"iotune_total_bytes": null, '
                            '"iotune_total_bytes": 12, '
                            '"iotune_total_iops": 21}}'}

        def fake_notify_nbs_qos_updated(self, *args, **kwargs):
            return {"requestId": "fake-nbs-request-id"}

        def fake_driver_update_nbs_qos(*args, **kwargs):
            return

        def fake_block_device_mapping_update_or_create(*args, **kwargs):
            conn_info = jsonutils.loads(args[1]['connection_info'])
            self.assertEqual(conn_info["qos_info"]["iotune_total_bytes"], 123)
            self.assertEqual(conn_info["qos_info"]["iotune_total_iops"], 21)

        self.stubs.Set(self.compute, '_get_instance_volume_bdm',
                        fake_get_instance_volume_bdm)
        self.stubs.Set(nova.virt.fake.FakeDriver, 'update_nbs_qos',
                        fake_driver_update_nbs_qos)
        self.stubs.Set(nova.compute.nbs_client.API,
                        'notify_nbs_qos_updated', fake_notify_nbs_qos_updated)
        ...
        instance = self._create_fake_instance()
        self.compute.update_nbs_qos(self.context, instance, 1, qos_info)
        db.instance_destroy(self.context, instance['uuid'])
...
class ComputeAPITestCase(BaseTestCase):
    ...
    def test_delete_in_resizing(self):
        def fake_quotas_reserve(context, expire=None, project_id=None,
            ...
            old_type_ecus = old_type['vcpus'] * \
                                int(old_type['extra_specs']['ecus_per_vcpu:'])
            self.assertEqual(deltas['ecus'], -old_type_ecus)
    ...
    def test_resize_request_spec(self):
        def _fake_cast(context, topic, msg):
            ...
            # NOTE(wangpan): the instance_type in request_spec was got from
            # db, and 'extra_specs': [{..., 'key': u'ecus_per_vcpu:', ...}]
            # was added, so we discard this key here.
    ...
    def test_attach_nbs_volume(self):
        ...
        def fake_nbs_get(self, *args, **kwargs):
            called['fake_nbs_get'] = True
            return {'volumes': [{'status': 'available'}]}

        def fake_get_device_for_nbs_volume(self, *args, **kwargs):
            called['fake_get_device_for_nbs_volume'] = True
            return {"slot": 31, "mountpoint": "/dev/vdd",
                    "real_path": "/dev/nbs/xdbf"}

        def fake_rpc_attach_nbs_volume(self, context, **kwargs):
            called['fake_rpc_attach_nbs_volume'] = True

        def fake_db_bdma_get_all_by_volume(self, *args, **kwargs):
            called['fake_db_bdma_get_all_by_volume'] = True
            return []

        def fake_db_bdm_update_or_create(self, *args, **kwargs):
            called['fake_db_bdm_update_or_create'] = True

        def fake_check_allow_attach(*args, **kwargs):
            return True

        self.stubs.Set(nova.compute.nbs_client.API, 'get', fake_nbs_get)
        self.stubs.Set(compute_rpcapi.ComputeAPI, 'get_device_for_nbs_volume',
                       fake_get_device_for_nbs_volume)
        self.stubs.Set(compute_rpcapi.ComputeAPI, 'attach_nbs_volume',
                       fake_rpc_attach_nbs_volume)
        self.stubs.Set(db, 'block_device_mapping_get_all_by_volume',
                       fake_db_bdma_get_all_by_volume)
        self.stubs.Set(db, 'block_device_mapping_update_or_create',
                       fake_db_bdm_update_or_create)
        self.stubs.Set(self.compute_api, 'check_allow_attach',
                       fake_check_allow_attach)

        instance = self._create_fake_instance()
        self.compute_api.attach_nbs_volume(self.context, instance, 1)

        self.assertTrue(called.get('fake_nbs_get'))
        self.assertTrue(called.get('fake_get_device_for_nbs_volume'))
        self.assertTrue(called.get('fake_rpc_attach_nbs_volume'))
        self.assertTrue(called.get('fake_db_bdma_get_all_by_volume'))
        self.assertTrue(called.get('fake_db_bdm_update_or_create'))
        db.instance_destroy(self.context, instance['uuid'])

    def test_attach_nbs_volume_inst_type_not_allow_attach(self):

        def fake_get_instance_type(*args, **kwargs):
            return {"extra_specs": {}}

        self.stubs.Set(self.compute_api, 'get_instance_type',
                       fake_get_instance_type)

        instance = self._create_fake_instance()
        self.assertRaises(exception.NbsAttachForbidden,
                self.compute_api.attach_nbs_volume, self.context, instance, 1)
        db.instance_destroy(self.context, instance['uuid'])

    def test_attach_nbs_volume_host_not_allow_attach_null(self):

        def fake_get_instance_type(*args, **kwargs):
            return {"extra_specs": {"nbs": "true"}}

        def fake_aggregate_metadata_get_by_host(*args, **kwargs):
            return {}

        self.stubs.Set(self.compute_api, 'get_instance_type',
                       fake_get_instance_type)
        self.stubs.Set(db, 'aggregate_metadata_get_by_host',
                       fake_aggregate_metadata_get_by_host)
    ...
    def test_attach_nbs_volume_host_not_allow_attach_set(self):
    ...
    def test_attach_nbs_volume_no_free_device(self):
        ...
        instance = self._create_fake_instance()
        self.assertRaises(exception.NoFreeDevice,
                        self.compute_api.attach_nbs_volume,
                        self.context, instance, 1)
        self.assertTrue(called.get('fake_nbs_get'))
        self.assertTrue(called.get('fake_get_device_for_nbs_volume'))
        db.instance_destroy(self.context, instance['uuid'])

    def test_attach_nbs_volume_get_error(self):
        called = {}

        def fake_nbs_get_None(self, *args, **kwargs):
            called['fake_nbs_get_None'] = True
            return None

        def fake_nbs_get_error_status(self, *args, **kwargs):
            called['fake_nbs_get_error_status'] = True
            return {'volumes': [{'status': 'in-use'}]}

        def fake_check_allow_attach(*args, **kwargs):
            return True

        self.stubs.Set(nova.compute.nbs_client.API, 'get', fake_nbs_get_None)
        self.stubs.Set(self.compute_api, 'check_allow_attach',
                               fake_check_allow_attach)
        ...
        self.assertRaises(exception.VolumeNotFound,
                        self.compute_api.attach_nbs_volume,
                        self.context, instance, 1)
        self.assertTrue(called.get('fake_nbs_get_None'))

        self.stubs.Set(nova.compute.nbs_client.API,
                        'get', fake_nbs_get_error_status)
        self.assertRaises(exception.InvalidVolume,
                        self.compute_api.attach_nbs_volume,
                        self.context, instance, 1)
        self.assertTrue(called.get('fake_nbs_get_error_status'))
        db.instance_destroy(self.context, instance['uuid'])

    def test_attach_nbs_volume_race_condition(self):
        called = {}
        ...
        def fake_db_bdma_get_all_by_volume(self, *args, **kwargs):
            called['fake_db_bdma_get_all_by_volume'] = True
            return [{'volume_id': 1, 'instance_uuid': 'fake-uuid-123'}]

        def fake_db_bdm_destroy_by_inst_and_dev(self, *args, **kwargs):
            called['fake_db_bdm_destroy_by_inst_and_dev'] = True
        ...
        self.stubs.Set(db,
                    'block_device_mapping_destroy_by_instance_and_device',
                    fake_db_bdm_destroy_by_inst_and_dev)
        self.stubs.Set(self.compute_api, 'check_allow_attach',
                               fake_check_allow_attach)
        ...
        self.assertTrue(called.get('fake_get_device_for_nbs_volume'))
        self.assertTrue(called.get('fake_db_bdma_get_all_by_volume'))
        self.assertTrue(called.get('fake_db_bdm_destroy_by_inst_and_dev'))
        db.instance_destroy(self.context, instance['uuid'])

    def test_check_allow_attach_ok(self):

        def fake_get_instance_type(*args, **kwargs):
            return {"extra_specs": {"nbs": "true"}}

        def fake_aggregate_metadata_get_by_host(*args, **kwargs):
            return {"nbs": set([u'false', u'true', u'f'])}
        ...
        instance = {"instance_type_id": 1, "host": "fake-host"}
        self.assertTrue(
                self.compute_api.check_allow_attach(self.context, instance))
    ...
    def test_detach_nbs_volume(self):
        ...
        def fake_nbs_get_attached_in_vm(self, *args, **kwargs):
            called['fake_nbs_get_attached_in_vm'] = True
            return {'volumes': [
                        {'status': 'in-use',
                         'attachments': [
        ...
        def fake_nbs_get_attached(self, *args, **kwargs):
            called['fake_nbs_get_attached'] = True
            return {'volumes': [
                        {'status': 'in-use',
                         'attachments': [
                            {'instanceId': instance['uuid'],
        ...
        def fake_rpc_detach_nbs_volume(self, context, **kwargs):
            called['fake_rpc_detach_nbs_volume'] = True

        self.stubs.Set(nova.compute.nbs_client.API,
                        'get', fake_nbs_get_attached_in_vm)
        self.stubs.Set(compute_rpcapi.ComputeAPI, 'detach_nbs_volume',
                       fake_rpc_detach_nbs_volume)

        self.compute_api.detach_nbs_volume(self.context, instance, 1)

        self.assertTrue(called.get('fake_nbs_get_attached_in_vm'))
        self.assertTrue(called.get('fake_rpc_detach_nbs_volume'))

        self.stubs.Set(nova.compute.nbs_client.API,
                        'get', fake_nbs_get_attached)
        self.compute_api.detach_nbs_volume(self.context, instance, 1)

        self.assertTrue(called.get('fake_nbs_get_attached'))
        self.assertTrue(called.get('fake_rpc_detach_nbs_volume'))
        db.instance_destroy(self.context, instance['uuid'])
    ...
    def test_detach_nbs_volume_not_attached(self):
        ...
        def fake_nbs_get_not_attached(self, *args, **kwargs):
            called['fake_nbs_get_not_attached'] = True
            return {'volumes': [
                        {'status': 'available',
                         'attachments': []}]}

        self.stubs.Set(nova.compute.nbs_client.API,
                        'get', fake_nbs_get_not_attached)

        instance = self._create_fake_instance()
        self.assertRaises(exception.InvalidVolume,
                        self.compute_api.detach_nbs_volume,
                        self.context, instance, 1)

        self.assertTrue(called.get('fake_nbs_get_not_attached'))
        db.instance_destroy(self.context, instance['uuid'])

    def test_detach_nbs_volume_attached_other_inst(self):

        called = {}

        def fake_nbs_get_attached_other_inst(self, *args, **kwargs):
            called['fake_nbs_get_attached_other_inst'] = True
            return {'volumes': [
                        {'status': 'available',
                         'attachments': [
                            {'instanceId': 'fake-nbs-vol-attached-inst',
                             'status': 'attached'}]}]}

        self.stubs.Set(nova.compute.nbs_client.API,
                        'get', fake_nbs_get_attached_other_inst)
        ...
        self.assertTrue(called.get('fake_nbs_get_attached_other_inst'))
        db.instance_destroy(self.context, instance['uuid'])
    ...
    def test_ensure_security_groups(self):
        """ Test 'ensure_to_instance' in compute/api.py """
        instance = self._create_fake_instance()

        self.compute.run_instance(self.context,
                instance=jsonutils.to_primitive(instance))
        instance = self.compute_api.get(self.context, instance['uuid'])
        security_group_a = self._create_group('secA')['name']
        security_group_b = self._create_group('secB')['name']
        security_group_c = self._create_group('secC')['name']

        self.security_group_api.add_to_instance(self.context,
                                            instance, security_group_a)
        self.security_group_api.add_to_instance(self.context,
                                            instance, security_group_b)

        # update instance with new security group properties
        instance = self.compute_api.get(self.context, instance['uuid'])

        # delete group_b, and add group_c
        target_groups = [security_group_a, security_group_c]
        self.security_group_api.ensure_to_instance(self.context,
                                                   instance, target_groups)
        instance = self.compute_api.get(self.context, instance['uuid'])

        result = [sec['name'] for sec in instance['security_groups']]
        self.assertEqual(target_groups, result)
    ...
    def test_describe_availability_zones(self):
        def build_service_obj(az, host, service, disabled=False):
            service_obj = models.Service()
            service_obj.topic = service
            service_obj.availability_zone = az
            service_obj.host = host
            service_obj.binary = service
            service_obj.disabled = disabled
            return service_obj

        def fake_db_service_get_all(obj, context, **kwargs):
            return [build_service_obj("nova", "host1", "compute"),
                    build_service_obj("nova", "host1", "network"),
                    build_service_obj("nova", "host2", "compute", True),
                    build_service_obj("nova", "host2", "network"),
                    build_service_obj("nova1", "host3", "compute", True),
                    build_service_obj("nova1", "host3", "network")]

        self.stubs.Set(db, "service_get_all", fake_db_service_get_all)
        result = self.compute_api.describe_availability_zones(self.context)
        expected = {'availability_zones':
                    [
                     {
        ...
        context = self.context.elevated()
        result = self.compute_api.describe_availability_zones(context)
        expected = {'availability_zones':
                    [
                     {
                      'zoneState': 'not available',
...
class ComputeAggrTestCase(BaseTestCase):
    ...
    def test_add_aggregate_host_passes_subordinate_info_to_driver(self):
        ...
        self.compute.add_aggregate_host(self.context, self.aggr.id,
            "the_host", subordinate_info="SLAVE_INFO")
    ...
    def test_remove_from_aggregate_passes_subordinate_info_to_driver(self):
        ...
        self.compute.remove_aggregate_host(self.context,
            self.aggr.id, "the_host", subordinate_info="SLAVE_INFO")
...
class ComputeReschedulingExceptionTestCase(BaseTestCase):
    ...
    def test_no_reschedule_on_unexpected_task_state(self):
        ...
        self.assertRaises(exception.UnexpectedTaskStateError,
                ...
                None, False, self.fake_instance)


class ComputeRelocateTestCase(BaseTestCase):
    """ Tests for relocate logic """

    def test_relocate_work_correctly(self):
        old_host = 'foo'
        called = {'old_host': False}

        def fake_call(ctxt, topic, msg, *args, **kwargs):
            if topic == 'compute.foo':
                called['old_host'] = True
        ...
        self.compute.relocate_instance(self.context, instance=instance,
                                       image=image_ref)

        self.assertTrue(called['old_host'])

        new_inst = db.instance_get_by_uuid(self.context, instance['uuid'])
        self.assertEqual(new_inst['host'], self.compute.host)

        self.compute.terminate_instance(context,
            instance=jsonutils.to_primitive(instance))

    def test_relocate_with_no_image(self):
        old_host = 'foo'
        called = {'old_host': False}
        ...
        self.compute.run_instance(self.context, instance=instance)
        instance['host'] = old_host
        db.instance_update(self.context, instance['uuid'], {'host': old_host})
    ...
    def test_relocate_with_old_host_dead(self):
        old_host = 'foo'

        def old_host_rpc_call_timeout(*args, **kwargs):
            raise rpc_common.Timeout()

        self.stubs.Set(self.compute.compute_rpcapi, 'pre_relocate_instance',
                       old_host_rpc_call_timeout)
