...
class LibvirtConnTestCase(test.TestCase):
    ...
    def setUp(self):
        ...
        self.flags(enable_cpuqos=False)
        self.user_id = 'fake'
        self.project_id = 'fake'
        self.context = context.get_admin_context()
        self.flags(instances_path='')
        self.flags(libvirt_snapshots_directory='')
        self.flags(allow_image_snapshot_optimization=False)
        ...
        def fake_set_nbs_qos_on_create(*args, **kwargs):
            pass

        self.stubs.Set(libvirt_driver.disk, 'extend', fake_extend)
        self.stubs.Set(libvirt_driver.LibvirtDriver, '_set_nbs_qos_on_create',
                        fake_set_nbs_qos_on_create)
    ...
    def test_gen_cpu_qos_paras(self):
        guest = config.LibvirtConfigGuest()
        conn = libvirt_driver.LibvirtDriver(True)

        self.flags(enable_cpuqos=True)
        self.flags(ecu_ratio=8)
        self.flags(free_bursting_ecu_limit=5)
        self.flags(free_bursting_ecu_num=2)
        self.flags(cpu_period=200000)
        self.flags(cpu_shares=2048)
        self.flags(fine_tuning_factors=[1.2, 1.1, 1.0, 0.9])

        instance_type = {"vcpus": 1, "extra_specs": {"ecus_per_vcpu:": '1'}}
        conn._gen_cpu_qos_paras(guest, instance_type)
        self.assertEqual(guest.cpu_period, FLAGS.cpu_period)
        self.assertEqual(guest.cpu_shares, FLAGS.cpu_shares * 1 * 1)
        self.assertEqual(guest.cpu_quota, int(float(1 + 2) / 8 * 1.0 * 200000))

        instance_type = {"vcpus": 3, "extra_specs": {"ecus_per_vcpu:": '5'}}
        conn._gen_cpu_qos_paras(guest, instance_type)
        self.assertEqual(guest.cpu_period, FLAGS.cpu_period)
        self.assertEqual(guest.cpu_shares, FLAGS.cpu_shares * 3 * 5)
        self.assertEqual(guest.cpu_quota, int(float(5) / 8 * 0.9 * 200000))
    ...
    def test_update_image_online(self):
        # start test
        image_service = nova.tests.image.fake.FakeImageService()
        ...
        with utils.tempdir() as d:
            ...
            utils.execute('qemu-img', 'create', '-f', 'qcow2',
                          '-o', 'backing_file=%s' % base, disk)
            out_path = os.path.join(d, 'snapshot')
            sent_meta = {'name': snapshot_name, 'is_public': False,
                         'status': 'active', 'properties': properties}
            # Create new image. It will be updated in snapshot method
            # To work with it from snapshot, the single image_service is needed
            recv_meta = image_service.create(context, sent_meta)
            ...
            conn = libvirt_driver.LibvirtDriver(False)
            conn.update_image_online(self.context, instance_ref,
                                     power_state.PAUSED, out_path,
                                     disk, recv_meta['id'],
                                     image_service, sent_meta)

            snapshot = image_service.show(context, recv_meta['id'])
            self.assertEquals(snapshot['properties']['image_state'],
                              'available')
            self.assertEquals(snapshot['status'], 'active')
            self.assertEquals(snapshot['name'], snapshot_name)
    ...
    def test_get_host_ip_addr(self):
        ...
        def fake_get_host_intranet_ip_success():
            return self.expected_ip

        def fake_get_host_intranet_ip_failure():
            return None

        conn = libvirt_driver.LibvirtDriver(False)

        #Test use_intranet = False
        FLAGS.use_intranet = False
        ip = conn.get_host_ip_addr()
        self.assertEquals(ip, FLAGS.my_ip)

        #Test use_intranet = True, get ip success
        FLAGS.use_intranet = True
        libvirt_driver.host_intranet_ip = None
        self.stubs.Set(libvirt_driver,
                       'get_host_intranet_ip',
                       fake_get_host_intranet_ip_success)
        self.expected_ip = '10.0.0.11'
        ip = conn.get_host_ip_addr()
        self.assertEquals(ip, self.expected_ip)

        #Test use_intranet = True, get ip failure
        FLAGS.use_intranet = True
        libvirt_driver.host_intranet_ip = None
        self.stubs.Set(libvirt_driver,
                       'get_host_intranet_ip',
                       fake_get_host_intranet_ip_failure)
        self.assertRaises(exception.ComputeHostNotFound,
                          conn.get_host_ip_addr)
        FLAGS.use_intranet = False
...
class NWFilterTestCase(test.TestCase):
    ...
    def _create_instance_with_metadata(self, key, value):
        return db.instance_create(self.context,
                                  {'user_id': 'fake',
                                   'project_id': 'fake',
                                   'instance_type_id': 1,
    ...
    def _remove_instance(self, instance_id):
        return db.instance_destroy(self.context, instance_id)
    ...
    def _test_setup_basic_filtering(self, meta_key, meta_value, expect_filter):
        """ Test setup_basic_filtering work correctly with different metadata
        """
        defined = {'xml': None}
        ...
        self.stubs.Set(self.fw, '_define_filter', fake_define)

        if meta_key and meta_value:
            instance_ref = self._create_instance_with_metadata(meta_key,
                                                            meta_value)
        else:
            instance_ref = self._create_instance()
        ...
        self.assertNotEqual(-1, defined['xml'].find(expect_filter))

        self._remove_instance(instance_ref.uuid)

    def test_setup_basic_filtering_with_lvs_node(self):
        """ Test setup_basic_filtering work correctly with lvs node instance"""
        self._test_setup_basic_filtering(meta_key='netease-node',
                                         meta_value='1',
                                         expect_filter='nova-lvs-node')

    def test_setup_basic_filtering_with_lvs(self):
        """ Test setup_basic_filtering work correctly with lvs instance"""
        self._test_setup_basic_filtering(meta_key='netease-reserved',
                                         meta_value='loadbalancer',
                                         expect_filter='nova-lvs')

    def test_setup_basic_filtering_with_normal_instance(self):
        """ Test setup_basic_filtering work correctly with normal instance"""
        self._test_setup_basic_filtering(meta_key=None,
                                         meta_value=None,
                                         expect_filter='nova-base')
...
disk size: 4.4M''', ''))
    ...
    def test_make_remote_instance_dir(self):
        def fake_execute(*cmd, **kwargs):
            dry_cmd = ('rsync', '--sparse', '--compress', '--dirs',
                       '--dry-run', 'src', 'nova@host::nova/dest',
                       '--password-file=/etc/nova.pas', '--port=10873')
            r_cmd = ('rsync', '--sparse', '--compress', '--dirs',
                     'src', 'nova@host::nova/dest',
                     '--password-file=/etc/nova.pas', '--port=10873')
            cmd_ok = (cmd == dry_cmd or cmd == r_cmd)
            self.assertEquals(cmd_ok, True)
            self.assertEquals(kwargs.pop('run_as_root', False), True)

        src = 'src'
        dest = 'dest'
        host = 'host'
        self.stubs.Set(utils, 'execute', fake_execute)
        libvirt_utils.make_remote_instance_dir(src, host, dest)

    def test_clean_remote_dir(self):
        def fake_execute(*cmd, **kwargs):
            dry_cmd = ('rsync', '--delete', '--force', '--recursive',
                    '--dry-run', '--include=src', '--include=src/*',
                    '--exclude=*',
                    '/var/lib/nova/instances/', 'nova@host::nova',
                    '--password-file=/etc/nova.pas', '--port=10873')
            r_cmd = ('rsync', '--delete', '--force', '--recursive',
        ...
        self.stubs.Set(utils, 'execute', fake_execute)
        libvirt_utils.clean_remote_dir(src, host)

    def test_copy_image_to_remote(self):
        def fake_execute(*cmd, **kwargs):
            dry_cmd = ('rsync', '--sparse', '--compress', '--dry-run',
                       'src', 'nova@host::nova/dest',
                       '--password-file=/etc/nova.pas', '--port=10873')
            r_cmd = ('rsync', '--sparse', '--compress',
                     'src', 'nova@host::nova/dest',
                     '--password-file=/etc/nova.pas', '--port=10873')
            cmd_ok = (cmd == dry_cmd or cmd == r_cmd)
            self.assertEquals(cmd_ok, True)
        ...
        dest = 'dest'
        self.stubs.Set(utils, 'execute', fake_execute)
        libvirt_utils.copy_image_to_remote(src, dest, host)
...
class LibvirtDriverTestCase(test.TestCase):
    ...
    def test_migrate_disk_and_power_off(self):
        ...
        disk_info = [{'type': 'qcow2', 'path': '/test/disk',
                     ...
                     {'type': 'raw', 'path': '/test/disk.local',
                      ...
                      'disk_size':'83886080'}]
        #record the disk copy to the destination
        disks = []
        ...
        def fake_execute(*args, **kwargs):
            if(len(args) >= 3 and (args[0] == 'cp' or args[0] == 'mv')):
                disk = os.path.basename(args[2])
                if(disk == 'disk' or disk == 'disk.local'):
                    disks.append(disk)

        def fake_copy_image(from_path, des_path, host=None):
            disks.append(os.path.basename(des_path))

        def fake_copy_image_to_remote(src, dest, host):
            disks.append(os.path.basename(dest))
        ...
        self.stubs.Set(libvirt_utils, 'copy_image_to_remote',
                       fake_copy_image_to_remote)

        ins_ref = self._create_instance()
        #use keep_ephemeral default value(True)
        """ dest is different host case """
        disks = []
        out = self.libvirtconnection.migrate_disk_and_power_off(
               None, ins_ref, '10.0.0.2', None, None)
        self.assertEquals(out, disk_info_text)
        self.assertTrue('disk' in disks)
        self.assertTrue('disk.local' in disks)
        ...
        #use keep_ephemeral = True
        disks = []
        out = self.libvirtconnection.migrate_disk_and_power_off(
               None, ins_ref, '10.0.0.2', None, None, None, True)
        self.assertEquals(out, disk_info_text)
        self.assertTrue('disk' in disks)
        self.assertTrue('disk.local' in disks)

        #use keep_ephemeral = False
        disks = []
        out = self.libvirtconnection.migrate_disk_and_power_off(
               None, ins_ref, '10.0.0.2', None, None, None, False)
        self.assertEquals(out, disk_info_text)
        self.assertTrue('disk' in disks)
        self.assertTrue('disk.local' not in disks)
    ...
    def test_restore_ephemeral_disk(self):
        """Test for _restore_ephemeral_disk of libvirt driver"""
        #local cache for record current status
        kwargs = dict()

        def fake_rebase_cow_image(backing_file, target):
            backing_file = os.path.basename(backing_file)
            target = os.path.basename(target)
            kwargs['target'] = target + '_' + backing_file

        def fake_create_cow_image(backing_file, target):
            backing_file = os.path.basename(backing_file)
            target = os.path.basename(target)
            kwargs['target'] = target + '_' + backing_file

        def fake_get_disk_size(target):
            return long(kwargs['target'].split('_')[3]) * \
                    1024L * 1024L * 1024L

        def fake_create_ephemeral(backing_file, size, fslabel, os_tpye):
            if 'ephemeral' not in kwargs:
                kwargs['ephemeral'] = []
            kwargs['ephemeral'].append(os.path.basename(backing_file))

        def fake_get_backing_file_success(target):
            return 'ephemeral_0_' + kwargs['target'].split('_')[3] + '_None'

        def fake_get_backing_file_fail(target):
            return None

        def fake_ensure_tree(dir):
            pass

        def fake_exists(target):
            target = os.path.basename(target)
            #check disk
            if(target == 'disk' or target == 'disk.local'):
                return None != kwargs['target']
            #check base
            else:
                return False

        _context = context.get_admin_context()
        instance_0 = self._create_instance({'ephemeral_gb': 0})
        instance_2 = self._create_instance({'ephemeral_gb': 2})
        instance_5 = self._create_instance({'ephemeral_gb': 5})

        self.stubs.Set(libvirt_utils, 'rebase_cow_image',
                       fake_rebase_cow_image)
        self.stubs.Set(libvirt_utils, 'create_cow_image',
                       fake_create_cow_image)
        self.stubs.Set(libvirt_utils, 'get_disk_size',
                       fake_get_disk_size)
        self.stubs.Set(self.libvirtconnection, '_create_ephemeral',
                       fake_create_ephemeral)
        self.stubs.Set(libvirt_utils, 'get_disk_backing_file',
                       fake_get_backing_file_success)
        self.stubs.Set(os.path, 'exists', fake_exists)

        #test resize 2 to 5
        kwargs.clear()
        kwargs['target'] = 'disk.local_ephemeral_0_2_None'
        self.libvirtconnection._restore_ephemeral_disk(_context, instance_5,
                                                       'disk.local')
        self.assertTrue('ephemeral_0_2_None' in kwargs['ephemeral'])
        self.assertTrue('ephemeral_0_5_None' in kwargs['ephemeral'])
        self.assertEquals('disk.local_ephemeral_0_5_None',
                          kwargs['target'])

        #test migrate
        kwargs.clear()
        kwargs['target'] = 'disk.local_ephemeral_0_5_None'
        self.libvirtconnection._restore_ephemeral_disk(_context, instance_5,
                                                       'disk.local')
        self.assertTrue('ephemeral_0_5_None' in kwargs['ephemeral'])
        self.assertEquals('disk.local_ephemeral_0_5_None',
        ...
        kwargs.clear()
        kwargs['target'] = 'disk.local_ephemeral_0_5_None'
        self.libvirtconnection._restore_ephemeral_disk(_context, instance_2,
                                                       'disk.local')
        self.assertTrue('ephemeral_0_2_None' not in kwargs['ephemeral'])
        self.assertTrue('ephemeral_0_5_None' in kwargs['ephemeral'])
        self.assertEquals('disk.local_ephemeral_0_5_None',
                          kwargs['target'])
        ...
        kwargs.clear()
        kwargs['target'] = 'disk.local_ephemeral_0_5_None'
        self.libvirtconnection._restore_ephemeral_disk(_context, instance_0,
                                                       'disk.local')
        self.assertTrue('ephemeral_0_0_None' not in kwargs['ephemeral'])
        self.assertTrue('ephemeral_0_5_None' in kwargs['ephemeral'])
        self.assertEquals('disk.local_ephemeral_0_5_None',
                          kwargs['target'])

        #test no backing file,resize 2 to 5
        self.stubs.Set(libvirt_utils, 'get_disk_backing_file',
                       fake_get_backing_file_fail)
        kwargs.clear()
        kwargs['target'] = 'disk.local_2'
        self.assertRaises(exception.DiskNotFound,
                          self.libvirtconnection._restore_ephemeral_disk,
                          _context, instance_5, 'disk.local')

    def test_create_snapshot_image(self):
        """Test for nova.virt.libvirt.connection.LibvirtConnection.
        _create_snapshot_image. """
        with utils.tempdir() as tmpdir:
            self.flags(instances_path=tmpdir)

            def fake_fetch_orig_image_ok(context, target, image_id,
                                            user_id, project_id):
                if not os.path.exists(os.path.dirname(target)):
                    os.system("mkdir -p " + os.path.dirname(target))
                os.system("echo " + image_id + ">>" + target)

            def fake_fetch_orig_image_fail(context, target, image_id,
                                        user_id, project_id):
                if os.path.exists(target):
                    os.remove(target)

            def fake_get_disk_backing_file_ok(target):
                os.system("touch %s/_base/1115711157.backing" % tmpdir)
                return "1115711157.backing"

            def fake_get_disk_backing_file_fail(target):
                return None

            def fake_copy_image(src, target):
                os.system("cp " + src + " " + target)

            def fake_extend(img, size):
                os.system("touch %s/_base/1115711157.extend" % tmpdir)

            def fake_rebase_cow_image(backing_file, cow):
                os.system("touch %s/_base/1115711157.rebase" % tmpdir)
            ...
            image_id = "image-fake"
            target = "%(tmpdir)s/1115711157.cow" % locals()
            if os.path.exists(target):
            ...
            # everything is OK
            self.stubs.Set(libvirt_utils, 'fetch_orig_image',
                            fake_fetch_orig_image_ok)
            self.stubs.Set(libvirt_utils, 'get_disk_backing_file',
                            fake_get_disk_backing_file_ok)
            self.stubs.Set(utils, 'ensure_tree', fake_ensure_tree)
            self.stubs.Set(libvirt_utils, 'copy_image', fake_copy_image)
            self.stubs.Set(libvirt_utils, 'rebase_cow_image',
                        fake_rebase_cow_image)
            self.stubs.Set(disk, 'extend', fake_extend)
            self.stubs.Set(libvirt_utils, "get_disk_size", fake_get_disk_size)
            self.libvirtconnection._create_snapshot_image(_context, instance,
                                                        image_id, target, size)
            self.assertTrue(os.path.exists(target))
            self.assertTrue(os.path.exists(
                "%(tmpdir)s/_base/1115711157.backing" % locals()))
            self.assertTrue(os.path.exists(
                "%(tmpdir)s/_base/1115711157.backing_1" % locals()))
            self.assertTrue(os.path.exists(
                "%(tmpdir)s/_base/1115711157.extend" % locals()))
            self.assertTrue(os.path.exists(
                "%(tmpdir)s/_base/1115711157.rebase" % locals()))

            # fetch_orig_image fail
            if os.path.exists(target):
                if os.path.isdir(target):
                    shutil.rmtree(target)
                else:
                    os.remove(target)
            self.stubs.Set(libvirt_utils, 'fetch_orig_image',
                            fake_fetch_orig_image_fail)
            self.assertRaises(exception.CouldNotFetchImage,
                    self.libvirtconnection._create_snapshot_image,
                    _context, instance, image_id, target, size)
            self.stubs.Set(libvirt_utils, 'fetch_orig_image',
                            fake_fetch_orig_image_ok)

            # get_disk_backing_file fail
            if os.path.exists(target):
                if os.path.isdir(target):
                    shutil.rmtree(target)
                else:
                    os.remove(target)
            self.stubs.Set(libvirt_utils, 'get_disk_backing_file',
                            fake_get_disk_backing_file_fail)
            self.assertRaises(exception.ImageUnacceptable,
                    self.libvirtconnection._create_snapshot_image,
                    _context, instance, image_id, target, size)
            self.stubs.Set(libvirt_utils, 'get_disk_backing_file',
    ...
    def test_get_base_image_by_backing_file(self):
        """Test for nova.virt.libvirt.connection.LibvirtConnection.
        _get_base_image_by_backing_file. """
        class FakeImageService():
            def get_image_properties(self, context, name, value):
                if value == "base_image_id_sha1":
                    return [{"image_id":"image-fake"}]
                else:
                    return None

        def fake_get_image_service(context, image_id):
            return FakeImageService(), image_id

        def fake_fetch_image_ok(context, target, image_id,
                                        user_id, project_id):
            if not os.path.exists(os.path.dirname(target)):
                os.system("mkdir -p " + os.path.dirname(target))
            os.system("echo " + image_id + ">>" + target)

        def fake_fetch_image_fail(context, target, image_id,
                                      user_id, project_id):
            if os.path.exists(target):
                os.remove(target)
        ...
        image_id = "image-fake"
        with utils.tempdir() as d:
            backing_file_ok = os.path.join(d, 'base_image_id_sha1')
            backing_file_fail = os.path.join(d, 'base_image_id_sha1_fail')

            # everything is OK
            self.stubs.Set(glance, 'get_remote_image_service',
                            fake_get_image_service)
            self.stubs.Set(libvirt_utils, 'fetch_image', fake_fetch_image_ok)
            self.libvirtconnection._get_base_image_by_backing_file(_context,
                                        instance, image_id, backing_file_ok)
            self.assertTrue(os.path.exists(backing_file_ok))

            # get_image_properties fail
            if os.path.exists(backing_file_fail):
                os.remove(backing_file_fail)
            self.assertRaises(exception.ImageNotFound,
                self.libvirtconnection._get_base_image_by_backing_file,
                _context, instance, image_id, backing_file_fail)

            # fetch_image fail
            if os.path.exists(backing_file_ok):
                os.remove(backing_file_ok)
            self.stubs.Set(libvirt_utils, 'fetch_image', fake_fetch_image_fail)
            self.assertRaises(exception.CouldNotFetchImage,
                self.libvirtconnection._get_base_image_by_backing_file,
                _context, instance, image_id, backing_file_ok)

    def test_resume_personality(self):
        """
        Test for nova.virt.libvirt.driver.LibvirtDriver._resume_personality
        """
        def fake_instance_system_metadata_extension_get_normal(context,
                                                               instance_uuid):
            return {'personality': '[["test_path", "test_content"]]'}

        def fake_instance_system_metadata_extension_get_empty(context,
                                                              instance_uuid):
            return {}

        def fake_instance_system_metadata_extension_get_exception(
                                context, instance_uuid):
            raise Exception()

        def fake_instance_system_metadata_extension_update(
                            context, instance_uuid, metadata, delete=False):
            pass
        ...
        # when personality is not None and injected_files is []
        self.stubs.Set(db, 'instance_system_metadata_extension_get',
                       fake_instance_system_metadata_extension_get_normal)
        result = self.libvirtconnection._resume_personality(_context,
                                            fake_instance, injected_files)
        self.assertEqual([["test_path", "test_content"]], result)

        # when personality is not None and injected_files is not []
        injected_files_not_empty = [["ori_path", "ori_content"]]
        self.stubs.Set(db, 'instance_system_metadata_extension_update',
                       fake_instance_system_metadata_extension_update)
        self.libvirtconnection._resume_personality(_context, fake_instance,
                                                   injected_files_not_empty)
        self.assertEqual([["ori_path", "ori_content"]],
                         injected_files_not_empty)

        # when personality is None
        self.stubs.Set(db, 'instance_system_metadata_extension_get',
                       fake_instance_system_metadata_extension_get_empty)
        result = self.libvirtconnection._resume_personality(_context,
                                            fake_instance, injected_files)
        self.assertEqual([], result)

        # when exception accurs
        self.stubs.Set(db, 'instance_system_metadata_extension_get',
                       fake_instance_system_metadata_extension_get_exception)
        result = self.libvirtconnection._resume_personality(_context,
                                            fake_instance, injected_files)
        self.assertEqual([], result)

    def test_inject_files_from_backend(self):
        """
        Test for
        nova.virt.libvirt.driver.LibvirtDriver._inject_files_from_backend
        """
        content = 'test_content'
        content_encode = base64.b64encode(content)

        def fake_get_inject_files_normal():
            return {'inject_files': [{'path': 'test_path',
                                      'contents': content_encode}]}

        def fake_get_inject_files_Invalid():
            raise exception.Invalid()

        def fake_get_inject_files_exception():
            raise Exception()

        def fake_get_inject_files_KeyError():
            return {'inject_files': [{'path2': 'test_path',
                                      'contents': content_encode}]}

        fake_instance = {'uuid': 'fake_uuid'}
        injected_files = []

        # when injected_files is []
        self.stubs.Set(inject_files, 'get_inject_files',
                       fake_get_inject_files_normal)
        result = self.libvirtconnection._inject_files_from_backend(
                                            fake_instance, injected_files)
        self.assertEqual([["test_path", content]], result)

        # when injected_files is not []
        injected_files_not_None = [['ori_path', 'ori_content']]
        self.stubs.Set(inject_files, 'get_inject_files',
                       fake_get_inject_files_normal)
        result = self.libvirtconnection._inject_files_from_backend(
                                    fake_instance, injected_files_not_None)
        self.assertEqual([['ori_path', 'ori_content'],
                          ["test_path", content]], result)

        # when get_inject_files Invalid occurs
        injected_files_not_None = [['ori_path', 'ori_content']]
        self.stubs.Set(inject_files, 'get_inject_files',
                       fake_get_inject_files_Invalid)
        result = self.libvirtconnection._inject_files_from_backend(
                                    fake_instance, injected_files_not_None)
        self.assertEqual([['ori_path', 'ori_content']], result)

        # when get_inject_files exception occurs
        injected_files = []
        self.stubs.Set(inject_files, 'get_inject_files',
                       fake_get_inject_files_exception)
        result = self.libvirtconnection._inject_files_from_backend(
                                            fake_instance, injected_files)
        self.assertEqual([], result)

        # when get_inject_files KeyError occurs
        injected_files = []
        self.stubs.Set(inject_files, 'get_inject_files',
                       fake_get_inject_files_KeyError)
        result = self.libvirtconnection._inject_files_from_backend(
                                            fake_instance, injected_files)
        self.assertEqual([], result)

    def test_get_device_for_nbs_volume(self):
        ...
        used_dev = {'dev': [], 'slot': []}
        result = self.libvirtconnection.get_device_for_nbs_volume(
                                                'fake-instance-name', used_dev)
        self.assertEqual(result, {'mountpoint': '/dev/vde',
                                  'real_path': '/dev/nbs/xdjg',
                                  'slot': 30})

        used_dev = {'dev': ['vde'], 'slot': [30]}
        result = self.libvirtconnection.get_device_for_nbs_volume(
                                                'fake-instance-name', used_dev)
        self.assertEqual(result, {'mountpoint': '/dev/vdf',
                                  'real_path': '/dev/nbs/xdiy',
                                  'slot': 29})

        used_dev = {'dev': ['vdf', 'vdg'], 'slot': [30, 28, 27]}
        result = self.libvirtconnection.get_device_for_nbs_volume(
                                                'fake-instance-name', used_dev)
        self.assertEqual(result, {'mountpoint': '/dev/vde',
                                  'real_path': '/dev/nbs/xdiy',
    ...
    def test_get_vcpu_pin_set(self):
        # correct syntax
        self.flags(vcpu_pin_set="4-$")
        self.libvirtconnection._get_vcpu_pin_set(5)
        self.assertEqual("4-4", self.libvirtconnection._vcpu_pin_set)
        self.assertEqual(1, self.libvirtconnection._vcpu_total)

        self.flags(vcpu_pin_set="4 -  $")
        self.libvirtconnection._get_vcpu_pin_set(20)
        self.assertEqual("4-19", self.libvirtconnection._vcpu_pin_set)
        self.assertEqual(16, self.libvirtconnection._vcpu_total)
        ...
        # invalid syntax
        self.flags(vcpu_pin_set="1-2,4-$,^8")
        self.assertRaises(exception.Invalid,
                        self.libvirtconnection._get_vcpu_pin_set, 20)

        self.flags(vcpu_pin_set="4-$,^8")
        self.assertRaises(exception.Invalid,
                        self.libvirtconnection._get_vcpu_pin_set, 20)

        self.flags(vcpu_pin_set="1-2,4-$")
        self.assertRaises(exception.Invalid,
                        self.libvirtconnection._get_vcpu_pin_set, 20)

        self.flags(vcpu_pin_set="1-2,4-$,1")
        self.assertRaises(exception.Invalid,
                        self.libvirtconnection._get_vcpu_pin_set, 20)

        self.flags(vcpu_pin_set="1-2,4-$,5-10")
        self.assertRaises(exception.Invalid,
                        self.libvirtconnection._get_vcpu_pin_set, 20)

        self.flags(vcpu_pin_set="2,3,4-$")
        self.assertRaises(exception.Invalid,
                        self.libvirtconnection._get_vcpu_pin_set, 20)
        ...
        self.flags(vcpu_pin_set="4-$")
        self.assertRaises(exception.Invalid,
                        self.libvirtconnection._get_vcpu_pin_set, 2)

        self.flags(vcpu_pin_set="4-$")
        self.assertRaises(exception.Invalid,
                        self.libvirtconnection._get_vcpu_pin_set, 4)
    ...
    def test_set_nbs_qos_on_create_with_total_bps(self):
        dom_xml = '''
            <domain type='kvm' id='18'>
              ...
              <devices>
                <disk type='block' device='disk'>
                  ...
                  <source dev='/dev/mapper/nbdisk01'/>
        ...
        def fake_get_instance_blkio_cgroup_path(name, uuid):
            return '/sys/fs/cgroup/blkio/libvirt/qemu/fake-name/'

        def fake_block_device_mapping_get_all_by_instance(*args, **kwargs):
            return [{"connection_info": '{"host_dev": "/dev/mapper/nbdisk01", '
                        '"qos_info": {"iotune_total_bytes": 123}}'}]

        def fake_get_dev_num(*args, **kwargs):
            return '10:1'

        def fake_execute(*args, **kwargs):
            called['execute'] = True
            self.assertEqual('tee', args[0])
            self.assertIn('/sys/fs/cgroup/blkio/libvirt/qemu'
                          '/fake-name/blkio.throttle', args[1])
            self.assertTrue(kwargs['run_as_root'])
            self.assertEqual('10:1 123\n', kwargs['process_input'])

        self.stubs.Set(utils, 'get_instance_blkio_cgroup_path',
                       fake_get_instance_blkio_cgroup_path)
        self.stubs.Set(self.libvirtconnection, '_get_dev_num',
                        fake_get_dev_num)
        self.stubs.Set(utils, 'execute', fake_execute)
        self.stubs.Set(db, 'block_device_mapping_get_all_by_instance',
                       fake_block_device_mapping_get_all_by_instance)

        self.libvirtconnection._set_nbs_qos_on_create(dom_xml)
        self.assertTrue(called['execute'])
