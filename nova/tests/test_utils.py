...
class ModprobeTestCase(test.TestCase):
    def test_modprobe_module(self):
        loaded_module = ['vhost_net']
        module_1 = 'vhost_net'
        module_2 = 'nbd nbds_max=32'
        self.module = None
        ...
        def fake_execute(*cmd, **kwargs):
            ...
            if 'modprobe' in cmd:
                exe_cmd = ['modprobe']
                exe_cmd.extend(self.module)
                self.assertEqual(list(cmd), exe_cmd)
                return '', ''

        self.stubs.Set(utils, 'execute', fake_execute)

        self.module = module_1.split()
        utils.modprobe_module(self.module)

        self.module = module_2.split()
        utils.modprobe_module(self.module)
...
class GenericUtilsTestCase(test.TestCase):
    ...
    def test_bool_from_string(self):
        ...
        self.assertTrue(utils.bool_from_string('Yes'))
        self.assertTrue(utils.bool_from_string('YeS'))
        self.assertTrue(utils.bool_from_string('y'))
        self.assertTrue(utils.bool_from_string('Y'))
        self.assertTrue(utils.bool_from_string(True))
        self.assertTrue(utils.bool_from_string(1))
        ...
        self.assertFalse(utils.bool_from_string('No'))
        self.assertFalse(utils.bool_from_string('n'))
        self.assertFalse(utils.bool_from_string('N'))
        self.assertFalse(utils.bool_from_string('0'))
        self.assertFalse(utils.bool_from_string(False))
        self.assertFalse(utils.bool_from_string(0))

        self.assertTrue(utils.bool_from_string(None))

        self.assertRaises(ValueError, utils.bool_from_string, 'test')
        self.assertRaises(ValueError, utils.bool_from_string, '11')
        self.assertRaises(ValueError, utils.bool_from_string, '-1')
...
class ProcessLoopingCallTestCase(test.TestCase):
    def test_work_correctly(self):
        def _inner():
            print '.'
            1 == 1 + 1

        poll = utils.ProcessLoopingCall(f=_inner)
        poll.start(interval=1)
        poll.stop()
        time.sleep(1)
        # Make sure forked process is dead.
        self.assertFalse(poll._process.is_alive())
