...
class ServersControllerTest(test.TestCase):
    ...
    def setUp(self):
        ...
        for name, value in all_types.iteritems():
            inst_type = nova.db.instance_type_get_by_name(admin_context, name)
            ext_spec = {'ecus_per_vcpu:': value}
            nova.db.instance_type_extra_specs_update_or_create(admin_context,
                                                    inst_type["flavorid"],
                                                    ext_spec)
...
class ServersControllerCreateTest(test.TestCase):
    ...
    def test_create_instance_above_quota_local_gb(self):
        msg = _('Quota exceeded for local_gb: Requested 40, but'
                ' already used 9 of 10 local_gb')
        self._do_test_create_instance_above_quota('local_gb', 1, 10, msg)
