...
class ServerActionsControllerTest(test.TestCase):
    ...
    def test_resize_server(self):
        ...
        body_default = dict(resize=dict(flavorRef="http://localhost/3"))

        def resize_mock(mock, context, instance, flavor_id=None,
                        keep_ephemeral=True, **kwargs):
            self.resize_called = True
            self.keep_ephemeral = keep_ephemeral

        self.stubs.Set(nova.compute.api.API, 'resize', resize_mock)

        #test default(without specify keep_ephemeral)
        self.resize_called = False
        self.keep_ephemeral = None
        req = fakes.HTTPRequest.blank(self.url)
        self.controller._action_resize(req, FAKE_UUID, body_default)
        self.assertEqual(self.resize_called, True)
        self.assertEqual(self.keep_ephemeral, True)
        #test keep_ephemeral = True
        for keep_ephemeral in ('True', 'true', '1', 1, True, None,
                                'Yes', 'yes', 'Y', 'y'):
            self.resize_called = False
            self.keep_ephemeral = None
            body_with_true = dict(resize=dict(
                                flavorRef="http://localhost/3",
                                keep_ephemeral=keep_ephemeral))
            self.controller._action_resize(req, FAKE_UUID, body_with_true)
            self.assertEqual(self.resize_called, True)
            self.assertEqual(self.keep_ephemeral, True)

        #test keep_ephemeral = False
        for keep_ephemeral in ('False', 'false', '0', 0, False,
                               'No', 'no', 'N', 'n'):
            self.resize_called = False
            self.keep_ephemeral = None
            body_with_false = dict(resize=dict(
                                flavorRef="http://localhost/3",
                                keep_ephemeral=keep_ephemeral))
            self.controller._action_resize(req, FAKE_UUID, body_with_false)
            self.assertEqual(self.resize_called, True)
            self.assertEqual(self.keep_ephemeral, False)

        #test invalid input
        for keep_ephemeral in ('test', 10, -1):
            self.resize_called = False
            self.keep_ephemeral = None
            body_with_invalid = dict(resize=dict(
                                flavorRef="http://localhost/3",
                                keep_ephemeral=keep_ephemeral))
            self.assertRaises(ValueError, self.controller._action_resize,
                              req, FAKE_UUID, body_with_invalid)
...
<createImage xmlns="http://docs.openstack.org/compute/api/v1.1"
    ...
    def test_resize(self):
        ...
        def get_serial_request(keep_ephemeral):
        ...
        for param in ['True', 'true', 'Yes', 'yes', 'Y', 'y', '1', True, 1]:
            request = self.deserializer.deserialize(
                get_serial_request(param), 'action')
            self.assertEquals(request['body'], expected_True)

        expected_False = {
            "resize": {"flavorRef": "http://localhost/flavors/3",
                       'keep_ephemeral': False},
        }
        for param in ['False', 'false', 'No', 'no', 'N', 'n', '0', False, 0]:
            request = self.deserializer.deserialize(
                    get_serial_request(param), 'action')
            self.assertEquals(request['body'], expected_False)

        self.assertRaises(ValueError,
            self.deserializer.deserialize,
                          get_serial_request('test'), 'action')
