...
class KeypairsTest(test.TestCase):
    ...
    def setUp(self):
        ...
        self.stubs.Set(db, "key_pair_destroy",
                       db_key_pair_destroy)
        self._extended_keypairs_stub()

    def _extended_keypairs_stub(self):

        def fake_extended_create(self, req, resp_obj, body):
            pass

        def fake_extended_show(self, req, resp_obj, id):
            pass

        def fake_extended_index(self, req, resp_obj):
            pass

        self.stubs.Set(extended_keypairs.ExtendedKeypairsController,
                       "create",
                       fake_extended_create)
        self.stubs.Set(extended_keypairs.ExtendedKeypairsController,
                       "show",
                       fake_extended_show)
        self.stubs.Set(extended_keypairs.ExtendedKeypairsController,
...
class KeypairPolicyTest(test.TestCase):
    ...
    def test_keypair_list_fail_policy(self):
        rules = {'compute_extension:keypairs:index': [['role:admin']]}
    ...
    def test_keypair_show_fail_policy(self):
        rules = {'compute_extension:keypairs:show': [['role:admin']]}
        policy.set_rules(rules)
    ...
    def test_keypair_show_pass_policy(self):
        def _db_key_pair_get(context, user_id, name):
            return {'name': name, 'public_key': 'XXX', 'fingerprint': 'YYY'}
    ...
    def test_keypair_create_fail_policy(self):
        rules = {'compute_extension:keypairs:create': [['role:admin']]}
        policy.set_rules(rules)
        req = fakes.HTTPRequest.blank('/v2/fake/os-keypairs')
        req.method = 'POST'
        self.assertRaises(exception.NotAuthorized,
    ...
    def test_keypair_create_pass_policy(self):
        body = {'keypair': {'name': 'create_test_policy'}}
    ...
    def test_keypair_delete_fail_policy(self):
        rules = {'compute_extension:keypairs:delete': [['role:admin']]}
        policy.set_rules(rules)
        req = fakes.HTTPRequest.blank('/v2/fake/os-keypairs/FAKE')
        req.method = 'DELETE'
        self.assertRaises(exception.NotAuthorized,
