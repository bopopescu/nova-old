...
class AdminActionsTest(test.TestCase):
    ...
    def test_migrate_force_host(self):

        def fake_compute_api_resize(mock, context, instance,
                                    flavor_id=None, keep_ephemeral=True,
                                    force_host=None, **kwargs):
            self.actual_force_host = force_host

        app = fakes.wsgi_app()
        self.stubs.Set(compute.API, 'resize', fake_compute_api_resize)
        expected_force_host = '10.0.0.2'
        req = webob.Request.blank('/v2/fake/servers/%s/action' %
                  self.UUID)
        req.method = 'POST'
        req.body = jsonutils.dumps({'migrate': {
            'force_host': expected_force_host}})
        req.content_type = 'application/json'
        res = req.get_response(app)
        self.assertEqual(res.status_int, 202)
        self.assertEqual(self.actual_force_host, expected_force_host)
        self.stubs.Set(compute.API, 'resize', fake_compute_api)
...
class RelocateTest(test.TestCase):
    def setUp(self):
        super(RelocateTest, self).setUp()
        self.uuid = utils.gen_uuid()

        def fake_get(self, context, instance_id):
            return dict(id=1, uuid=instance_id, vm_state=vm_states.ACTIVE)

        def fake_schedule_relocate(self, context, inst, image):
            pass

        self.stubs.Set(compute.API,
                       'get', fake_get)
        self.stubs.Set(compute.API,
                       'schedule_relocate', fake_schedule_relocate)
        ...
        url = '/fake/servers/%s/action' % self.uuid
        self.request = fakes.HTTPRequest.blank(url)
        self.request.environ['nova.context'] = ctxt

    def test_relocate_not_specified_image(self):
        body = {'os-relocate': None}
        res = self.admin_api._relocate(self.request, 'inst_id', body)
    ...
    def test_relocate_specified_image(self):
        body = {'os-relocate': {'imageRef': 'fakeimage'}}
        res = self.admin_api._relocate(self.request, 'inst_id', body)
