...
def fake_compute_node_get_all(context):
    return [{'service': {'host': 'host-1'}, 'hypervisor_hostname': 'host-1'},
            {'service': {'host': 'host-2'}, 'hypervisor_hostname': 'host-2'}]
...
class ExtendedServerAttributesTest(test.TestCase):
    ...
    def test_detail_list_optimization(self):
        url = '/v2/fake/servers/detail?list_optimization=1'
        req = webob.Request.blank(url)
        req.headers['Accept'] = self.content_type
        res = req.get_response(fakes.wsgi_app())
        self.assertEqual(res.status_int, 200)
