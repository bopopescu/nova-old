...
def quota_set(id):
    return {'quota_set': {'id': id, 'metadata_items': 128, 'volumes': 10,
            ...
            'instances': 10, 'injected_files': 5, 'cores': 20, 'ecus': 50,
            'injected_file_content_bytes': 10240, 'local_gb': -1,
...
class QuotaSetsTest(test.TestCase):
    ...
    def test_quotas_index_as_admin(self):
        req = fakes.HTTPRequest.blank('/v2/fake4/os-quota-sets',
                                      use_admin_context=True)
        res_dict = self.controller.index(req)

        # return empty for no usages or no updates
        self.assertEqual(res_dict, {})

    def test_quotas_index_after_update(self):
        body = {'quota_set': {'instances': 50, 'cores': 50, 'ecus': 50,
        ...
        req = fakes.HTTPRequest.blank('/v2/fake4/os-quota-sets',
                                      use_admin_context=True)
        res_dict = self.controller.index(req)
        to_compare = {u"update_me": {}}
        for k in body['quota_set']:
            if k == 'id':
                continue
            to_compare[u'update_me'][k] = {
                                      'reserved': 0,
                                      'limit': body['quota_set'][k],
                                      'in_use': 0}
        self.assertEqual(res_dict, to_compare)

    def test_quotas_index_as_unauthorized_user(self):
        req = fakes.HTTPRequest.blank('/v2/fake4/os-quota-sets')
    ...
    def test_quotas_update_invalid_body(self):
        body = {'NOT-VALID': {'instances': 0}}
        req = fakes.HTTPRequest.blank('/v2/fake4/os-quota-sets/update_me',
                                      use_admin_context=True)
    ...
    def test_quotas_update_invalid_value(self):
        expected_resp = {'quota_set': {
                              ...
                              'security_groups': 10,
                              'security_group_rules': 20,
                              'key_pairs': 100, 'ecus': 50,
                              'gigabytes': 1000, 'volumes': 10}}
