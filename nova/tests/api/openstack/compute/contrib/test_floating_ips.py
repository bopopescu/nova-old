...
# test floating ip add/remove -> associate/disassociate
    ...
    def test_floating_ip_associate_during_resize(self):
        def fake_compute_api_get(self, context, instance_id):
            return dict(uuid=FAKE_UUID, id=instance_id,
                        instance_type_id=1, host='bob',
                        vm_state='active', task_state='resize_migrated')
    ...
    def test_floating_ip_disassociate(self):
        ...
        def fake_floating_ip_get_by_address(context, address):
            return {'fixed_ip_id': 'fake_fixed_ip_id'}

        def fake_get_instance_by_floating_ip(self, context,
                                            address, floating_ip):
            instance = {'uuid': 'test_inst', 'id': 'test_inst'}
            return instance

        self.stubs.Set(db, 'floating_ip_get_by_address',
                           fake_floating_ip_get_by_address)
        self.stubs.Set(floating_ips, 'get_instance_by_floating_ip',
                           fake_get_instance_by_floating_ip)
...
# these are a few bad param tests
    ...
    def test_remove_floating_ip_bad_address(self):
        body = dict(removeFloatingIp='11.0.0.1')
        req = fakes.HTTPRequest.blank('/v2/fake/servers/test_inst/action')
        self.assertRaises(webob.exc.HTTPBadRequest,
                   self.manager._remove_floating_ip, req, 'test_inst',
