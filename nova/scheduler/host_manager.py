...
class HostState(object):
    ...
    def __init__(self, host, topic, capabilities=None, service=None):
        ...
        self.private_network_mbps_used = 0
        self.public_network_mbps_used = 0
        self.total_private_network_mbps = 0
        self.total_public_network_mbps = 0
    ...
    def update_from_compute_node(self, compute):
        ...
        cpu_info = jsonutils.loads(compute['cpu_info'])
        if cpu_info.get('ecu_ratio'):
            self.ecu_ratio = cpu_info.get('ecu_ratio', 0)
            self.ecus_total = cpu_info.get('ecus', 0)
            self.ecus_used = cpu_info.get('ecus_used', 0)
            self.free_ecus = self.ecus_total - self.ecus_used

        self.private_network_mbps_used = compute.get(
                                            'private_network_mbps_used', 0)
        self.public_network_mbps_used = compute.get(
                                            'public_network_mbps_used', 0)
        self.total_private_network_mbps = compute.get(
                                            'total_private_network_mbps', 0)
        self.total_public_network_mbps = compute.get(
                                            'total_public_network_mbps', 0)

    def _consume_network_bandwith_from_instance(self, instance):
        private_network_bandwith = instance['private_network_bandwith']
        public_network_bandwith = instance['public_network_bandwith']

        LOG.debug(_('instance consume netqos: %(private_network_bandwith)s, '
                    '%(public_network_bandwith)s') % locals())

        self.private_network_mbps_used += private_network_bandwith
        self.public_network_mbps_used += public_network_bandwith

    def consume_from_instance(self, instance, inst_type=None):
        ...
        #                enable or not, because it is only reported when
        #                cpu qos is enabled on the host.
        if self.ecu_ratio and inst_type is not None:
            ecus_per_vcpu = int(inst_type['extra_specs']['ecus_per_vcpu:'])
            ecus_total = inst_type['vcpus'] * ecus_per_vcpu
            self.ecus_used += ecus_total
            self.free_ecus -= ecus_total

        if FLAGS.use_network_qos:
            self._consume_network_bandwith_from_instance(instance)
