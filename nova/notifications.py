...
def _send_instance_update_notification(context, instance, old_vm_state=None,
    ...
    if old_display_name is not None:
        payload["old_display_name"] = old_display_name
        try:
            nw_info = jsonutils.loads(instance["info_cache"]["network_info"])
            fixed_ip = nw_info[0]["network"]["subnets"][0]["ips"][0]["address"]
        except (KeyError, IndexError, TypeError):
            payload["fixed_ips"] = [{"address": None}]
        else:
            payload["fixed_ips"] = [{"address": fixed_ip}]
