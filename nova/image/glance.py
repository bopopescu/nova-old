...
def _create_glance_client(context, host, port, use_ssl, version=1):
    ...
    if context:
        log_id = context.to_dict().get('unified_log_id', None)
        log_seq = context.to_dict().get('unified_log_seq', None)
    if log_id and log_seq:
        log_seq_nums = log_seq.split('.')
        log_seq_nums[-1] = str(int(log_seq_nums[-1]) + 1)
        log_seq = '.'.join(log_seq_nums)
        context.unified_log_seq = log_seq

    params['unified_log_id'] = log_id
    params['unified_log_seq'] = log_seq
...
class GlanceImageService(object):
    ...
    def get_image_properties(self, context, name, value):
        """Get image properties by given name and value in image_properties
        table in glance database."""
        data = self._client.call(context, 1, 'get_image_properties',
                                 name, value)
        return data
    ...
    def update(self, context, image_id, image_meta, data=None,
            purge_props=True, features=None):
        ...
        try:
            image_meta = self._client.call(context, 1, 'update',
                                           image_id, features, **image_meta)
        except Exception:
            _reraise_translated_image_exception(image_id)
        else:
            return self._translate_from_glance(image_meta)

    def delete(self, context, image_id, **kwargs):
        ...
        """
        try:
            self._client.call(context, 1, 'delete', image_id, **kwargs)
        except glanceclient.exc.Forbidden, ex:
            explanation = ex.details
...
def _convert_timestamps_to_datetimes(image_meta):
    ...
    for attr in ['created_at', 'updated_at', 'deleted_at']:
        if image_meta.get(attr):
            try:
                image_meta[attr] = timeutils.parse_local_strtime(
                                                            image_meta[attr])
            except Exception:
                msg = (_("parse local time format error,"
                              "time string is %s") % image_meta[attr])
                LOG.debug(msg)
