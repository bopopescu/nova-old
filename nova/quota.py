...
quota_opts = [
    ...
    cfg.IntOpt('quota_ecus',
               default=50,
               help='number of instance ecus allowed per project'),
    ...
    cfg.IntOpt('quota_private_floating_ips',
               default=10,
               help='number of private floating ips allowed per project'),
    cfg.IntOpt('quota_local_gb',
               default=-1,
               help='gigabytes of local storage allowed per project'),
    ...
    cfg.IntOpt('quota_public_network_bandwidth',
               default=30,
               help='public network bandwidth per project'),
...
class QuotaEngine(object):
    ...
    def get_quotas(self, context, quota_class=None,
                           defaults=True, usages=True):
        """Retrieve the quotas of all projects.
...
def _sync_ecus(context, project_id, session):
    instances = db.instance_get_all_by_project_with_session(context,
                                   project_id, session)

    def get_ecus(instance):
        inst_type = instance.instance_type
        for ext_spec in inst_type["extra_specs"]:
            if ext_spec.key == "ecus_per_vcpu:":
                return inst_type["vcpus"] * int(ext_spec.value)
        LOG.warning(_("ecus_per_vcpu NOT found in instance_type %s") %
                    inst_type["name"], instance=instance)
        return 0
    return sum([get_ecus(inst) for inst in instances])
...
def _sync_private_floating_ips(context, project_id, session):
    return dict(private_floating_ips=db.floating_ip_count_by_project_type(
            context, project_id, True, FLAGS.private_floating_pools,
            session=session))
...
def _sync_public_network_bandwidth(context, project_id, session):
    return dict(public_network_bandwidth=db.public_bandwidth_count_by_project(
                                                    context,
                                                    project_id,
                                                    session=session))


def _sync_private_network_bandwidth(context, project_id, session):
    return dict(private_network_bandwidth=db.\
                                          private_bandwidth_count_by_project(
                                              context,
                                              project_id,
                                              session=session))
...
resources = [
    ...
    ReservableResource('ecus', _sync_instances, 'quota_ecus'),
    ...
    CountableResource('key_pairs', db.key_pair_count_by_user,
                      'quota_key_pairs'),
    ReservableResource('local_gb', _sync_instances, 'quota_local_gb'),
    ]

if FLAGS.use_private_floating_ip:
    resources.append(ReservableResource('private_floating_ips',
                _sync_private_floating_ips, 'quota_private_floating_ips'))

if FLAGS.use_network_qos:
    resources.append(ReservableResource('public_network_bandwidth',
                                        _sync_public_network_bandwidth,
                                        'quota_public_network_bandwidth'))
